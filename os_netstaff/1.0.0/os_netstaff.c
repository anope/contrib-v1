...
#define VERSION "$Id: os_netstaff.c v1.0.0 17-10-2007 n00bie $"
...
************************************************************************************************
** Tested: Anope-1.7.19, Unreal3.2.7
**
** Providing Command:
**
** /msg OperServ HELP NETSTAFF
** /msg OperServ [ADD|DEL|CLEAR] [nick]
...
int doLoadConfig(void)
{
	...
	Directive directivas[] = {
		{"NetStaffDBName", {{PARAM_STRING, PARAM_RELOAD, &tmp}}},
	};
    ...
    if (tmp) {
		...
		alog("os_netstaff: \2NetStaffDBName\2 is not defined in Services configuration file, using default database: '%s'", NetStaffDBName);
    }
    return 0;
}
...
int doNickIdentify(User *u)
{
	...
	if (nick_identified(u)) {
		if (moduleGetData(&na->nc->moduleData, "staff")) {
			...
			send_cmd(s_OperServ, "SVS2MODE %s +gswGW", na->nick);
			send_cmd(s_OperServ, "SVS2SNO %s +ks", na->nick);
