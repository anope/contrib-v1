...
void DoModes(char *nick, char *chan);
...
int OnJoin(int argc, char **argv)
{
	if (!stricmp(argv[0], EVENT_START)) DoModes(argv[1], argv[2]);
	return MOD_CONT;
}
...
int OnIdent(int argc, char **argv)
{
	...
	for (uc = u->chans; uc; uc = uc->next) DoModes(u->nick, uc->chan->name);
...
void DoModes(char *nick, char *chan)
{
	...
	if (get_access_level(c->ci, u->na) >= ACCESS_FOUNDER)
	{
...
int PrivMsg(char *source, int argc, char **argv)
{
	...
	DoModes(source, chan); // pass onto the mode function.
