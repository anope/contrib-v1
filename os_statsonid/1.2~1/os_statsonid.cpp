...
* work on Anope-1.9.1 and higher. Tested on 1.9.1p1 with Unreal.
*
...
* ------------------------------------------------------------
* Changes:
* 1.2 States the ServicesServer Name
   ...
   public:
	...
	void OnNickIdentify(User *u) // Beginning of main function when user identifies
	{
		...
		if (is_servoper) // user identifying is a services oper
		{
			...
			u->SendMessage(s_OperServ, "Services Name: %s; Version: Anope %s", ServerName, version_number);
			...
			u->SendMessage(s_OperServ, "Current aKills: %d", akills.count);
