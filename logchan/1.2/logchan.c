...
{  
	...
	if (!LogChanInit())		/* Turn on log channel */
	{
		alog("\002MODULE\002 [LogChan] Unable to initialize logchannel");
		return MOD_STOP;
	}

	if (!BindEvents())		/* Bind to our events */
	{
		alog("\002MODULE\002 [LogChan] Unable to initialize bindings");
		return MOD_STOP;
	}
...
int log_botUnassign(int argc, char **argv)
{
	...
	alog("\002%s\002: %s (%s@%s \"%s\") has been unassigned from %s",
	     s_BotServ, bi->nick, bi->user, bi->host, bi->real, ci->name);
	return MOD_CONT;
}

/* log_defcon, log change of defcon level */
int log_defcon(int argc, char **argv) {
	alog("\002DEFCON\002 Defcon level has been changed to %s",
	     argv[0]); 
	return MOD_CONT; 
}
...
int log_botAssign(int argc, char **argv)
{
	...
	alog("\002%s\002: %s (%s@%s \"%s\") has been assigned to %s",
	     s_BotServ, bi->nick, bi->user, bi->host, bi->real, ci->name);
	return MOD_CONT;
}
...
int log_botadd(int argc, char **argv)
{
	...
	alog("\002%s\002: %s!%s@%s (%s) created",
	     s_BotServ, bi->nick, bi->user, bi->host, bi->real); 
	return MOD_CONT; 
}
...
int log_botdel(int argc, char **argv)
{
	...
	alog("\002%s\002: %s!%s@%s (%s) deleted",
	     s_BotServ, bi->nick, bi->user, bi->host, bi->real); 
	return MOD_CONT;
}
...
int log_joins(int argc, char **argv)
{
	...
	if (!argv[1] || !argv[2] || !(u = finduser(argv[1])))
	{
		return MOD_CONT;
	}
	...
	{
		alog("\002JOIN\002 %s (%s@%s) [%s] joins %s",
		     argv[1], u->vident, u->chost, u->hostip, argv[2]);
	}
...
int log_parts(int argc, char **argv)
{
	...
	if (stricmp(argv[0], EVENT_STOP) == 0)
	{
		alog("\002PART\002 %s (%s@%s) [%s] parts %s (%s)",
		     argv[1], u->vident, u->chost, u->hostip, argv[2], (checkPart == 1 ? argv[3] : ""));
	}
...
int log_botBan(int argc, char **argv)
{
	...
	alog("\002%s\002: %s (%s@%s) was banned from %s",
	     bi->nick, u->nick, u->vident, u->chost, ci->name);
	return MOD_CONT;
}
...
int log_topic(int argc, char **argv)
{
	...
	alog("\002TOPIC\002 %s (%s@%s) changed the topic on %s",
	     u->nick, u->vident, u->chost, ci->name);
	return MOD_CONT;
}
...
void my_set_umode(User * user, int ac, char **av)
{
	...
	if (debug)
	{
		alog("debug: Changing mode for %s to %s",
		     user->nick, modes);
	}
	...
	while (*modes) 
	{
		switch (*modes++) 
		{
			...
			case 'B': /* BOT */ 
				if (myadd == 1)
				{ 
					alog("\002USERMODE\002 %s (%s@%s => %s@%s) is now a Bot (+B)",
					     user->nick, user->username, user->hostip, user->vident, user->vhost); 
				}
				else if (myadd == 0) 
				{
					alog("\002USERMODE\002 %s (%s@%s => %s@%s) is no longer a Bot (-B)",
					     user->nick, user->username, user->hostip, user->vident, user->vhost); 
				} 
			break; 
		}
...
int log_away(char *source, int argc, char **argv) 
{
	...
	if (argc > 0) {
		alog("\002AWAY\002 %s (%s@%s) [%s] is now marked as away (%s)",
		     u->nick, u->vident, u->chost, u->hostip, argv[0]);
	} else {
		alog("\002AWAY\002 %s (%s@%s) [%s] is no longer marked as away",
		     u->nick, u->vident, u->chost, u->hostip);
	}
	return MOD_CONT; 
}
...
int log_nick_change(char *source, int argc, char **argv) 
{
	...
	if (!strcmp(source, "") || !strcmp(argv[0], "") || !(u = finduser(source)))
	{
		return MOD_CONT; 
	}
	...
	if (argc <= 3) 
	{
		alog("\002NICK\002 %s (%s@%s => %s@%s) is now known as %s",
		     source, u->username, u->hostip, u->vident, u->chost, argv[0]);
	} 
	return MOD_CONT;
}
...
int log_newnick(int argc, char **argv) 
{
	...
	if (!strcmp(argv[0],"") || !(u = finduser(argv[0]))) 
	{
		return MOD_CONT; 
	}
...
int log_user_logoff(char *source, int argc, char **argv) 
{
	...
	alog("\002QUIT\002 %s (%s@%s, %s) [%s] left the network (%s) [%s]",
	     u->nick, u->username, u->host, u->realname, u->hostip, u->server->name, argv[0]); 
	return MOD_CONT; 
} 
...
int log_user_kill(char *source, int argc, char **argv) 
{
	...
	reason = myStrGetTokenRemainder(argv[1], ' ', 1);

	alog("\002KILL\002 %s (%s@%s) was killed by %s!%s@%s [%s]",
	     u2->nick, u2->username, u2->host, u->nick, u->username, u->host, reason);
...
int log_tkl(char *source, int argc, char **argv) {
	if (!strcmp(argv[0],"+")) {
		if (!strcmp(argv[1], "Z")) {
			alog("\002GZLINE\002 Added for %s@%s by %s [%s]",
			     argv[2], argv[3], argv[4], argv[7]); 
		} 
		if (!strcmp(argv[1], "G")) {
			alog("\002GLINE\002 Added for %s@%s by %s [%s]",
			     argv[2], argv[3], argv[4], argv[7]);
		} 
		if (!strcmp(argv[1], "z")) {
			alog("\002ZLINE\002 Added for %s@%s by %s [%s]",
			     argv[2], argv[3], argv[4], argv[7]);
		}
	}
	if (!strcmp(argv[0],"-")) {
		if (!strcmp(argv[1], "Z")) {
			alog("\002GZLINE\002 Removed by %s for %s@%s",
			     argv[4], argv[2], argv[3]); 
		}
		if (!strcmp(argv[1], "G")) {
			alog("\002GLINE\002 Removed by %s for %s@%s",
			     argv[4], argv[2], argv[3]);
		}
		if (!strcmp(argv[1], "z")) {
			alog("\002GLINE\002 Removed by %s for %s@%s",
...
int log_chan_mode(char *source, int argc, char **argv)
{
	...
	if (argc == 2) 
	{
		alog("\002MODE\002 %s %s by %s (%s@%s)",
		     argv[0], argv[1], u->nick, u->username, u->chost); 
	} 
	if (argc == 3)
	{
		alog("\002MODE\002 %s %s %s by %s (%s@%s)",
		     argv[0], argv[1], argv[2], u->nick, u->username, u->chost);
	}
	return MOD_CONT; 
} 
