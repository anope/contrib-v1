 ...
 * Date    : 05/08/2006  (Last update: 24/12/2011)
 * Version : 2.4
 * -----------------------------------------------------------------------------
 ...
 * -----------------------------------------------------------------------------
 * This module adds the NOTINCHANLIST command to operserv. It allows Services Admins
 ...
 *   - InspIRCd [1.1, 1.2 and 2.0]
 *   - Plexus 3.0 [or later]
 *   - PTLink
 *   - UltimateIRCd 2.8.2 [or later]
 ...
 * Changes:
 *
 *  2.4  Added timer functionality to run periodically obviating the need for external scripts.
 *       Fixed InspIRCd support for JOIN parameter.
 *       Added support for JOIN parameter on UltimateIRCd 3.0.
 *
...
#
# This determines how long users will be Akilled if the AKILL argument of the
# notinchanlist is specified. Please note this is subject to ExpireTimeout.
...
# OSNotInChanScheduler [OPTIONAL]
# Module: os_notinchanlist
#
# Specifies if and how often the NOTINCHANLIST command is run automatically.
# When set, NOTINCHANLIST will be triggered with OSNotInChanSchedAction and
# OSNotInChanSchedNotID settings at roughly the defined interval. 
#OSNotInChanScheduler 10m

# OSNotInChanSchedAction [OPTIONAL - Required by OSNotInChanScheduler]
# Module: os_notinchanlist
#
# Specifies what action the module must undertake when triggered automatically
# by the scheduled.
# Possible values are: NOTICE, KILL, AKILL, JOIN, SHUN, TSHUN and FAKILL.
# The description of these actions can be found in the modules help:
# /msg OperServ HELP NOTINCHANLIST
# Note that not all possible actions may be supported on your IRCd!
#OSNotInChanSchedAction "NOTICE"

# OSNotInChanSchedNotID [OPTIONAL - Only applicable to OSNotInChanScheduler]
# Module: os_notinchanlist
#
# When set, users that are not in a channel, but are IDENTIFIED with NickServ
# will be omitted from scheduled scans.
#OSNotInChanSchedNotID
...
/* variables */
int niclAkillTime, schedulerTime, schedulerAction, schedulerNotID;
...
int schedule_handler(int ac, char **av);
int do_scan(User *u, int action, int notid, int quiet);
...
int AnopeInit(int argc, char **argv) {
	...
	if (schedulerTime)
		moduleAddCallback("notinchan", time(NULL) + schedulerTime, schedule_handler, 0, NULL);
...
/**
 * Main function to interpret the command...
 **/
int do_find_nochan_users(User * u) {
	int notid = 0, ok = 1, action = 0, quiet = 0;
...
/**
 * Handle the callback from core when a new scan is scheduled to occur.
 **/
int schedule_handler(int ac, char **av) {
	int count;
	count = do_scan(NULL, schedulerAction, schedulerNotID, 1);
	alog("[os_notinchanlist] Scheduled scan complete. Found %d possible malicious clients.", count);

	/* Reschedule for next scan.. */
	moduleAddCallback("notinchan", time(NULL) + schedulerTime, schedule_handler, 0, NULL);
...
/**
 * Perform the a scan of all users.
 *
 * @param u User that triggered the command, if any. NULL if it's a scheduled job.
 * @param action Action to be performed on found clients.
 * @param notid Are identified users to be excluded.
 * @param quiet Report each hit back to the command trigger.
 **/
int do_scan(User *u, int action, int notid, int quiet) {
				...
				if (!is_oper(u2) && !u2->chans) {
					...
					if (u && action == 0) {
						...
						if (action == 1) {
							...
							add_akill(u, akillmask, u ? u->nick : s_OperServ, time(NULL) + niclAkillTime, niclAkillReason);

						} else if (action == 4) {
							if (!stricmp(IRCDModule,"inspircd11") || !stricmp(IRCDModule,"inspircd12") ||
									!stricmp(IRCDModule,"inspircd20") || !stricmp(IRCDModule,"plexus3") ||
									!stricmp(IRCDModule,"ptlink") || !stricmp(IRCDModule,"ultimate2") ||
									!stricmp(IRCDModule,"ultimate3") || !stricmp(IRCDModule,"unreal31") ||
									!stricmp(IRCDModule,"unreal32") || !stricmp(IRCDModule,"viagra")) {
...
void load_config(void) {
	int i;
	char *tkillreason = NULL, *takillreason = NULL, *idlechan = NULL, *tschedulerAction = NULL;
	...
	Directive confvalues[][1] = {
		...
		{{ "OSNotInChanScheduler", { { PARAM_TIME, PARAM_RELOAD, &schedulerTime } } }},
		{{ "OSNotInChanSchedAction", { { PARAM_STRING, PARAM_RELOAD, &tschedulerAction } } }},
		{{ "OSNotInChanSchedNotID", { { PARAM_SET, PARAM_RELOAD, &schedulerNotID } } }}
	};
	...
	if (!niclAkillTime)
		...
		niclAkillTime = 30 * 60;
		alog("[\002os_notinchanlist\002] Warning: OSNotInChanAkillTime needs to be at least 60. Using default values...");
	}
	...
	if (schedulerTime && !tschedulerAction) {
		alog("[\002os_notinchanlist\002] Error: Scheduler enabled but action not specified! Disabling scheduler..");
		schedulerTime = 0;
	}

	if (tschedulerAction) {
		schedulerAction = parse_action(tschedulerAction);
		if (schedulerAction == 4 && stricmp(IRCDModule,"inspircd11") && !stricmp(IRCDModule,"inspircd12") &&
				!stricmp(IRCDModule,"inspircd20") && !stricmp(IRCDModule,"plexus3") &&
				!stricmp(IRCDModule,"ptlink") && !stricmp(IRCDModule,"ultimate2") &&
				!stricmp(IRCDModule,"ultimate3") && !stricmp(IRCDModule,"unreal31") &&
				!stricmp(IRCDModule,"unreal32") && !stricmp(IRCDModule,"viagra")) {
			alog("[\002os_notinchanlist\002] Error: Scheduler action JOIN not supported by this IRCd! Disabling scheduler..");
			schedulerTime = 0;
			schedulerAction = 0;
		}
	}
	...
	if (debug)
		alog("debug: [os_notinchanlist] Config vars: AkillTime=%d KillReason='%s' AkillReason='%s' IdleChan='%s'", 
				niclAkillTime, niclKillReason, niclAkillReason, niclIdleChan);
	if (schedulerTime)
		alog("[os_notinchanlist] Scheduler enabled: Interval=%ds; Action=%s [%d]; Identified users are%s exempted.", 
				schedulerTime, tschedulerAction, schedulerAction, schedulerNotID ? "" : " NOT");
	else if (debug)
		alog("debug: [os_notinchanlist] Scheduler disabled.");

	/* free everything we used... */
	free(tkillreason);
	free(takillreason);
	free(idlechan);
	if (tschedulerAction) free(tschedulerAction);
}
...
int reload_config(int argc, char **argv) {
	if (argc >= 1) {
		/* Clear callbacks before reloading.. */
		moduleDelCallback("notinchan");
