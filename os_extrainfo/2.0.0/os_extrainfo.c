...
#define VERSION "$Id: os_extrainfo.c v2.0.0 20-08-2007 n00bie $"
...
** The info command if given with a 'nickserv' param will display total registered, 
** forbidden, noexpired, private, suspended and unconfirmed nicknames. Likely, if 
** given with a 'chanserv' param will display total registered, forbidden, noexpired, 
** private and suspended channels.
** 
** Providing command:
** /msg OperServ INFO {nickserv|chanserv}
...
** v2.0.0 - Changed the 'nickname' and 'channel' param to 'nickserv' and 'chanserv'
**          It causes some confusion before :S
**          Fixed a segfault (many thanks to Viper)
*****************************************************************************************
...
int myOperServInfoOperHelp(User *u)
{
	notice(s_OperServ, u->nick, "Syntax: \2INFO {nickserv|chanserv}\2");
	notice(s_OperServ, u->nick, " ");
	notice(s_OperServ, u->nick, "The info command if given with a \2nickserv\2 param will");
	...
	notice(s_OperServ, u->nick, "a \2chanserv\2 param will display total registered, forbidden,");
...
int do_extrainfo(User *u)
{
	...
	if (stricmp(option, "nickserv") == 0) {
		...
		notice(s_OperServ, u->nick, "-------------------------");
	} else if (stricmp(option, "chanserv") == 0) {
		for (i = 0; i < 256; i++) {
