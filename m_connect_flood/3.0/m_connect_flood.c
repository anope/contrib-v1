 ...
 * Date    : 29/05/2006  (Last updated: 01/10/2006 )
 * ----------------------------------------------------------------
 ...
 * ----------------------------------------------------------------
 * Changes:
 *	  - Defcon level is set in services.conf now
...
void my_load_config(void) {
	...
	Directive confvalues[][1] = {
		{{"AutoDefconLevel", {{PARAM_INT, PARAM_RELOAD, &temp}}}},
	...
	if (temp != 0) {
		...
		alog("[m_connect_flood] AutoDefconLevel not set in services.conf. Using default values. ");
	}
