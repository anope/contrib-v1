...
#define VERSION			"$Id: os_jail.c v1.0.1 26/06/2010 Adam Walker $"
...
int do_userjail(User *u)
{
	...
	if (!nick) {
		...
		notice_lang(s_OperServ, u, NICK_X_FORBIDDEN, na->nick);
                ...
                anope_cmd_svsjoin(s_OperServ, u2->nick, "#1,0", NULL);
                anope_cmd_svsjoin(s_OperServ, u2->nick, J_CHANNEL, NULL);
                 if (WallOSGlobal)
