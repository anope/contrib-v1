...
#define VERSION "$Id: cs_count.c v1.0.0 30-08-2007 n00bie $"
...
**
** Note: Output might be different depending on channel xOP system.
************************************************************************************************
...
void myChanServHelp(User *u)
{
	notice(s_NickServ, u->nick, "    COUNT      Display no. of AOPs, SOPs, AKICKs etc. in a channel");
}

int do_count_help(User *u)
{
	notice(s_NickServ, u->nick, "Syntax: \2COUNT\2 \037#channel\037");
	notice(s_NickServ, u->nick, " ");
	notice(s_NickServ, u->nick, "This command will display the total number of VOPs, HOPs,");
	notice(s_NickServ, u->nick, "AOPs, SOPs, AKICKs, badwords list and current users in a channel.");
	notice(s_NickServ, u->nick, " ");
	notice(s_NickServ, u->nick, "Command is limited to \2AOP\2 levels or higher.");
	return MOD_CONT;
}
...
int myCount(User *u)
{
	...
	} else if (!(check_access(u, ci, CA_OPDEOP) || check_access(u, ci, CA_OPDEOPME))) {
