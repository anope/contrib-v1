...
class CommandBSRequest : public Command
{
	...
	bool IsValidBot(CommandSource &source, const std::vector<Anope::string> &params)
	{
		...
		{
			source.Reply(_("Bot Idents may only contain %d characters."), networkinfo->Get<unsigned>("userlen"));
			return false;
		}

		if (host.length() > networkinfo->Get<unsigned>("hostlen"))
		{
			source.Reply(_("Bot Hosts may only contain %d characters."), networkinfo->Get<unsigned>("hostlen"));
			return false;
		}
	...
	void Execute(CommandSource &source, const std::vector<Anope::string> &params)
	{
		...
		if (req)
		{
		    ...
		    if (BSRequestMemoUser && MemoServService)
		      MemoServService->Send(BotServ->nick, na->nick, _("[auto memo] Your requested bot has been approved."), true);
	...
	void Execute(CommandSource &source, const std::vector<Anope::string> &params)
	{
		...
		if (req)
		{
			...
			if (BSRequestMemoUser && MemoServService)
			{
				...
				MemoServService->Send(BotServ->nick, nick, message, true);
			}
...
class BSRequest : public Module
{
 ...
 public:
	...
	void OnReload()
	{
		// TODO: Move this out of OnReload()
		BSRequestMemoUser = Config->GetModule(this)->Get<bool>("memouser");
		BSRequestMemoOper = Config->GetModule(this)->Get<bool>("memooper");
...
void req_send_memos(CommandSource &source, const Anope::string &botfullhost, const Anope::string &botreal)
{
  if (BSRequestMemoOper == 1 && MemoServService)
	  for (unsigned i = 0; i < Config->Opers.size(); ++i)
	  {
		  ...
		  MemoServService->Send(BotServ->nick, na->nick, message, true);
	  }
}
