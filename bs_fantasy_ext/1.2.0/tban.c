...
int do_tban(User * u, Channel *c, char *target, int time) {
	 ...
	 **/
		...
		noticeLang(ci->bi->nick, u, LANG_TBAN_RESPONSE, mask, c->name, t);
		free(t);
	} else {
		...
		noticeLang(ci->bi->nick, u, LANG_TBAN_RESPONSE, target, c->name, t);
		free(t);
	}
				...
				while (cu) {
					if (match_usermask(target, cu->user)) {
						...
						if ((ci->flags & CI_PEACE) && (get_access(cu->user, ci) >= get_access(u, ci)))
							noticeLang(ci->bi->nick, u, LANG_KB_KICK_DENIED, cu->user->nick);
						else
#endif
