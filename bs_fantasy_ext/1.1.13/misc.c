...
void check_core_modules(void) {
	...
	if (OverrideCoreCmds) {
		...
		if (!findModule("bs_fantasy")) {
			if (loadModule(createModule("bs_fantasy"),NULL) == MOD_ERR_OK)
		...
		if (!findModule("bs_fantasy_kickban")) {
			if (loadModule(createModule("bs_fantasy_kickban"),NULL) == MOD_ERR_OK)
...
#ifdef SUPPORTED
#ifndef _WIN32
	if (unloadModule(findModule("os_raw"),NULL) == MOD_ERR_OK) {
		alog("[bs_fantasy_ext] os_raw has been unloaded...");
	}
#endif
...
static void show_modinfo(User *u, ChannelInfo *ci) {
	char tmp[BUFSIZE];
	snprintf(tmp, BUFSIZE, "%s%s%d", version_flags, (supported) ? "S" : "U", CPU_USAGE_REDUCTION);

	notice(ci->bi->nick, u->nick, "Fantasy commands provided by \002bs_fantasy_ext\002. [Author: \002%s\002] [Version: \002%s\002 (%s)] [Anope rev. %s]",
			AUTHOR, VERSION, tmp, version_build);
}
