 ...
 * German         : Han` <Han@mefalcon.org>                 [ Translation incomplete!]
 * Spanish        : DoctorYerva <admin@cannabisdreams.es>   [ Translation outdated!]
 *
...
int mod_lang_init() {
	...
	mod_load_lang(LANG_FR, "fr");
	mod_load_lang(LANG_DE, "de");
	mod_load_lang(LANG_ES, "es");
...
void noticeLang(char *source, User * dest, int message, ...) {
	...
	fmt = getLangString(dest->na, message);
...
char *getLangString(NickAlias *na, int index) {
	int i;
	char *s = NULL;

	if (na && na->nc && !(na->status & NS_VERBOTEN)) {
		if (debug)
			alog("[bs_fantasy_ext] Looking for language string %d in language %d.",
					index, na->nc->language);

		/* Check to make sure the users language is supported */
		for (i = 0; i < MOD_NUM_LANGS; i++) {
			if (modlanglist[i] == na->nc->language) {
				if (modlangtexts[modlanglist[i]] != NULL)
