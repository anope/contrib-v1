...
int do_cmd_list(User *u, ChannelInfo *ci) {
	...
	*end = 0;
	...
	end += snprintf(end, sizeof(buf) - (end - buf), "%s%srappend | %strapp", need_comma ? commastr : "", BSFantasyCharacter, BSFantasyCharacter);
	need_comma = 1;
#endif

#ifdef ENABLE_PREPENDTOPIC
	end += snprintf(end, sizeof(buf) - (end - buf), "%s%sprependtopic | %stprepend", need_comma ? commastr : "", BSFantasyCharacter, BSFantasyCharacter);
	end += snprintf(end, sizeof(buf) - (end - buf), "%s%srprepend | %strprep", need_comma ? commastr : "", BSFantasyCharacter, BSFantasyCharacter);
	need_comma = 1;
#endif
	...
	if (OverrideCoreCmds) {
		end += snprintf(end, sizeof(buf) - (end - buf), "%s%skick", need_comma ? commastr : "", BSFantasyCharacter);
	...
	*end = 0;
	...
	/* Commands provided by other modules.. */
	if (en_f_dt || en_f_karma || en_f_vhost || en_why) {
		end = buf;
		*end = 0;
		need_comma = 0;

		if (en_f_dt) {
			end += snprintf(end, sizeof(buf) - (end - buf), "%s%sdt", need_comma ? commastr : "", BSFantasyCharacter);
			end += snprintf(end, sizeof(buf) - (end - buf), "%s%sgdt", commastr, BSFantasyCharacter);
			need_comma = 1;
		}

		if (en_f_karma) {
			end += snprintf(end, sizeof(buf) - (end - buf), "%s%skarma", need_comma ? commastr : "", BSFantasyCharacter);
			need_comma = 1;
		}

		if (en_f_vhost) {
			end += snprintf(end, sizeof(buf) - (end - buf), "%s%svhost", need_comma ? commastr : "", BSFantasyCharacter);
			end += snprintf(end, sizeof(buf) - (end - buf), "%s%sgroupvhost", need_comma ? commastr : "", BSFantasyCharacter);
			need_comma = 1;
		}

		if (en_why) {
			end += snprintf(end, sizeof(buf) - (end - buf), "%s%swhy", need_comma ? commastr : "", BSFantasyCharacter);
			need_comma = 1;
		}

		noticeLang(ci->bi->nick, u, LANG_CMD_LIST_EXTMODS, buf);
	}
...
int do_help(User *u, ChannelInfo *ci, char *cmd, char *param, char *opt) {
	...
	if (cmd == NULL) {
		...
		/* If the fantasy command is provided by another module which is loaded,
		 * do not respond to !help queries for that command, let that module
		 * handle it.. */
		if ((en_f_dt && (!stricmp(cmd, "dt") || !stricmp(cmd, "gdt")))
				|| (en_f_karma && !stricmp(cmd, "karma"))
				|| (en_f_vhost && (!stricmp(cmd, "vhost") || !stricmp(cmd, "groupvhost")))
				|| (en_why && !stricmp(cmd, "why")))
			return MOD_CONT;

		/* Help on op/deop/etc. commands normally implemented by the core
		 * first check if the command is a valid match.. */
		do {
			if (stricmp(cmd, util->bsname) == 0)
				isCoreCmd = 1;
		} while (!isCoreCmd && ((++util)->name != NULL));
		if (isCoreCmd) {
			if (!ircd->halfop && (!stricmp(cmd, "halfop") || !stricmp(cmd, "dehalfop")))
				noticeLang(ci->bi->nick, u, LANG_HOPS_UNSUPPORTED);
			else
				noticeLang(ci->bi->nick, u, LANG_STATUS_MODE_SYNTAX_EXT, util->bsname);

		/* list commands */
		} else if (!stricmp(cmd, "general")) {
		...
		/* help on commands... */
			...
			noticeLang(ci->bi->nick, u, LANG_CMD_DISABLED);
#endif
		else if (!stricmp(cmd, "ctopic"))
#ifdef ENABLE_TOPIC
			noticeLang(ci->bi->nick, u, LANG_CTOPIC_SYNTAX_EXT);
#else
			noticeLang(ci->bi->nick, u, LANG_CMD_DISABLED);
#endif
			...
			noticeLang(ci->bi->nick, u, LANG_CMD_DISABLED);
#endif
		else if ((!stricmp(cmd, "replaceappendtopic")) || (!stricmp(cmd, "rappend"))
				|| (!stricmp(cmd, "trappend")) || (!stricmp(cmd, "rappendt")) || (!stricmp(cmd, "trapp")))
#ifdef ENABLE_APPENDTOPIC
			noticeLang(ci->bi->nick, u, LANG_RAPPEND_SYNTAX_EXT);
#else
			noticeLang(ci->bi->nick, u, LANG_CMD_DISABLED);
#endif

		else if ((!stricmp(cmd, "prependtopic")) || (!stricmp(cmd, "tprepend")))
#ifdef ENABLE_PREPENDTOPIC
			noticeLang(ci->bi->nick, u, LANG_PREPENDT_SYNTAX_EXT);
#else
			noticeLang(ci->bi->nick, u, LANG_CMD_DISABLED);
#endif
		else if ((!stricmp(cmd, "replaceprependtopic")) || (!stricmp(cmd, "rprepend"))
				|| (!stricmp(cmd, "trprepend")) || (!stricmp(cmd, "rprependt")) || (!stricmp(cmd, "trprep")))
#ifdef ENABLE_PREPENDTOPIC
			noticeLang(ci->bi->nick, u, LANG_RPREPEND_SYNTAX_EXT);
#else
			noticeLang(ci->bi->nick, u, LANG_CMD_DISABLED);
#endif
...
#ifdef ENABLE_UNBAN
			if (OverrideCoreCmds)
				noticeLang(ci->bi->nick, u, LANG_UNBAN_SYNTAX_EXT);
			else
				noticeLang(ci->bi->nick, u, LANG_CMD_DISABLED);
#else
			noticeLang(ci->bi->nick, u, LANG_CMD_DISABLED);
			...
			noticeLang(ci->bi->nick, u, LANG_CMD_DISABLED);
#endif
		} else if ((!stricmp(cmd, "k")) || (!stricmp(cmd, "kick"))) {
#ifdef ENABLE_KICK
			if (OverrideCoreCmds)
				noticeLang(ci->bi->nick, u, LANG_KICK_SYNTAX_EXT);
			else
				noticeLang(ci->bi->nick, u, LANG_CMD_DISABLED);
#else
			noticeLang(ci->bi->nick, u, LANG_CMD_DISABLED);
			...
			if (!param) {
				...
				else if (!en_f_karma || stricmp(param, "karma")) {
					noticeLang(ci->bi->nick, u, LANG_SET_UNKNOWN_OPTION, param);
				}
			}
#else
			noticeLang(ci->bi->nick, u, LANG_CMD_DISABLED);
