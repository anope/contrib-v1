...
int AnopeInit(void) {
	...
	if (!moduleMinVersion(1,8,8,3112)) {
	...
	hook = createEventHook(EVENT_MODLOAD, event_modload);
	if (moduleAddEventHook(hook) != MOD_ERR_OK) {
		alog("[\002bs_fantasy_ext\002] Can't hook to EVENT_MODLOAD event.");
		return MOD_STOP;
	}

	hook = createEventHook(EVENT_MODUNLOAD, event_modunload);
	if (moduleAddEventHook(hook) != MOD_ERR_OK) {
		alog("[\002bs_fantasy_ext\002] Can't hook to EVENT_MODUNLOAD event.");
		return MOD_STOP;
	}

	hook = createEventHook(EVENT_ADDCOMMAND, event_check_cmd);
	if (moduleAddEventHook(hook) != MOD_ERR_OK) {
		alog("[\002bs_fantasy_ext\002] Can't hook to EVENT_ADDCOMMAND event.");
		return MOD_STOP;
	}

	load_config();
	update_version();
	if (mod_lang_init() == MOD_STOP) {
		alog("[\002bs_fantasy_ext\002] Error loading lang files... Aborting! ");
		return MOD_STOP;
	}
	create_dummy();
	check_core_modules();
	...
	if (debug) {
		...
		alog("debug: [bs_fantasy_ext] Fantasy command \"prependtopic\" has been enabled.");
#endif
...
#ifdef ENABLE_KICK
		alog("debug: [bs_fantasy_ext] Fantasy command \"kick\"has been enabled.");
#endif
		if (OverrideCoreCmds)
...
int do_fantasy(int ac, char **av) {
	...
	/* Check whether we parse fantasy commands from bots. */
	if (IgnoreBots && has_umode_B(u))
		return MOD_CONT;
	...
	if (stricmp(av[0], "sop") == 0) {
		if (ac == 3)
			noticeLang(ci->bi->nick, u, LANG_SOP_SYNTAX);
		else {
			char *arg1 = myStrGetToken(av[3],' ',0);
			char *arg2 = myStrGetToken(av[3],' ',1);

			if (readonly)
				noticeLang(ci->bi->nick, u, LANG_CMD_NOT_AV);
		...
		if (ac == 3)
			noticeLang(ci->bi->nick, u, LANG_AOP_SYNTAX);
		else {
			char *arg1 = myStrGetToken(av[3],' ',0);
			char *arg2 = myStrGetToken(av[3],' ',1);
		...
		if (ac == 3)
			noticeLang(ci->bi->nick, u, LANG_HOP_SYNTAX);
		else {
			char *arg1 = myStrGetToken(av[3],' ',0);
			char *arg2 = myStrGetToken(av[3],' ',1);
		...
		if (ac == 3)
			noticeLang(ci->bi->nick, u, LANG_VOP_SYNTAX);
		else {
			char *arg1 = myStrGetToken(av[3],' ',0);
			char *arg2 = myStrGetToken(av[3],' ',1);
	...
	if ((stricmp(av[0], "access") == 0) || (stricmp(av[0], "acc") == 0) || (stricmp(av[0], "axx") == 0)) {
		if (ac == 3)
			noticeLang(ci->bi->nick, u, LANG_ACCESS_SYNTAX);
		else {
			char *arg1 = myStrGetToken(av[3],' ',0);
			char *arg2 = myStrGetToken(av[3],' ',1);
		...
		if (ac == 3)
			noticeLang(ci->bi->nick, u, LANG_LEVELS_SYNTAX);
		else {
			char *arg1 = myStrGetToken(av[3],' ',0);
			char *arg2 = myStrGetToken(av[3],' ',1);
			char *arg3 = myStrGetToken(av[3],' ',2);
				...
				if (stricmp("set",arg1) == 0) {
					if (arg2) {
						if (arg3) {
							...
							if (*error != '\0')
								noticeLang(ci->bi->nick, u, LANG_LEVELS_ERR_VAL);
							else {
								if (is_founder(u, ci) || (is_services_admin(u) && SAdminOverride))
									do_levels(u, c, arg1, arg2, level);
								else
									noticeLang(ci->bi->nick, u, LANG_ONLY_FOUNDERS);
							}
						} else
							noticeLang(ci->bi->nick, u, LANG_LEVELS_ERR_NO_VAL);
					} else
						noticeLang(ci->bi->nick, u, LANG_LEVELS_SYNTAX);
					...
					if (my_check_access(u, ci, CA_ACCESS_CHANGE) || (is_services_admin(u) && SAdminOverride))
						do_levels(u, c, arg1, arg2, 0);
					else
						noticeLang(ci->bi->nick, u, LANG_ONLY_FOUNDERS);
			...
			if (target) {
				if (!is_founder(u, ci) && !is_services_root(u))
					...
					if (!arg2)
						...
						noticeLang(ci->bi->nick, u, LANG_UPDOWN_SYNTAX);
				}
	...
	if (stricmp(av[0], "badwords") == 0) {
		if (ac == 3)
			noticeLang(ci->bi->nick, u, LANG_BADWORDS_SYNTAX);
		else {
			char *arg1 = myStrGetToken(av[3],' ',0);
			char *arg2 = myStrGetToken(av[3],' ',1);
			char *arg3 = myStrGetToken(av[3],' ',2);
				...
				if (stricmp("add",arg1) == 0) {
					if (arg2 && arg3) {
						if ((!stricmp(arg3,"start")) || (!stricmp(arg3,"end")) || (!stricmp(arg3,"single"))
							...
							noticeLang(ci->bi->nick, u, LANG_BADWORDS_SYNTAX);
							noticeLang(ci->bi->nick, u, LANG_BADWORDS_ERR_STYLE);
						}
					} else
						noticeLang(ci->bi->nick, u, LANG_BADWORDS_SYNTAX);
	...
	if (stricmp(av[0], "set") == 0) {
		if (ac == 3)
			noticeLang(ci->bi->nick, u, LANG_SET_SYNTAX);
			...
			/* chanserv set commands */
				...
				if (arg2) {
					...
					noticeLang(ci->bi->nick, u, LANG_SET_FOUNDER_SYNTAX);
				...
				if (arg2) {
					...
					noticeLang(ci->bi->nick, u, LANG_SET_SUCCESSOR_SYNTAX);

			} else if (stricmp(arg1, "DESC") == 0) {
				if (arg2) {
					do_set_desc(u, ci, arg2);
				} else
					noticeLang(ci->bi->nick, u, LANG_SET_DESC_SYNTAX);
				...
				if (arg2) {
					...
					noticeLang(ci->bi->nick, u, LANG_SET_BANTYPE_SYNTAX);
				...
				if (arg2 && (!stricmp(arg2, "ON") || !stricmp(arg2, "OFF") || !stricmp(arg2, "LEVEL"))) {
			...
			/* botserv set commands */
				...
				if (arg2 && ((stricmp(arg2, "ON") == 0) || (stricmp(arg2, "OFF") == 0))) {
					...
					noticeLang(ci->bi->nick, u, LANG_SET_DONTKICKOPS_SYNTAX);
			} else if (stricmp(arg1, "DONTKICKVOICES") == 0) {
				if (arg2 && ((stricmp(arg2, "ON") == 0) || (stricmp(arg2, "OFF") == 0))) {
					do_set(u, ci, arg1, arg2);
				} else
					noticeLang(ci->bi->nick, u, LANG_SET_DONTKICKVOICES_SYNTAX);
			} else if (stricmp(arg1, "GREET") == 0) {
				if (arg2 && ((stricmp(arg2, "ON") == 0) || (stricmp(arg2, "OFF") == 0))) {
					do_set(u, ci, arg1, arg2);
				} else
					noticeLang(ci->bi->nick, u, LANG_SET_GREET_SYNTAX);
			} else if (stricmp(arg1, "FANTASY") == 0) {
				if (arg2 && ((stricmp(arg2, "ON") == 0) || (stricmp(arg2, "OFF") == 0))) {
					do_set(u, ci, arg1, arg2);
				} else
					noticeLang(ci->bi->nick, u, LANG_SET_FANTASY_SYNTAX);
			} else if (stricmp(arg1, "SYMBIOSIS") == 0) {
				if (arg2 && ((stricmp(arg2, "ON") == 0) || (stricmp(arg2, "OFF") == 0))) {
					do_set(u, ci, arg1, arg2);
				} else
					noticeLang(ci->bi->nick, u, LANG_SET_SYMBIOSIS_SYNTAX);
				...
				if (arg2 && ((stricmp(arg2, "ON") == 0) || (stricmp(arg2, "OFF") == 0))) {
					...
					noticeLang(ci->bi->nick, u, LANG_SET_NOBOT_SYNTAX);

			/* Only send the error if it isn't handled by another module.. */
			} else if (!en_f_karma || stricmp(arg1, "karma")){
	...
	if ((stricmp(av[0], "replaceappendtopic") == 0) || (stricmp(av[0], "rappend") == 0)
			|| (stricmp(av[0], "trappend") == 0) || (stricmp(av[0], "rappendt") == 0) || (stricmp(av[0], "trapp") == 0)){
		if (ac == 3)
			topic_replace_append(u, c, NULL);
...
#ifdef ENABLE_PREPENDTOPIC
	if ((stricmp(av[0], "prependtopic") == 0) ||(stricmp(av[0], "tprepend") == 0)){
		if (ac == 3)
	...
	if ((stricmp(av[0], "replaceprependtopic") == 0) || (stricmp(av[0], "rprepend") == 0)
			|| (stricmp(av[0], "trprepend") == 0) || (stricmp(av[0], "rprependt") == 0) || (stricmp(av[0], "trprep") == 0)){
		if (ac == 3)
			topic_replace_prepend(u, c, NULL);
			...
			if (readonly)
				...
				char *arg1 = myStrGetTokenRemainder(av[3],' ',0);
				topic_replace_prepend(u, c, arg1);
				if (arg1) free(arg1);
			}
		}
	} else
	...
	if ((stricmp(av[0], "kb") == 0) || (stricmp(av[0], "kickban") == 0)) {
		if (OverrideCoreCmds) {
			if (ac == 3) {
				if (my_check_access(u, ci, CA_BANME)) {
					noticeLang(ci->bi->nick, u, LANG_KICKBAN_SYNTAX);
...
#ifdef ENABLE_KICK
	if ((stricmp(av[0], "k") == 0) || (stricmp(av[0], "kick") == 0)) {
		if (OverrideCoreCmds) {
			if (ac == 3) {
				if (my_check_access(u, ci, CA_KICKME)) {
					noticeLang(ci->bi->nick, u, LANG_KICK_SYNTAX);
					do_core_kick(u, c, NULL, "Look I can fly... !");
				} else
					notice_lang(ci->bi->nick, u, PERMISSION_DENIED);
				...
				if (my_check_access(u, ci, CA_KICK)) {
					do_core_kick(u, c, arg1, arg2);
				} else
					notice_lang(ci->bi->nick, u, PERMISSION_DENIED);
	...
	if (stricmp(av[0], "mode") == 0) {
		if (!check_opercmds(u))
			...
			if (ac == 3)
				noticeLang(ci->bi->nick, u, LANG_MODE_SYNTAX);
	...
	if (stricmp(av[0], "akill") == 0) {
		if (!check_opercmds(u))
			...
			if (readonly)
				...
				if (ac == 3)
					noticeLang(ci->bi->nick, u, LANG_AKILL_SYNTAX);
	...
	if (stricmp(av[0], "ignore") == 0) {
		if (!check_opercmds(u))
			...
			if (ac == 3)
				noticeLang(ci->bi->nick, u, LANG_IGNORE_SYNTAX);
	...
	if (stricmp(av[0], "info") == 0) {
		if (ac == 3)
			...
			if (!arg1)
				...
				noticeLang(ci->bi->nick, u, LANG_INFO_SYNTAX);
	...
	if (stricmp(av[0], "bkick") == 0) {
		if (ac == 3)
			noticeLang(ci->bi->nick, u, LANG_BKICK_SYNTAX);
			...
			if (readonly)
				...
				if (arg2 && ((stricmp(arg2, "ON") == 0) || (stricmp(arg2, "OFF") == 0)))
					...
					noticeLang(ci->bi->nick, u, LANG_BKICK_BOLDS_SYNTAX);

			} else if (stricmp(arg1, "BADWORDS") == 0) {
				if (arg2 && ((stricmp(arg2, "ON") == 0) || (stricmp(arg2, "OFF") == 0)))
					do_set_kick(u, ci, arg1, arg2, arg3, NULL, NULL);
				else
					noticeLang(ci->bi->nick, u, LANG_BKICK_BADWORDS_SYNTAX);

			} else if (stricmp(arg1, "COLORS") == 0) {
				if (arg2 && ((stricmp(arg2, "ON") == 0) || (stricmp(arg2, "OFF") == 0)))
					do_set_kick(u, ci, arg1, arg2, arg3, NULL, NULL);
				else
					noticeLang(ci->bi->nick, u, LANG_BKICK_COLORS_SYNTAX);

			} else if (stricmp(arg1, "REVERSES") == 0) {
				if (arg2 && ((stricmp(arg2, "ON") == 0) || (stricmp(arg2, "OFF") == 0)))
					do_set_kick(u, ci, arg1, arg2, arg3, NULL, NULL);
				else
					noticeLang(ci->bi->nick, u, LANG_BKICK_REVERSES_SYNTAX);
				...
				if (arg2 && ((stricmp(arg2, "ON") == 0) || (stricmp(arg2, "OFF") == 0)))
					...
					noticeLang(ci->bi->nick, u, LANG_BKICK_UNDERLINES_SYNTAX);

			} else if (stricmp(arg1, "CAPS") == 0) {
				if (arg2 && ((stricmp(arg2, "ON") == 0) || (stricmp(arg2, "OFF") == 0)))
					do_set_kick(u, ci, arg1, arg2, arg3, arg4, arg5);
				else
					noticeLang(ci->bi->nick, u, LANG_BKICK_CAPS_SYNTAX);

			} else if (stricmp(arg1, "FLOOD") == 0) {
				if (arg2 && ((stricmp(arg2, "ON") == 0) || (stricmp(arg2, "OFF") == 0)))
					do_set_kick(u, ci, arg1, arg2, arg3, arg4, arg5);
				else
					noticeLang(ci->bi->nick, u, LANG_BKICK_FLOOD_SYNTAX);

			} else if (stricmp(arg1, "REPEAT") == 0) {
				if (arg2 && ((stricmp(arg2, "ON") == 0) || (stricmp(arg2, "OFF") == 0)))
					do_set_kick(u, ci, arg1, arg2, arg3, arg4, NULL);
				else
					noticeLang(ci->bi->nick, u, LANG_BKICK_REPEAT_SYNTAX);
		...
		if (ac == 3) {
			...
			if (my_check_access(u, ci, CA_BAN)) {
				if (!arg2)
					...
					noticeLang(ci->bi->nick, u, LANG_INVALID_TIME, arg2);
				else
	...
	if (stricmp(av[0], "tkb") == 0 || stricmp(av[0], "tkban") == 0) {
		if (ac == 3) {
			if (my_check_access(u, ci, CA_BANME)) {
				noticeLang(ci->bi->nick, u, LANG_TKBAN_SYNTAX);
	...
	if (stricmp(av[0], "suspend") == 0) {
		if (!check_opercmds(u))
			...
			if (ac == 3)
				noticeLang(ci->bi->nick, u, LANG_SUSPEND_SYNTAX);
	...
	if (stricmp(av[0], "sync") == 0) {
		if (en_sync) {
			if (ac == 3) {
				...
				noticeLang(ci->bi->nick, u, LANG_SYNC_SYNTAX);
		} else
	...
	if (stricmp(av[0], "shun") == 0) {
		if (!check_opercmds(u))
			...
			if (en_shun) {
				if (ac == 3)
					noticeLang(ci->bi->nick, u, LANG_SHUN_SYNTAX);
	...
	if (stricmp(av[0], "tshun") == 0) {
		if (!check_opercmds(u))
			...
			if (en_shun) {
				if (ac == 3)
					noticeLang(ci->bi->nick, u, LANG_TSHUN_SYNTAX);
