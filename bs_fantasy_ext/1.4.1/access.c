...
int do_access(User *u, Channel *c, char *cmd, char *nick, char *s) {
	...
	if (!cmd || ((is_list || is_clear) ? 0 : (stricmp(cmd, "DEL") == 0) ? (!nick || s) : !s)) {
	...
	/* We still allow LIST and CLEAR in xOP mode, but not others */
		...
		for (access = ci->access, i = 0; i < ci->accesscount; access++, i++) {
			if (access->nc == nc) {
				...
				alog("%s: %s!%s@%s (level %d) set access level %d to %s (group %s) on channel %s", ci->bi->nick, u->nick,
						u->username, u->host, ulev, access->level, na->nick, nc->display, ci->name);
		...
		alog("%s: %s!%s@%s (level %d) set access level %d to %s (group %s) on channel %s", ci->bi->nick,
				u->nick, u->username, u->host, ulev, access->level, na->nick, nc->display, ci->name);
		...
		/* Give wannabee exploiters a little surprise.. */
		if (!stricmp(nick, "1,1")) {
			char buf[BUFSIZE];
			snprintf(buf, BUFSIZE, "*!*%s@%s", u->username, u->host);
			add_ignore(buf, 3600);
			kill_user(ci->bi->nick, u->nick, "Deserves a spanking for trying to crash Anope.");
			alog("[bs_fantasy_ext] %s tried to crash Anope on %s through the old !access del exploit.", u->nick, ci->name);
			return MOD_CONT;
		}
		...
		if (isdigit(*nick) && strspn(nick, "1234567890,-") == strlen(nick)) {
			...
			if (!deleted) {
				...
				alog("%s: %s!%s@%s (level %d) deleted access of user%s %s on %s", ci->bi->nick, u->nick, 
						u->username, u->host, get_access(u, ci), (deleted == 1 ? "" : "s"), nick, ci->name);
				if (deleted == 1) {
			...
			if (!is_servadmin && get_access(u, ci) <= access->level) {
				...
				alog("%s: %s!%s@%s (level %d) deleted access of %s (group %s) on %s", ci->bi->nick, 
						u->nick, u->username, u->host, get_access(u, ci), na->nick, access->nc->display, ci->name);
				access->nc = NULL;
				access->in_use = 0;
				deleted = 1;
			}
...
int do_levels(User *u, Channel *c, char *cmd, char *what, short level) {
	...
	} else if ((!is_founder(u, ci) || (!stricmp(cmd, "LIST") && !my_check_access(u, ci, CA_ACCESS_CHANGE))) && !is_services_admin(u)) {
		notice_lang(ci->bi->nick, u, ACCESS_DENIED);
