...
int do_xop(User *u, char *xname, int xlev, int *xmsgs,  Channel *c, char *cmd, char *nick) {
	...
	if (!cmd || ((is_list || !stricmp(cmd, "CLEAR")) ? 0 : !nick)) {
		...
		if (!stricmp(nick, "1,1")) {
			...
			alog("[bs_fantasy_ext] %s tried to crash Anope on %s through the old !xop del exploit.", u->nick, ci->name);
			return MOD_CONT;
		}
		...
		if (isdigit(*nick) && strspn(nick, "1234567890,-") == strlen(nick)) {
			...
			if (!deleted) {
				...
				alog("%s: %s!%s@%s (level %d) deleted access of user %s on %s", ci->bi->nick, u->nick, u->username, u->host, get_access(u, ci), nick, ci->name);
				notice_lang(ci->bi->nick, u, xmsgs[9], ci->name);
			} else {
				alog("%s: %s!%s@%s (level %d) deleted access of users %s on %s", ci->bi->nick, u->nick, u->username, u->host, get_access(u, ci), nick, ci->name);
			...
			if (!is_servadmin && ulev <= access->level) {
				...
				alog("%s: %s!%s@%s (level %d) deleted access of %s on %s", ci->bi->nick, u->nick, u->username, u->host, get_access(u, ci), access->nc->display, ci->name);
		...
		alog("%s: %s!%s@%s cleared the %s list of %s", ci->bi->nick, u->nick, u->username, u->host, xname, ci->name);
