...
int do_badwords(User * u, Channel *c, char *cmd, char *word, char *style) {
	...
	if (!cmd || (need_args ? 0 : !word)) {
		...
		if (readonly) {
			notice_lang(ci->bi->nick, u, BOT_BADWORDS_DISABLED);
			return MOD_CONT;
		}
		...
		for (bw = ci->badwords, i = 0; i < ci->bwcount; bw++, i++) {
			if (bw->word && ((BSCaseSensitive && (!strcmp(bw->word, word))) || (!BSCaseSensitive
				...
				notice_lang(ci->bi->nick, u, BOT_BADWORDS_ALREADY_EXISTS, bw->word, ci->name);
				return MOD_CONT;
			}
		}
		...
		if (i == ci->bwcount) {
			if (i < BSBadWordsMax) {
				...
				notice_lang(ci->bi->nick, u, BOT_BADWORDS_REACHED_LIMIT, BSBadWordsMax);
				return MOD_CONT;
			}
		}
		bw = &ci->badwords[i];
		...
		alog("%s: %s!%s@%s added badword \"%s\" to %s",
				ci->bi->nick, u->nick, u->username, u->host, bw->word, ci->name);
		notice_lang(ci->bi->nick, u, BOT_BADWORDS_ADDED, bw->word, ci->name);
		...
		if (isdigit(*word) && strspn(word, "1234567890,-") == strlen(word)) {
			...
			if (!deleted) {
				if (count == 1) {
					notice_lang(ci->bi->nick, u, BOT_BADWORDS_NO_SUCH_ENTRY, last, ci->name);
				} else {
					notice_lang(ci->bi->nick, u, BOT_BADWORDS_NO_MATCH, ci->name);
				}
			} else if (deleted == 1) {
				alog("%s: %s!%s@%s deleted 1 badword from %s",
						ci->bi->nick, u->nick, u->username, u->host, ci->name);
				notice_lang(ci->bi->nick, u, BOT_BADWORDS_DELETED_ONE, ci->name);
			} else {
				alog("%s: %s!%s@%s deleted %d badwords from %s",
						ci->bi->nick, u->nick, u->username, u->host, deleted, ci->name);
				notice_lang(ci->bi->nick, u, BOT_BADWORDS_DELETED_SEVERAL, deleted, ci->name);
			}
		} else {
			for (i = 0; i < ci->bwcount; i++) {
			...
			if (i == ci->bwcount) {
				notice_lang(ci->bi->nick, u, BOT_BADWORDS_NOT_FOUND, word, ci->name);
				return MOD_CONT;
			}
			bw = &ci->badwords[i];
			alog("%s: %s!%s@%s deleted badword \"%s\" from %s",
					ci->bi->nick, u->nick, u->username, u->host,  bw->word, ci->name);
			notice_lang(ci->bi->nick, u, BOT_BADWORDS_DELETED, bw->word, ci->name);
			if (bw->word)
				free(bw->word);
			bw->word = NULL;
			bw->in_use = 0;
		...
		if (ci->bwcount == 0) {
			notice_lang(ci->bi->nick, u, BOT_BADWORDS_LIST_EMPTY, ci->name);
			return MOD_CONT;
		}
		...
		alog("%s: %s!%s@%s cleared badwords on %s",
				ci->bi->nick, u->nick, u->username, u->host, ci->name);
		notice_lang(ci->bi->nick, u, BOT_BADWORDS_CLEAR);
