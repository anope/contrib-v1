...
int do_shun(User *u, Channel *c, char *arg) {
	...
	char *cmd = NULL, *target = NULL, *expiry = NULL, *reason = NULL, *temp = NULL;
	...
	/* Do not allow less than a minute expiry time */
		...
		if ((u2 = finduser(target))) {
			if (!action) {
				...
				noticeLang(ci->bi->nick, u, LANG_SHUN_ADDED, "*", u2->host);
				...
				noticeLang(ci->bi->nick, u, LANG_SHUN_DELETION, "*", u2->host);
			...
			if (!action) {
				send_cmd(NULL, "TKL + s %s %s %s %ld %ld :%s", uname, host, u->nick,
						(long int) (time(NULL) + expires), (long int) time(NULL), reason);
				noticeLang(ci->bi->nick, u, LANG_SHUN_ADDED, uname, host);
				...
				noticeLang(ci->bi->nick, u, LANG_SHUN_DELETION, uname, host);
...
int do_tshun(User *u, Channel *c, char *arg) {
	...
	User *u2;
	char breason[BUFSIZE];
	char *cmd = NULL, *target = NULL, *reason = NULL;
	int index = 0, action = 0;
	...
	if (!target)
		...
		noticeLang(ci->bi->nick, u, LANG_TSHUN_TARGET_NEXIST, target);
	else if (is_oper(u2))
		noticeLang(ci->bi->nick, u, LANG_TSHUN_TARGET_OPER);
	else {
		if (!action) {
			send_cmd(NULL, "TEMPSHUN +%s %s", u2->nick, reason);
			noticeLang(ci->bi->nick, u, LANG_TSHUN_ADDED, u2->nick);
			anope_cmd_global(ci->bi->nick, "%s added a TEMPSHUN on %s (%s)", u->nick, u2->nick, reason);
		} else {
			send_cmd(NULL, "TEMPSHUN -%s", u2->nick);
			noticeLang(ci->bi->nick, u, LANG_TSHUN_DELETION, u2->nick);
