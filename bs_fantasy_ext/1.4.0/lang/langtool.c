/**
 * Language tool taken from Anope.
 *
 ***********
...
int main(int argc, char *argv[])
{
	...
	/* Build the index.h file */
	else if (!strcmp(argv[1], "index.h"))
	{
		FILE *fd = fopen("index", "r");
		FILE *fdout = fopen("index.h", "w");
		char buf[1024];
		int i = 0;
		...
		fprintf(stderr, "Generating index.h... ");

        while (fgets(buf, 1023, fd)) {
			fprintf(fdout, "#define %-32s %d\n", strip(buf), i++);
        }

		fprintf(fdout, "#define MOD_NUM_STRINGS %d\n", i);
