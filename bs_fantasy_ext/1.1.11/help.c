...
int do_cmd_list(User *u, ChannelInfo *ci) {
...
#ifdef ENABLE_STAFF
	end += snprintf(end, sizeof(buf) - (end - buf), "%s%sstaff | %sircops", need_comma ? commastr : "", BSFantasyCharacter, BSFantasyCharacter);
	need_comma = 1;
#endif
	...
	*end = 0;
...
#ifdef ENABLE_TOPIC
	end += snprintf(end, sizeof(buf) - (end - buf), "%s%stopic", need_comma ? commastr : "", BSFantasyCharacter);
	need_comma = 1;
#endif
	...
	end += snprintf(end, sizeof(buf) - (end - buf), "%s%skb", need_comma ? commastr : "", BSFantasyCharacter);
	need_comma = 1;
#endif
 ...
 **/
int do_help(User *u, ChannelInfo *ci, char *cmd, char *param) {
#ifndef ENABLE_HELP
	moduleNoticeLang(ci->bi->nick, u, LANG_CMD_DISABLED);
#else
...
#ifdef ENABLE_TOPIC
			moduleNoticeLang(ci->bi->nick, u, LANG_TOPIC_SYNTAX_EXT, BSFantasyCharacter);
#else
			moduleNoticeLang(ci->bi->nick, u, LANG_CMD_DISABLED);
#endif
