	...
	if (!cmd || (!mask && (!stricmp(cmd, "ADD") || !stricmp(cmd, "STICK")
		...
		if (i == ci->akickcount) {
			if (ci->akickcount >= CSAutokickMax) {
				notice_lang(ci->bi->nick, u, CHAN_AKICK_REACHED_LIMIT, CSAutokickMax);
				return MOD_CONT;
			}
			ci->akickcount++;
		...
		if (mask && isdigit(*mask) && strspn(mask, "1234567890,-") == strlen(mask)) {
			...
			for (akick = ci->akick, i = 0; i < ci->akickcount; akick++, i++) {
				if (!(akick->flags & AK_USED))
					continue;
				if (mask) {
					if (!(akick->flags & AK_ISNICK) && !match_wild_nocase(mask, akick->u.mask))
						continue;
					if ((akick->flags & AK_ISNICK) && !match_wild_nocase(mask, akick->u.nc->display))
						continue;
				}
