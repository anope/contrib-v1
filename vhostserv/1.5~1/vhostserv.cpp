...
void HSSync(NickCore *nc, char *ident, char *host, const char *creator, time_t time);
...
class CommandHelp : public Command
{
	public:
		...
		void OnSyntaxError(User *u, const ci::string &m = "")
		{
...
class CommandRestrictedHosts : public Command
{
	private:
		...
		void DoAdd(User *u, const ci::string &host)
		{
		...
		void DoDel(User *u, const ci::string &host)
		{
			std::string rhost = host.c_str();
...
class Module_vHostServ : public Module
{
	public:
		...
		{
			if (!u->Account() || !command || ci->bi != vHostServ)
			{
				return;
			}
			else if (!params)
		...
		void OnReload(bool startup)
		{
			...
			BotHost = config.ReadValue("vhostserv", "host", Config.ServiceHost, 0);
...
bool SetVhost(User *u, std::string &param, bool group)
{
	...
	{
		if (strlen(ident) > Config.UserLen - 1)
		{
			notice_lang(s_vHostServ, u, HOST_SET_IDENTTOOLONG, Config.UserLen - 1);
		}
		else
		{
			for (s = ident; *s; s++)
	...
	if (identok)
	{
		...
		else if (strlen(host) > Config.HostLen - 1)
		{
			notice_lang(s_vHostServ, u, HOST_SET_TOOLONG, Config.HostLen - 1);
		}
		...
		else
		{
			if (!group)
			{
				...
				if (na)
				{
					na->hostinfo.SetVhost(ident ? ident : "", host, u->nick);
					ircdproto->SendVhost(u, ident ? ident : "", host);
				}
			}
			else
			{
				NickAlias *na = findnick(u->nick);
				if (na)
					HSSync(na->nc, ident, host, u->nick.c_str(), t);
			}
			...
			if (ident)
			{
				Alog() << s_vHostServ << ": Set the VHost of " << u->nick << " (" << u->GetIdent() << "@" << u->host << ") to \2" << ident << "@" << host << "\2";
				me->NoticeLang(s_vHostServ, u, LNG_SETHOST_IDENT, ident, host);
			}
			else
			{
				Alog() << s_vHostServ << ": Set the VHost of " << u->nick << " (" << u->GetIdent() << "@" << u->host << ") to \2" << host << "\2";
...
class DoUnban : public CallBack
{
	...
	public:
		DoUnban(time_t time, std::string &cname, std::string &bmask) : CallBack(me, time), channame(cname), banmask(bmask)
		{
		}
		...
		void Tick(time_t)
		{
			...
			if (ci && ci->c)
			{
				ci->c->RemoveMode(vHostServ, CMODE_BAN, banmask.c_str());
			}
		}
};
...
void PlaceBan(User *u, ChannelInfo *ci)
{
	...
	{
		if ((c = findchan(ci->name.c_str())) && c->ci)
		{
			...
			new DoUnban(BanTimeT, channame, banmask2);
		}
	}
}
...
void HSSync(NickCore *nc, char *ident, char *host, const char *creator, time_t time)
{
	...
	for (i = 0; i < nc->aliases.count; ++i)
	{
		...
		na->hostinfo.SetVhost(ident ? ident : "", host, creator, time);
