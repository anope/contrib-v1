...
#define VERSION     "$Id: os_modreload.c 1.1 $"
...
char *buf, *name;
    ...
    {
        /* reloading itself works fine but can result in weird crashes for future reloads. */
        if (stricmp(name, mod_current_module_name))
        {
            ...
            notice_user(s_OperServ, u, "Module \002%s\002 can't reload itself", name);
    }
    else
        notice_user(s_OperServ, u, "Module \002%s\002 not found", name);
