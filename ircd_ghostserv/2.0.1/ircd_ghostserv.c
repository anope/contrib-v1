...
#define VERSION "$Id: ircd_ghostserv.c v2.0.1 25-02-2007 n00bie $"
 ...
 * v2.0.1	- Fixed a mem leak (thanks Viper)
 * ----------------------------------------------------------------------------------
...
int load_config(void)
{
    ...
    if (!GSNick) {
		alog("ircd_ghostserv: \2GSNick\2 is missing on services configuration file!");
		retval = 0;
	}

    if (!GSIdent) {
        alog("ircd_ghostserv: \2GSIdent\2 is missing on services configuration file!");
		retval = 0;
	}

    if (!GSReal) {
        alog("ircd_ghostserv: \2GSReal\2 is missing on services configuration file!");
		retval = 0;
	}
	return retval;
}
