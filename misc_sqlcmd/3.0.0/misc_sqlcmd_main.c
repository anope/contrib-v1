...
int AnopeInit(int ac, char **av) {
	...
	status += sqlcmd_create("NICK_CONF", 2, sqlcmd_handle_nickconf);
	...
	status += sqlcmd_create("CHAN_DROP", 3, sqlcmd_handle_chandrop);
...
int sqlcmd_handle(int ac, char **av) {
	...
	while ((sqlcmd_row = mysql_fetch_row(sqlcmd_res))) {
		...
		if (debug)
			alog("[misc_sqlcmd] debug: Command %d: %s - Params: %s - Timestamp: %s - Checksum: %s", atoi(sqlcmd_row[0]), sqlcmd_row[1], sqlcmd_row[2], sqlcmd_row[3], sqlcmd_row[4]);
		chksum = sqlcmd_checksum(sqlcmd_row[1], sqlcmd_row[2], sqlcmd_row[3]);
		if (!chksum || stricmp(chksum, sqlcmd_row[4]) != 0) {
			...
			cmd_status = cmd->handle(new_ac, param_array);
