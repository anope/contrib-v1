 ...
 *************************************************************/
 ...
 /*
  * $Id: misc_sqlcmd_mysql.c 32 2007-08-23 20:47:21Z heinz $
  */
...
/* sqlcmd_mysql_quote - Quote a string to prevent injections */
char *sqlcmd_mysql_quote(char *unquoted)
{
	char *quoted;

	quoted = smalloc((1 + (strlen(unquoted) * 2)) * sizeof(char));
	mysql_real_escape_string(sqlcmd_mysql, quoted, unquoted, strlen(unquoted));
