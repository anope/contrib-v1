 ...
 *
 * We require the length of the pass in the DB because we cannot differentiate between strings
 * and hashes and strlen() tends to dislike 0 characters in hashes.
 **/
char *sqlcmd_checksum(char *cmd, char *param_str, char *pass, unsigned long pass_len, char *ts) {
	char *chksum_sql = NULL, *quote_param = NULL, *quote_pass = NULL, *chksum = NULL;
	int checksum_len = 0;

	checksum_len = strlen(cmd) +  + strlen(ts) + strlen(sqlcmd_chksum_salt) + 40;

	if (param_str) {
		quote_param = sqlcmd_mysql_quote(param_str, strlen(param_str));
		checksum_len += (strlen(quote_param) * 2);
	} else
		quote_param = sstrdup("");
	if (pass && pass_len) {
		quote_pass = sqlcmd_mysql_quote(pass, pass_len);
		checksum_len += (pass_len * 2);
	} else
		quote_pass = sstrdup("");

	chksum_sql = smalloc(sizeof(char) * checksum_len);
	snprintf(chksum_sql, checksum_len - 1, "SELECT MD5('%s:%s:%s:%s:%s')", cmd, quote_param,
			quote_pass, ts, sqlcmd_chksum_salt);
	free(quote_param);
	free(quote_pass);
