 ...
 * Date    : 25/08/2007  (Last update: 16/03/2008
 * Version : 1.0
 * -----------------------------------------------------------------------------
 * Tested     : Anope-1.7.21 SVN + UnrealIRCd 3.2.6
 * Requires   : Anope-1.7.22 or higher
 * -----------------------------------------------------------------------------
 *
 *                     ! ! ! ! ! ! ! ! ! ! ! ! ! !
 *                     ! ! ! READ THIS FIRST ! ! !
 ...
 * Just in case services are restarted or so...
 *
 * When switching from any other encryption type, the module works gradually,
 ...
 * This is because the module does not *yet* track which nicknames' passwords have
 * allready been converted to the new encryption method and does only provide the
 ...
 * TODO:
 *
 *  Add tracking ability of which nickgroups don't have new pass enc yet.
 *  Prevent use of getpass on passes that haven't been confirmed updated.
 *
 **/
...
int AnopeInit(int argc, char **argv) {
	...
	if (!moduleMinVersion(1,7,21,1370)) {
...
int do_immed_encrypt() {
    ...
    for (i = 0; i < 256; i++) {
        for (ci = chanlists[i]; ci; ci = ci->next) {
			...
			if (!ci->founderpass) {
				alog("[\002enc_switchover\002] No password present for Channel %s.", ci->name);
				continue;
			}
 ...
 * and old encryption type and update stored password if necessary.
 **/
int checkupdate_ns_password(NickAlias *na, const char *plaintext) {
	int res1, res2;
...
void update_ns_password(NickCore *nc, const char *password) {
	...
	if (enc_encrypt(pwd, len, nc->pass, PASSMAX) < 0) {
		memset(pwd, 0, len);
...
void update_cs_password(ChannelInfo *ci, const char *password) {
	...
	if (enc_encrypt(pwd, len, ci->founderpass, PASSMAX) < 0) {
		memset(pwd, 0, len);
...
void load_config(void) {
	...
	Directive confvalues[][1] = {
		{{"OldEncModule", {{PARAM_STRING, PARAM_RELOAD, &OldEncModule}}}},
	};

	/* We do not support changing OldEncModule at runtime! */
	if (OldEncModule) {
		...
		if (OldEncModule && strcmp(OldEncModule,"enc_none") && strcmp(OldEncModule,"enc_old")
				&& strcmp(OldEncModule,"enc_md5") && strcmp(OldEncModule,"enc_sha1")) {
		...
		if (OldEncModule && !strcmp(OldEncModule, EncModule)) {
