...
int AnopeInit(int argc, char **argv)
{
	...
	hook = createEventHook(EVENT_NEWNICK, my_nickservreglock_connect);
	moduleAddEventHook(hook);
	hook = createEventHook(EVENT_CHANGE_NICK, my_nickservreglock_change);
	moduleAddEventHook(hook);
	hook = createEventHook(EVENT_USER_LOGOFF, m_ns_someuserquit);
	moduleAddEventHook(hook);
