...
int AnopeInit(int argc, char **argv)
{
	...
	// Create NickServ TICKETGHOST command
	c = createCommand("TICKETGHOST", ticketGhostCallback, NULL, -1, -1, -1, -1, -1);
	
	moduleAddHelp(c, ticketGhostHelpRequest);

	status = moduleAddCommand(NICKSERV, c, MOD_HEAD);
	alog("ns_sso: INFO: Add command 'TICKETGHOST' status: %d", status);
...
int checkTicket(char* ticket, char* hostip, char* usernick, int markused)
{
	...
	{
		alog("ns_sso: ERROR: CheckTicket(%s, %s, %s, %d): supplied values are not valid.",
			ticket, hostip, usernick, markused);
	...
	if (PQresultStatus(res) != PGRES_TUPLES_OK)
	{
		alog("ns_sso: ERROR: CheckTicket(%s, %s, %s, %d): Unable to query for ticket: %s",
			ticket, hostip, usernick, markused, PQerrorMessage(dbConn));
	...
	if (PQntuples(res) != 1)
	{
		alog("ns_sso: FAIL: CheckTicket(%s, %s, %s, %d): No valid ticket exists (expected %d "
			"ticket(s), got %d ticket(s))", ticket, hostip, usernick, markused, 1, PQntuples(res));
	...
	{
		alog("ns_sso: ERROR: CheckTicket(%s, %s, %s, %d): Some columns do not exist.",
			ticket, hostip, usernick, markused);

		alog("ns_sso: ERROR: CheckTicket(%s, %s, %s, %d): Created=%d, Nickname=%d",
			ticket, hostip, usernick, markused, COL_CREATED, COL_NICKNAME);
	...
	{
		alog("ns_sso: ERROR: CheckTicket(%s, %s, %s, %d): creation time stamp is empty",
			ticket, hostip, usernick, markused);
	...
	{
		alog("ns_sso: ERROR: CheckTicket(%s, %s, %s, %d): nickname ticket is for is empty",
			ticket, hostip, usernick, markused);
	...
	{
		alog("ns_sso: FAIL: CheckTicket(%s, %s, %s, %d): A ticket exists, but it is "
			"not for this nickname (expected: %s, got: %s).",
			ticket, hostip, usernick, markused, nicknameTicketIsFor, usernick);
	...
	{
		alog("ns_sso: FAIL: CheckTicket(%s, %s, %s, %d): Ticket expired (%d seconds is " 
			"older than TTL of %d seconds)", ticket, hostip, usernick, markused,
	...
	alog("ns_sso: SUCCESS: CheckTicket(%s, %s, %s, %d): The ticket seems valid and has " 
		"a remaining TTL of %d seconds", ticket, hostip, usernick, markused,
		(timeNow - createdTimeStamp));

	PQclear(res);

	// Finally, mark the ticket as used if desired
	if (markused == 1)
	{
		res = PQexecParams(dbConn,
		...
		if (PQresultStatus(res) != PGRES_COMMAND_OK)
		{
			alog("ns_sso: ERROR: CheckTicket(%s, %s, %s, %d): Unable to set ticket to "
				"used: %s", ticket, hostip, usernick, markused, PQerrorMessage(dbConn));
		...
		alog("ns_sso: SUCCESS: CheckTicket(%s, %s, %s, %d): The ticket has been marked as "
			"used in the database.", ticket, hostip, usernick, markused);

		PQclear(res);
	}
	else
	{
		alog("ns_sso: INFO: CheckTicket(%s, %s, %s, %d): The ticket has NOT been marked as "
			"used in the database.", ticket, hostip, usernick, markused);
	}
...
int ticketIdentifyCallback(User *u)
{
	...
	res = checkTicket(ticket, u->hostip, u->nick, 1);
...
{
	// Code mostly shoplifted from Anope ns_ghost core module. :-|
	...
	if (!nick || !ticket)
	{
		moduleNoticeLang(s_NickServ, u, LNG_TICKETGHOST_SYNTAX);

		alog("ns_sso: NOTICE: %s!%s@%s missing nick or ticket for TICKETGHOST command.",
			u->nick, u->username, u->host);

		goto done;
	}

	// Is nick to ghost online?	
	if (!(u2 = finduser(nick)))
	{
	...
	// Is bob trying to ghost bob?
	if (stricmp(nick, u->nick) == 0)
	{
		notice_lang(s_NickServ, u, NICK_NO_GHOST_SELF);
		goto done;
	}
	
	// Here the stealing and style change of ns_ghost.c ends; we now check the
	// ticket's validity and if it passes, ghost the nick.
	
	// In this case, we check whether the ticket's nick is the nick to ghost.
	// hostip should still be the one we expect. 0 means don't mark ticket used.
	int res = checkTicket(ticket, u->hostip, nick, 0);
	...
	if (res != TICKET_OK)
	{
		...
		{
			case TICKET_BAD: // Ticket not found, wrong IP, etc...
			alog("ns_sso: NOTICE: Failed TICKETGHOST of %s for %s!%s@%s",
		...
		default:
			case TICKET_ERROR: // checkTicket could not do its job
			alog("ns_sso: NOTICE: Error during TICKETGHOST of %s for %s!%s@%s: "
			"checkTicket returned %d", nick, u->nick, u->username, u->host, res);

			// "Sorry, identification failed." fits here
			notice_lang(s_NickServ, u, NICK_IDENTIFY_FAILED);
	...
	// If we get here, the ticket is fine
	char buf[NICKMAX + 32];
	snprintf(buf, sizeof(buf), "GHOST command used by %s", u->nick);
	
	// We are not modifying the above message ("GHOST command") by changing it
	// to "TICKETGHOST command" because there are IRC clients in the wild that
	// do special things when they see "GHOST command" in a quit message. 
...
{
	moduleNoticeLang(s_NickServ, u, LNG_TICKETGHOST_HELP, SITE_NAME);
	return MOD_CONT;
}
...
void createAndRegisterLanguageConstants(void)
{
	...
	char *langtable_en_us[] = {
		...
		/* LNG_TICKETGHOST_HELP */
		"Syntax: TICKETGHOST nickname ticket\n"
			"\n"
			...
			"that nick, or you must supply a valid ticket for the\n"
			"nickname.",
	...
	char *langtable_de[] = {
		...
		"Syntax: GHOST Nickname Ticket\n"
			...
			"du ein gültiges Ticket für den Nickname haben.\n",
		/* LNG_CHANGE_EMAIL_AT_SITE */
