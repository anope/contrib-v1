...
#define findMessage myFindMessage
...
static Message *myFindMessage(MessageHash * msgTable[], const char *name)
{
	int idx;
...
int AnopeInit(int argc, char **argv)
{
	...
	for (i = 0; i < 256; i++)
	{
		for (ci = chanlists[i]; ci; ci = ci->next)
		{
			if (!ci->c && ci->bi)
			{
				anope_cmd_join(ci->bi->nick, ci->name, ci->time_registered);
				chan_create(ci->name, ci->time_registered);
				check_modes(ci->c);
			}
		}
	}
