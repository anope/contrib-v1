...
int AnopeInit(int argc, char **argv) 
{
	...
	moduleSetHelpHelp(myMassModeHelp);
...
{
	if (is_services_admin(u)) {
	notice(s_OperServ, u->nick, "Syntax: /002massmode +(-)modes/002");
	notice(s_OperServ, u->nick, "This will allow you to set mode(s)'s on all registered channels.");
   }
}   
