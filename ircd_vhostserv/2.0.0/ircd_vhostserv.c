...
#define VERSION "$Id: ircd_vhostserv.c v2.0.0 09-02-2007 n00bie $"
 ...
 * ------------------------------------------------------------------------------------
 * This Version is tested only on UnrealIRCd3.2.6 and Anope-1.7.18
 ...
 * ------------------------------------------------------------------------------------
 * Thanks to rassi for informing about the bugs ^^
 * Many many thanks to EZEki3l for testing the module.. :)
 * Borrowed some codes from Trystan vhost fantasy module ;)
 *
...
{
	ChannelInfo *ci = cs_findchan(HostChan);
	Message *msg = NULL;
	int status;
	...
	if (!stricmp(IRCDModule, "bahamut") || !stricmp(IRCDModule, "dreamforge") ||
		!stricmp(IRCDModule, "hybrid") || !stricmp(IRCDModule, "ratbox")) {
			alog("ircd_vhostserv: \2IRCd not supported.\2");
			return MOD_STOP;
	}
	if (!ci) {
		if (LogChannel) {
			alog("ircd_vhostserv: Channel %s is not registered. You need to register the channel first for this module to work.", HostChan);
		} else {
			notice(s_OperServ, argv[0], "Channel %s is not registered. You need to register the channel first for this module to work. Unloading module.", HostChan);
		}
		return MOD_STOP;
	}
...
int vhost_fantasy(User *u, char *cur_buffer)
{
	...
	if ((na = findnick(nick))) {
		...
		addHostCore(nick, vIdent, hostmask, u->nick, time(NULL));
		if (vIdent) {
