 ...
 * Date    : 01/04/2008  (Last update: 05/04/2008)
 * Version : 1.0
 * -----------------------------------------------------------------------------
 ...
 * If this is the case, bs_fantasy_ext will unload itself if it s a config directive,
...
void cs_do_join(ChannelInfo *ci) {
	if (BSSmartJoin) {
		...
		if (count) {
			...
			for (i = 0; i < count; i++) {
				if (match_wild_nocase(ci->c->bans[i], cs_mask)) {
					anope_cmd_mode(s_ChanServ, ci->name, "-b %s", bans[i]);
					if (ircdcap->tsmode)
						av[3] = sstrdup(bans[i]);
					else
						av[2] = sstrdup(bans[i]);
