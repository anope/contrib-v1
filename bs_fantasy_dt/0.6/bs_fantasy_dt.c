 ...
 * Date    : 04/10/2009  (Last update: 10/01/2011)
 * Version : 0.7-GIT
 * -----------------------------------------------------------------------------
 * Tested     : Anope-1.8.5-SVN + InspIRCd 1.2.7
 * Requires   : Anope-1.8.5-SVN
 * -----------------------------------------------------------------------------
 ...
 *            $2 && !$3 && $1- == "anope rulezzz" || !$1 && $me == "viper"
 *            !$3 && $1- == "anope rulezzz" || !$1 && $me == "viper"
 *            (!$3 && $1- == "anope rulezzz") || (!$1 && $me == "viper")
 *
 * -----------------------------------------------------------------------------
 * Translations:
 ...
 * Changelog:
 *
 *    0.7    Fixed some potentially uninitialized variables. (Spotted by Adam)
 *
 ...
 * TODO:
 *
 *     - think about some anti flood precautions..
 **/
...
/**
 * Number of entries to store in a channels' anti-flood list. [ NOT YET IMPLEMENTED ]
 **/
int aflEntries = 3;
...
#define TRIGGERDBVERSION 1


/* Language defines */
#define LANG_NUM_STRINGS 					127
...
typedef struct antifloodchan_ AntiFloodChan;
typedef struct antiflooddata_ AntiFloodData;
...
struct antifloodchan_ {
	AntiFloodChan *prev, *next;
	char name[CHANMAX];
	AntiFloodData *afd;
};

struct antiflooddata_ {
	char *trigger;
	time_t time;
};
...
AntiFloodChan *aflc[256];


/* Functions */
int do_trigger(char *source, int ac, char **av);
void send_trigger(ChannelInfo *ci, User *u, TriggerData *td, TriggerEntry *te, int is_global, char *args);
char *buildTriggerReply(char *txt, char *nick, char *args);
char *nextSpecialChar(char *ptr);
int validate_condition_syntax(char *cond);
int check_triggerreply_condition(char *cond, char *nick, char *args);
int check_triggerreply_condition_ext(char **ptr, char *nick, char *args, int subcond);
int parse_statement(char **stmt, char *nick, char *args);
int get_rand_reply(TriggerEntry *te, char *nick, char *args);

int do_fantasy(int ac, char **av);
void send_help(char *bot, User *u, char *cmd, short int global);
...
void send_trigger(ChannelInfo *ci, User *u, TriggerData *td, TriggerEntry *te, int is_global, char *args) {
	...
	if (te->count > 1) {
		/* There are multiple items in the trigger entry.. */
		if (te->flags & TE_LIST) {
			/* It s a list.. show all stored entries. */
			for (i = 0; i < te->count; i++) {
				if (!check_triggerreply_condition(te->replies[i]->condition, u->nick, args))
					continue;
				msg = buildTriggerReply(te->replies[i]->data, u->nick, args);
				if ((td && (td->flags & TD_USE_PRIVMSG)) || (!td && (DefTdFlags & TD_USE_PRIVMSG)))
					anope_cmd_privmsg(botnick, ci->name, "%s%s[%d]: %s", (is_global ? "[Global] " : ""), te->trigger, i+1, msg);
				else
					anope_cmd_notice(botnick, ci->name, "%s%s[%d]: %s", (is_global ? "[Global] " : ""), te->trigger, i+1, msg);
				free(msg);
			}
		} else {
			/* It s no list.. randomly pick a line to display.. */
			r = get_rand_reply(te, u->nick, args);
			if (r >= 0) {			/* It's possible there are entries matching all conditions.. */
				msg = buildTriggerReply(te->replies[r]->data, u->nick, args);
				if ((td && (td->flags & TD_USE_PRIVMSG)) || (!td && (DefTdFlags & TD_USE_PRIVMSG)))
					anope_cmd_privmsg(botnick, ci->name, "%s%s[%d]: %s", (is_global ? "[Global] " : ""), te->trigger, r+1, msg);
				else
					anope_cmd_notice(botnick, ci->name, "%s%s[%d]: %s", (is_global ? "[Global] " : ""), te->trigger, r+1, msg);
				free(msg);
			}
		}
	} else if (check_triggerreply_condition(te->replies[0]->condition, u->nick, args)) {
		/* There s only one entry to display.. */
		msg = buildTriggerReply(te->replies[0]->data, u->nick, args);
		if ((td && (td->flags & TD_USE_PRIVMSG)) || (!td && (DefTdFlags & TD_USE_PRIVMSG)))
			anope_cmd_privmsg(botnick, ci->name, "%s%s: %s", (is_global ? "[Global] " : ""), te->trigger, msg);
		else
			anope_cmd_notice(botnick, ci->name, "%s%s: %s", (is_global ? "[Global] " : ""), te->trigger, msg);
		free(msg);
	}
}
 ...
 * @param nick Nickname of the user calling the trigger.
...
int validate_condition_syntax(char *cond) {
	char **ptr = &cond;
	if (check_triggerreply_condition_ext(ptr, NULL, NULL, 0) == -1)
		return 0;
	return 1;
}
 ...
 * @param nick String Nick of the user requesting the trigger.
...
int check_triggerreply_condition(char *cond, char *nick, char *args) {
	char **ptr = &cond;
	int ret = check_triggerreply_condition_ext(ptr, nick, args, 0);
	return (ret <= 0 ? 0 : 1);
}
...
int check_triggerreply_condition_ext(char **ptr, char *nick, char *args, int subcond) {
	...
	while ((*ptr)[0]) {
		...
		if ((*ptr)[0] ==  '(' && operation) {
			*ptr += 1;
			result = check_triggerreply_condition_ext(ptr, nick, args, 0);
		...
		if ((*ptr)[0] == '$' && operation) {
			result = parse_statement(ptr, nick, args);
			if (result == -1) return result;
		...
		if ((*ptr)[0] == '&' && (*ptr)[1] == '&' && !operation) {
			...
			*ptr += 2;
			result = check_triggerreply_condition_ext(ptr, nick, args, 1);
			if (result == -1) return result;
			ret = ret || result;
		} else
			return -1;
	...
	if ((*stmt)[0] >= '0' && (*stmt)[0] <= '9') {
		...
		*stmt += cnt;
		...
		if (strstr(*stmt, "me") == *stmt) {
			*stmt += 2;
			if (nick)			/* This can be empty when called by for validation purposes.. */
				value = sstrdup(nick);
		}
	}
 ...
 **/
int get_rand_reply(TriggerEntry *te, char *nick, char *args) {
	int matching[MaxLines], matches = 0, i, r;
	for (i = 0; i < te->count; i++) {
		if (check_triggerreply_condition(te->replies[i]->condition, nick, args)) {
			matching[matches] = i;
			matches++;
		}
	}
...
int do_fantasy(int ac, char **av) {
	...
	if (!stricmp(av[0], "gdt")) {
		...
		if (ac > 3) {
			...
			if (!stricmp(cmd, "dt")) {
				send_help(botnick, u, param, 0);
				ret = MOD_STOP;
			} else if (!stricmp(cmd, "gdt")) {
				send_help(botnick, u, param, 1);
				ret = MOD_STOP;
			}
			free(cmd);
			if (param) free(param);
...
void load_trigger_db(void) {
	...
	char *key, *value;
	int retval = 0, adding_globals = 0, last_data = -1;
	TriggerData *td = NULL;
	TriggerEntry *te = NULL;
	...
	while (1) {
		...
		if (retval == DB_READ_ERROR) {
			...
			/* Link entries must be loaded last.. (They should be saved at the end too..) */
				...
				if ((int)atoi(value) != TRIGGERDBVERSION) {
...
#ifdef SUPPORTED
	snprintf(tmp, BUFSIZE, "%s-%s-T%d-L%d", version_flags, ((supported == -1) ? "U" : 
			(supported == 0) ? "u" : "S"), MaxTriggers, MaxLines);
#else
	snprintf(tmp, BUFSIZE, "%s-HU-T%d-L%d", version_flags, MaxTriggers, MaxLines);
#endif
	flags = sstrdup(tmp);
	return flags;
}
...
void load_config(void) {
	...
	if (debug)
		alog ("[bs_fantasy_dt] debug: TriggerDB set to %s. TriggerCharacter is %s.", TriggerDB, TriggerCharacter );
}
...
void add_languages(void) {
	char *langtable_en_us[] = {
		/* LANG_TRIGGER_HELP */
		" To query the trigger database use \037%strigger [arguments]\037.",
		/* LANG_DT_DESC */
		...
		" Syntax: %sdt APP trigger [+nr] reply\n"
		" \n"
		" DT APP appends an existing trigger reply. If a trigger has multiple replies a number specifying which reply\n"
		" to append is mandatory.",
		/* LANG_DT_DEL_SYNTAX */
		...
		" Syntax: %BSFdt LINK trigger target\n"
		" \n"
		" DT LINK links a trigger to the target trigger.",
		/* LANG_GDT_LINK_SYNTAX */
		" Syntax: %BSFgdt LINK trigger target",
		/* LANG_GDT_LINK_SYNTAX_EXT */
		" Syntax: %BSFgdt LINK trigger target\n"
		" \n"
		" GDT LINK links a trigger to the target trigger.",
		/* LANG_DT_EXISTS */
	...
	char *langtable_nl[] = {
		/* LANG_TRIGGER_HELP */
		" Gebruik \037%strigger [parameters]\037 om items uit de database op te vragen.",
		/* LANG_DT_DESC */
		...
		" Syntax: %BSFdt APP item [+nr] antwoord",
		/* LANG_DT_APP_SYNTAX_EXT */
		" Syntax: %sdt APP item [+nr] antwoord\n"
		" \n"
		" DT APP voegt tekst aan een reeds bestaand antwoord toe. Als een item meerdere antwoorden heeft moet een nummer\n"
		" specifieren waaraan de tekst toegevoegd moet worden.",
		/* LANG_DT_DEL_SYNTAX */
		...
		" Syntax: %BSFdt LINK item doel\n"
		" \n"
		" DT LINK schakelt item door naar het doel item.",
		/* LANG_GDT_LINK_SYNTAX */
		" Syntax: %BSFgdt LINK item doel",
		/* LANG_GDT_LINK_SYNTAX_EXT */
		" Syntax: %BSFgdt LINK item doel\n"
		" \n"
		" GDT LINK schakelt item door naar het doel item.",
		/* LANG_DT_EXISTS */
		" Item bestaat reeds.",
		/* LANG_DT_LINKED */
		...
		" Syntax: %BSFdt DUP item doel\n"
		" \n"
		" DT DUP kopieert de inhoud van een bestaande trigger naar een\n"
		" nieuwe onbestaande trigger.",
		/* LANG_GDT_DUP_SYNTAX */
		" Syntax: %BSFgdt DUP item doel",
		/* LANG_GDT_DUP_SYNTAX_EXT */
		" Syntax: %BSFgdt DUP item doel\n"
		" \n"
		" GDT DUP kopieert de inhoud van een bestaande trigger naar een\n"
		" nieuwe onbestaande trigger.",
		/* LANG_DT_DUP */
