...
int do_chanconf (User *u)
{
				...
				notice(s_OperServ, u->nick, " - ");
				notice(s_OperServ, u->nick, "%s Configuration - Modes:%s",s_ChanServ, ircd->chanservmode);
				notice(s_OperServ, u->nick, "Chan Expires: %s             Registration Max: %s                 Access List Max: %s", my_csexpire, my_csregmax, my_csaccessmax);
				notice(s_OperServ, u->nick, "Opers only: %s            Inhabit length: %s                             Default bantype: %s", (my_csopersonly ? "Enabled" : "Disabled" ), my_csinhabit, my_csdefbantype);
				notice(s_OperServ, u->nick, "List for opers only: %s            Max entries per list: %s                 Akick List Max: %s", (my_cslistopersonly ? "Enabled" : "Disabled" ), my_cslistmax, my_csakickmax);
				notice(s_OperServ, u->nick, "Default akick reason: %s", my_csautokickreason);

				if (is_oper(u)) {
					if (findModule("cs_getpass")) {
						notice(s_OperServ, u->nick, "Chan GETPASS: %s", (my_csgetpass ? "Root Only" : "Admin" ));
					} else {
						notice(s_OperServ, u->nick, "Chan GETPASS: Disabled");
					}
					notice(s_OperServ, u->nick, " - ");
					notice(s_OperServ, u->nick, "Core Modules: %s", my_chancoremodules);
				}
				notice(s_OperServ, u->nick, " - ");
				notice(s_OperServ, u->nick, "End of Configuration");
...
int do_memoconf (User *u)
{
				...
				moduleGetConfigDirective(confvalues2);
					...
					notice(s_OperServ, u->nick, " - ");
					notice(s_OperServ, u->nick, "%s Configuration - Modes:%s",s_MemoServ, ircd->memoservmode);
					notice(s_OperServ, u->nick, "Max Memos: %s         Send Delay: %s", my_msmaxmemos, my_mssenddelay);

					if (is_oper(u)) {
						notice(s_OperServ, u->nick, "Core Modules: %s", my_memocoremodules);
					}
					notice(s_OperServ, u->nick, " - ");
					notice(s_OperServ, u->nick, "End of Configuration");
...
int do_hostconf (User *u)
{
				...
				moduleGetConfigDirective(confvalues1);
						...
						notice(s_OperServ, u->nick, " - ");
						notice(s_OperServ, u->nick, "%s Configuration - Modes:%s",s_HostServ, ircd->hostservmode);
						notice(s_OperServ, u->nick, "HostSetters: %s", (my_hostsetters ? my_hostsetters : "None" ));

						if (is_oper(u)) {
							notice(s_OperServ, u->nick, "Core Modules: %s", my_hostcoremodules);
						}
						notice(s_OperServ, u->nick, " - ");
						notice(s_OperServ, u->nick, "End of Configuration");
...
int do_botconf (User *u)
{
				...
				notice(s_OperServ, u->nick, " - ");
				notice(s_OperServ, u->nick, "%s Configuration - Modes:%s",s_BotServ, ircd->botservmode);
				notice(s_OperServ, u->nick, "Minimum users on channel: %s          Max badwords: %s               Keep data: %s", my_bsminusers, my_bsbadwordsmax, my_bskeepdata);
				notice(s_OperServ, u->nick, "Smart join: %s Gentle reason: %s Case sensitive: %s", (my_bssmartjoin), (my_bsgentlebwreason ? my_bsgentlebwreason : "Disabled"), (my_bscasesensitive ? my_bscasesensitive : "Disabled"));
				notice(s_OperServ, u->nick, "Fantasy character: %s", my_bsfantasycharacter);

				if (is_oper(u)) {
					notice(s_OperServ, u->nick, "Core Modules: %s", my_botcoremodules);
				}
				notice(s_OperServ, u->nick, " - ");
				notice(s_OperServ, u->nick, "End of Configuration");
...
int do_nickconf (User *u)
{
	...
	if (is_oper(u)) {
		if (UseSVSHOLD) {
			notice(s_OperServ, u->nick, "Enforcer type: SVSHOLD");
		} else {
			notice(s_OperServ, u->nick, "Enforcer type: %s", my_enforceruser);
		}
		notice(s_OperServ, u->nick, "Core Modules: %s", my_nickcoremodules);
	}

	if (findModule("ns_getpass")) {	
		notice(s_OperServ, u->nick, "Nick GETPASS: %s", (my_nsgetpass ? "Root Only" : "Admin"));
	} else {
		notice(s_OperServ, u->nick, "Nick GETPASS: Disabled");
	}
	notice(s_OperServ, u->nick, " - ");
	notice(s_OperServ, u->nick, "End of Configuration");
