...
void vident_check(char *s);
void vhost_check(char *s);
...
int set_vhost(int argc, char **argv) {
	...
	cust_host = strdup(NetworkHost);
	cust_host = str_replace(rep_nick, na->nick, cust_host);
	...
	/* Make sure ident and host are valid */
	if (vIdent) vident_check(vIdent);
	vhost_check(vHost);
...
/**
 * Replaces non-letters and non-numbers by _s.
 **/
void vident_check(char *s) {
	register int c;
...
char *str_replace(char *replace_target, char *replace_with, char *source) {
	...
	while ((tmp = strstr(ret, replace_target))) {
		length = strlen(ret);

		if (diff > 0) {
			ret = (char *) realloc(ret, sizeof(char) * (length + diff) + 1);
			memmove(tmp + strlen(replace_with), tmp + strlen(replace_target),
					sizeof(char) * (strlen(tmp) - strlen(replace_target)) +1);
		}

		memcpy(tmp, replace_with, strlen(replace_with));
