 ...
 * Version : 1.4
 * Date    : (Created) August 7th, 2010 (Last modified) June 26th, 2011
 * ----------------------------------------------------------------------------
 ...
 * Changelog:
 *
 *   1.4 - Fix potential crash on trying to free a null pointer in do_set.
 *       - Only allow a trigger character with an ASCII value over 32.
 *   1.3 - Fix compilation error under Windows.
...
static const char *const VERSION = "v1.4";
			...
			if (ci && !(ci->flags & CI_VERBOTEN))
			{
				...
				if (cmd && !stricmp(cmd, "FANTASY"))
				{
					...
					if (is_services_admin(u) || (ci->flags & CI_SECUREFOUNDER ? is_real_founder(u, ci) : is_founder(u, ci)))
					{
						...
						/* We only accept 1-character triggers as that is all the core accepts anyways, and it must not be a control character or a space */
						else if (strlen(trigger) == 1 && trigger[0] > 32)
						{
