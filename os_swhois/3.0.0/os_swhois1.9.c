...
int swhois_set(User *u) {
	...
	if (!nick_identified(u))
		...
		swhois = get_swhois(u);
		ircdproto->SendSWhois(s_NickServ, u->nick, swhois);
...
int do_on_identify(int ac, char **av) {
	...
	me->DelEventHook(EVENT_NICK_IDENTIFY);
	return MOD_CONT;
}
int nick_drop(int ac, char **av) {
	...
	me->DelEventHook(EVENT_NICK_DROPPED);
	return 1;
}
int nick_expire(int ac, char **av) {
	if (nick_exists(NULL, av[0]))
	...
	me->DelEventHook(EVENT_NICK_EXPIRE);
	return 1;
}
int do_group(int ac, char **av) {
	if (nick_exists(NULL, av[0]))
...
MODULE_INIT("os_swhois", OSSwhois)
