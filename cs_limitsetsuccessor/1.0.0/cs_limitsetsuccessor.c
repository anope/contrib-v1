...
int cs_limitsetsuccessor_conf()
{
	...
	if (!tmp || (stricmp(tmp, "root") && stricmp(tmp, "admin") && stricmp(tmp, "oper"))) {
		...
		if (mod_current_user) {
	   		notice(s_OperServ, mod_current_user->nick, "\002%s\002 - Invalid or Missing configuration directive. See services.log for more information.", MODNAME, set_successor_level);
			if (set_successor_level)
