...
HE_TOS v1.2
	Changes:
		+ Added !tos to fantasy.
	TODO:
		Make it a editable command.
	
	Tested by Fleck.
	Fantasy taken from bs_911 by Viper
*/
...
int he_tos_fantasy(int argc, char **argv);
...
int AnopeInit(int argc, char **argv) {
	...
	hook = createEventHook(EVENT_BOT_FANTASY, he_tos_fantasy);
	if (moduleAddEventHook(hook) != MOD_ERR_OK) {
		alog("[\002he_tos\002] Can't hook to EVENT_BOT_FANTASY event");
		return MOD_STOP;
	}
	hook = createEventHook(EVENT_BOT_FANTASY_NO_ACCESS, he_tos_fantasy);
	if (moduleAddEventHook(hook) != MOD_ERR_OK) {
		alog("[\002he_tos\002] Can't hook to EVENT_BOT_FANTASY_NO_ACCESS event");
		return MOD_STOP;
	}
...
int he_tos_fantasy(int argc, char **argv) {
	...
	if (argc >= 3 && !stricmp(argv[0], "tos")) { 
		if (((ci = cs_findchan(argv[2]))) && ((u = finduser(argv[1])))) {
			anope_cmd_privmsg(ci->bi->nick, ci->name, "You can find the Terms of Service at \002%s\002", tos_conf);
		} 
	}
	return MOD_CONT; 
}
...
int load_config(void) {
	...
	if (tos_conf) {
		alog("[\002he_tos\002] TOS link is set to '%s' in the config, config loaded! :D", tos_conf);
		return true;
	} else {
