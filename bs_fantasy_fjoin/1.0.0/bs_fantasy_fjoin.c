/*
 * -----------------------------------------------------------------------------
 * Name    : bs_fantasy_fjoin
 * Author : Bubblez6000 <bubblez@phantomirc.net> Base Code by: Viper <Viper@Absurd-IRC.net>
 * Date    : 30/04/2008  (Last update: 23/08/2007)
 * Version : 1.0
 * -----------------------------------------------------------------------------
 * Limitations : Any IRCd which supports SVSJOIN
 * Requires    : Anope-1.7.21 or higher
 * Tested      : Anope-1.7.21 + UnrealIRCd 3.2.7
 * -----------------------------------------------------------------------------
 * This module creates the !fjoin fantasy command.
...
#define LANG_FJOIN_SYNTAX					0
#define LANG_NO_SUCH_USER					1
#define LANG_CHANNAME_TOO_LONG				2
#define LANG_USER_IN_CHAN				3
#define LANG_FJOINING						4
#define LANG_FORCE_JOINED					5

/* Functions */
int do_fantasy(int ac, char **av);
int do_fantasy_denied(int ac, char **av);
void do_fjoin(User *u, Channel *c, char *target, char *reason);
int valid_ircd(void);
...
int AnopeInit(int argc, char **argv) {
	...
	alog("[\002bs_fantasy_fjoin\002] Loading module...");

	if (!valid_ircd()) {
		alog("[\002bs_fantasy_fjoin\002] ERROR: IRCd not supported by this module");
		alog("[\002bs_fantasy_fjoin\002] Unloading module...");
		return MOD_STOP;
	}

	if (!moduleMinVersion(1,7,17,1192)) {
		alog("[\002bs_fantasy_fjoin\002] Your version of Anope isn't supported. Please update to a newer release.");
		return MOD_STOP;
	}

	hook = createEventHook(EVENT_BOT_FANTASY, do_fantasy);
	if (moduleAddEventHook(hook) != MOD_ERR_OK) {
		alog("[\002bs_fantasy_fjoin\002] Can't hook to EVENT_BOT_FANTASY event");
		return MOD_STOP;
	}

	hook = createEventHook(EVENT_BOT_FANTASY_NO_ACCESS, do_fantasy_denied);
	if (moduleAddEventHook(hook) != MOD_ERR_OK) {
		alog("[\002bs_fantasy_fjoin002] Can't hook to EVENT_BOT_FANTASY_NO_ACCESS event");
		return MOD_STOP;
	}
	...
	alog("[\002bs_fantasy_fjoin\002] Module loaded successfully...");
...
int do_fantasy(int ac, char **av) {
	...
	if (stricmp(av[0], "fjoin") == 0) {
		/* If executed without any params, show help */
		if (ac == 3)
			moduleNoticeLang(ci->bi->nick, u, LANG_FJOIN_SYNTAX, BSFantasyCharacter);
		else {
...
void do_fjoin(User *u, Channel *c, char *nick, char *reason) {
	User *target = NULL;
	...
	if (!(target = finduser(nick))) {
        ...
        /* Check whether user is already on channel */
        if (is_on_chan(c, target)) {
                moduleNoticeLang(c->ci->bi->nick, u, LANG_USER_IN_CHAN, nick);
                return;
        }


	moduleNoticeLang(c->ci->bi->nick, u, LANG_FJOINING, nick, c->name);

	/* make some noise to prevent abusers */
	wallops(c->ci->bi->nick, "%s used SVSJOIN to join %s to %s. Reason: [%s]", u->nick, nick, c->name, reason);
	moduleNoticeLang(s_ChanServ, target, LANG_FORCE_JOINED, c->name, u->nick, reason);

	/* Force them in */
	anope_cmd_svsjoin(c->ci->bi->nick, nick, c->name, 0);
}
...
void add_languages(void) {
	char *langtable_en_us[] = {
		/* LANG_FJOIN_SYNTAX */
		"Syntax: %sfjoin[\037nick\037] [\037reason\037]",
		/* LANG_NO_SUCH_USER */
		" User [%s] does not exist or is a services client",
		/* LANG_CHANNAME_TOO_LONG */
		" That channel name is too long.",
		/* LANG_USER_IN_CHAN */
		" The user [%s] is in channel.",
		/* LANG_FJOINING */
		"Forcing %s to join %s",
		/* LANG_FORCE_JOINED */
		" You were forced into %s by %s for reason [%s].\n"
