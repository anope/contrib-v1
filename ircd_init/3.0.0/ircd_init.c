 ...
 *    is loaded or whenever services get restarted. (settable on services.conf)
 ...
 * v3.0.0 : Added conf directives for ChanServ whether to join or not
 *          all registered channel(s) on startup/init (requested by ziggyjr )
 * -----------------------------------------------------------------------
...
#define VERSION "$Id: ircd_init.c v3.0.0 20-5-2008 n00bie $"
void load_chanserv();
void load_services();
void load_bots();
int IRCDINITChanServ;
...
int AnopeInit(int argc, char **argv)
{
	...
	if (status != MOD_ERR_OK) {
		alog("ircd_init: Cannot hook to EVENT_RELOAD.");
		return MOD_STOP;
	}
	if (LogChannel) {
		logchan = 1;
...
{
    Directive d[] = {
        {"IRCDINITChanServ", {{PARAM_SET, PARAM_RELOAD, &IRCDINITChanServ}}},
    };
