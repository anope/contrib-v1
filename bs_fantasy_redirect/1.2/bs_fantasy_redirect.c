 ...
 * Date: 15/1/2012   Last Updated: 28/10/2012
 * Version : 1.2
 * -----------------------------------------------------------------------------
 ...
 *   1.1 - Cleaned up some messy code. do_redirect() only uses one function instead of two now.
 *   1.2 - Added support for redirect reason. You can now declare a reason to be shown to the user that gets redirected.
           if no reason is defined, a default reason will be shown to the user.
 *
 * -----------------------------------------------------------------------------
 **/
...
#define LANG_FORCE_REDIRECTCUSTOM                       6

/* Functions */
int do_fantasy(int ac, char **av);
void do_redirect(User *u, Channel *c, char *nick, char *c2, char *reason);
int valid_ircd(void);
	...
	if (stricmp(av[0], "redirect") == 0) {
		...
		if (ac == 3)
			...
			if (is_services_oper(u)) {
				...
				if (!arg1 || !arg2) moduleNoticeLang(ci->bi->nick, u, LANG_REDIRECT_SYNTAX, BSFantasyCharacter);
				else {
                                 if (!arg3) do_redirect(u, c, arg1, arg2, NULL);
                                 else do_redirect(u, c, arg1, arg2, arg3);
                                 }
				if (arg1) free(arg1);
				if (arg2) free(arg2);
                            if (arg3) free(arg3);
...
void do_redirect(User *u, Channel *c, char *nick, char *c2, char *reason) {
	...
	wallops(c->ci->bi->nick, "%s redirected %s from channel %s to channel %s .", u->nick, nick, c->name , c2);
       if(reason==NULL) moduleNoticeLang(s_ChanServ, target, LANG_FORCE_REDIRECT, c->name, c2,u->nick);
       else moduleNoticeLang(s_ChanServ, target, LANG_FORCE_REDIRECTCUSTOM, c->name, c2,u->nick,reason);
	/* Force them out */
