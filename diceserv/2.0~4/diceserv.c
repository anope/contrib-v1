 ...
 * version.
 * ----------------------------------------------------------------------------
 * Requires: Anope-1.9.0 or Anope-1.9.0-p1 (Will not work with earlier or any
 *                                          other 1.9.x)
 * ----------------------------------------------------------------------------
 * Description:
 *
...
static std::vector<std::string> ignored_channels; /* Have to do this since the Channel struct doesn't inherit the Extensible class like NickCore, ChannelInfo, and User do */
 ...
 */
static int find_chan_ignore(const std::string &chan)
{
	if (ignored_channels.empty())
		return -1;

	for (unsigned i = 0, len = ignored_channels.size(); i < len; ++i)
		if (!stricmp(chan.c_str(), ignored_channels[i].c_str()))
			return i;
 ...
 */
static void del_chan_ignore(const std::string &chan)
{
 ...
 */
static void add_chan_ignore(const std::string &chan)
{
 ...
 */
inline static unsigned is_function(const std::string &str, unsigned pos = 0)
{
	/* We only need a 5 character substring as that's the largest substring we will be looking at */
	const char *func = str.substr(pos, 5).c_str();
 ...
 */
inline static unsigned function_argument_count(const std::string &str)
{
	const char *func = str.c_str();
	if (!stricmp(func, "max") || !stricmp(func, "min") || !stricmp(func, "rand"))
		return 2;
	return 1;
}
 ...
 */
inline static unsigned is_constant(const std::string &str, unsigned pos = 0)
{
	/* We only need a 2 character substring as that's the largest substring we will be looking at */
	const char *constant = str.substr(pos, 2).c_str();
	/* pi */
 ...
 */
inline static int would_pop(const std::string &adding, const std::string &topstack)
{
	if (adding.empty())
...
struct Infix
{
	...
	Infix(const std::string &newStr, std::vector<unsigned> newPositions)
	{
		this->str = newStr;
		this->positions = newPositions;
	}
	Infix(const std::string &newStr, unsigned newPositions[], unsigned num)
	{
		this->str = newStr;
 ...
 */
static Infix fix_infix(const std::string &infix)
{
	if (infix == "%")
	{
...
/** Version of PostfixValue for std::string */
class PostfixValueString : public PostfixValueBase
{
	std::string *val;
 public:
	PostfixValueString() : PostfixValueBase(), val(NULL) { }
	PostfixValueString(std::string *Val) : PostfixValueBase(), val(Val) { }
...
/** Adds an std::string value to the Postfix vector.
 * @param postfix The Postfix vector to add to
 * @param str The std::string value we are adding
 */
static void add_to_postfix(Postfix &postfix, const std::string &str)
{
...
static Postfix infix_to_postfix(const Infix &infix)
{
	...
	while (tokens.GetToken(token))
	{
		if (token[0] == '_')
		{
			...
			{
				if (!stricmp(token.substr(1).c_str(), "e"))
					number = -exp(1.0);
				else if (!stricmp(token.substr(1).c_str(), "pi"))
					number = -atan(1.0) * 4;
			}
			else
		...
		{
			double number = 0.0;
			if (!stricmp(token.c_str(), "e"))
				number = exp(1.0);
			else if (!stricmp(token.c_str(), "pi"))
				number = atan(1.0) * 4;
			add_to_postfix(postfix, number);
			prev_was_number = true;
		}
	...
	for (unsigned x = 0, len = postfix.size(); x < len; ++x)
	{
		if (postfix[x].second == POSTFIX_VALUE_STRING)
		{
			...
			if (is_function(token))
			{
				...
				num_stack.pop();
				if (!stricmp(token.c_str(), "abs"))
					val = fabs(val1);
				else if (!stricmp(token.c_str(), "acos"))
				{
					if (fabs(val1) > 1)
					{
				...
				else if (!stricmp(token.c_str(), "acosh"))
				{
					if (val1 < 1)
					{
						DiceErrCode = DICE_ERR_UNDEFINED;
				...
				else if (!stricmp(token.c_str(), "asin"))
				{
					if (fabs(val1) > 1)
					{
						DiceErrCode = DICE_ERR_UNDEFINED;
				...
				else if (!stricmp(token.c_str(), "asinh"))
					val = asinh(val1);
				else if (!stricmp(token.c_str(), "atan"))
					val = atan(val1);
				else if (!stricmp(token.c_str(), "atanh"))
				{
					if (fabs(val1) >= 1)
					{
				...
				else if (!stricmp(token.c_str(), "cbrt"))
					val = cbrt(val1);
				else if (!stricmp(token.c_str(), "ceil"))
					val = ceil(val1);
				else if (!stricmp(token.c_str(), "cos"))
					val = cos(val1);
				else if (!stricmp(token.c_str(), "cosh"))
					val = cosh(val1);
				else if (!stricmp(token.c_str(), "deg"))
					val = val1 * 45.0 / atan(1.0);
				else if (!stricmp(token.c_str(), "exp"))
					val = exp(val1);
				else if (!stricmp(token.c_str(), "fac"))
				{
				...
				else if (!stricmp(token.c_str(), "floor"))
					val = floor(val1);
				else if (!stricmp(token.c_str(), "log"))
				{
					if (val1 <= 0)
					{
				...
				else if (!stricmp(token.c_str(), "log10"))
				{
					if (val1 <= 0)
					{
						DiceErrCode = DICE_ERR_DIV0;
				...
				else if (!stricmp(token.c_str(), "max"))
				{
					double val2 = val1;
					val1 = num_stack.top();
					num_stack.pop();
				...
				else if (!stricmp(token.c_str(), "min"))
				{
					double val2 = val1;
					val1 = num_stack.top();
					num_stack.pop();
				...
				else if (!stricmp(token.c_str(), "rad"))
					val = val1 * atan(1.0) / 45.0;
				else if (!stricmp(token.c_str(), "rand"))
				{
					double val2 = val1;
					val1 = num_stack.top();
					num_stack.pop();
				...
				else if (!stricmp(token.c_str(), "round"))
					val = my_round(val1);
				else if (!stricmp(token.c_str(), "sin"))
					val = sin(val1);
				else if (!stricmp(token.c_str(), "sinh"))
					val = sinh(val1);
				else if (!stricmp(token.c_str(), "sqrt"))
				{
					if (val1 < 0)
					{
						DiceErrCode = DICE_ERR_UNDEFINED;
				...
				else if (!stricmp(token.c_str(), "tan"))
				{
				...
				else if (!stricmp(token.c_str(), "tanh"))
					val = tanh(val1);
				else if (!stricmp(token.c_str(), "trunc"))
					val = trunc(val1);
 ...
 */
static bool ErrorHandler(User *u, const std::string &dice, unsigned ErrorPos)
{
	bool WasError = true;
	switch (DiceErrCode)
	{
		case DICE_ERR_PARSE:
		{
			...
			notice_user(s_DiceServ, u, " %s", dice.c_str());
			std::string spaces(ErrorPos > dice.size() ? dice.size() : ErrorPos, ' ');
			notice_user(s_DiceServ, u, "(%s^)", spaces.c_str());
			me->NoticeLang(s_DiceServ, u, DICE_ERRSTR);
			notice_user(s_DiceServ, u, "%s", DiceErrStr.c_str());
			break;
		}
		case DICE_ERR_DIV0:
 ...
 */
static void diceserv_roller(User *u, const std::string &origdice, const std::string &chan, const std::string &comment, DICE_TYPES type, bool round_result = true)
{
	...
	if (u->GetExt("diceserv_ignore"))
		return;
	if (u->na && u->na->nc->GetExt("diceserv_ignore"))
		return;
	...
	std::string origdicecpy = origdice, chancpy = chan, commentcpy = comment;
	...
	std::string commentstr; /* Full comment, constructed from given arguments */
	...
	if (tilde != std::string::npos)
	{
		...
		std::string times = origdicecpy.substr(0, tilde);
	...
	else
	{
		...
		if (type == DICE_TYPE_EARTHDAWN)
		{
			...
			step = num;
			if (!dice.empty())
				origdicecpy = std::string("(") + EarthdawnStepTable[step] + ")" + dice;
			else
...
{
	me->NoticeLang(s_DiceServ, u, DICE_HELP_ROLL, s_DiceServ, s_DiceServ, s_DiceServ, s_DiceServ, s_DiceServ, s_DiceServ);
	return MOD_CONT;
}
...
{
	char *text = moduleGetLastBuffer(), *origdice = myStrGetToken(text, ' ', 0), *chan = myStrGetToken(text, ' ', 1), *comment = myStrGetTokenRemainder(text, ' ', 2);
	SourceIsBot = false;
	diceserv_roller(u, origdice ? origdice : "", chan ? chan : "", comment ? comment : "", DICE_TYPE_ROLL);
	if (origdice)
		delete [] origdice;
	if (chan)
		delete [] chan;
...
static int do_calc(User *u)
{
	char *text = moduleGetLastBuffer(), *origdice = myStrGetToken(text, ' ', 0), *chan = myStrGetToken(text, ' ', 1), *comment = myStrGetTokenRemainder(text, ' ', 2);
	SourceIsBot = false;
	diceserv_roller(u, origdice ? origdice : "", chan ? chan : "", comment ? comment : "", DICE_TYPE_CALC, false);
	if (origdice)
		delete [] origdice;
	if (chan)
		delete [] chan;
...
static int do_exroll(User *u)
{
	char *text = moduleGetLastBuffer(), *origdice = myStrGetToken(text, ' ', 0), *chan = myStrGetToken(text, ' ', 1), *comment = myStrGetTokenRemainder(text, ' ', 2);
	SourceIsBot = false;
	diceserv_roller(u, origdice ? origdice : "", chan ? chan : "", comment ? comment : "",
		!origdice || !strcmp(origdice, "%") || strchr(origdice, 'd') || strchr(origdice, 'D') || strcasestr(origdice, "rand(") ? DICE_TYPE_EXROLL : DICE_TYPE_ROLL);
	if (origdice)
		delete [] origdice;
	if (chan)
		delete [] chan;
...
static int do_excalc(User *u)
{
	...
	SourceIsBot = false;
	diceserv_roller(u, origdice ? origdice : "", chan ? chan : "", comment ? comment : "",
		!origdice || !strcmp(origdice, "%") || strchr(origdice, 'd') || strchr(origdice, 'D') || strcasestr(origdice, "rand(") ? DICE_TYPE_EXCALC : DICE_TYPE_CALC, false);
	if (origdice)
		delete [] origdice;
	if (chan)
		delete [] chan;
...
{
	char *text = moduleGetLastBuffer(), *chan = myStrGetToken(text, ' ', 0), *comment = myStrGetTokenRemainder(text, ' ', 1);
	SourceIsBot = false;
	diceserv_roller(u, "", chan ? chan : "", comment ? comment : "", DICE_TYPE_DND3E);
	if (chan)
		delete [] chan;
	if (comment)
		delete [] comment;
...
static int do_earthdawn(User *u)
{
	char *text = moduleGetLastBuffer(), *origdice = myStrGetToken(text, ' ', 0), *chan = myStrGetToken(text, ' ', 1), *comment = myStrGetTokenRemainder(text, ' ', 2);
	SourceIsBot = false;
	diceserv_roller(u, origdice ? origdice : "", chan ? chan : "", comment ? comment : "", DICE_TYPE_EARTHDAWN);
	if (origdice)
		delete [] origdice;
	if (chan)
		delete [] chan;
...
static int do_set(User *u)
{
	...
	char *text = moduleGetLastBuffer(), *cmd = myStrGetToken(text, ' ', 0);
	/* No sub-command given, don't continue processing */
	if (!cmd)
	{
		me->NoticeLang(s_DiceServ, u, DICE_SET_SYNTAX);
		return MOD_CONT;
	}
	/* SET IGNORE sub-command */
	if (!stricmp(cmd, "IGNORE"))
	{
		char *where = myStrGetToken(text, ' ', 1), *param = myStrGetTokenRemainder(text, ' ', 2);
		/* We require a parameter, otherwise don't continue processing */
		if (!param)
		{
			me->NoticeLang(s_DiceServ, u, is_services_oper(u) ? DICE_SERVADMIN_SET_IGNORE_SYNTAX : DICE_SET_IGNORE_SYNTAX);
			if (where)
				delete [] where;
			delete [] cmd;
			return MOD_CONT;
		...
		if (*where == '#')
		{
			...
			else if (ci)
			{
				...
				if (is_services_admin(u) || (DiceServChanOpCanIgnore ? isOp : false) || ((ci->flags & CI_SECUREFOUNDER) ? is_real_founder(u, ci) : is_founder(u, ci)))
				{
					/* Either add or delete the ignore data */
					if (mode == IGNORE_ADD)
					{
						ci->Extend("diceserv_ignore", sstrdup("1"));
						if (c)
							add_chan_ignore(c->name);
					}
					else
					{
						char *diceserv_ignore;
		...
		else
		{
			...
			else if (na)
			{
				...
				if (mode == IGNORE_ADD)
				{
					na->nc->Extend("diceserv_ignore", sstrdup("1"));
					if (nu)
						nu->Extend("diceserv_ignore", sstrdup("1"));
				}
				else
				{
					char *diceserv_ignore;
	...
	{
		me->NoticeLang(s_DiceServ, u, DICE_SET_UNKNOWN_OPTION, cmd);
...
{
	char *text = moduleGetLastBuffer(), *what = myStrGetToken(text, ' ', 0);
...
{
	char *text = moduleGetLastBuffer(), *showtype = myStrGetToken(text, ' ', 0), *what = myStrGetToken(text, ' ', 1), *pattern = myStrGetToken(text, ' ', 2);
	...
	char *regtype = myStrGetToken(text, ' ', 3);
	enum
	{
		REG_SHOW_ALL,
		REG_SHOW_REG,
	...
	me->NoticeLang(s_DiceServ, u, DICE_LIST_HEADER, me->GetLangString(u, show == SHOW_IGNORED ? DICE_LIST_HEADER_IGNORED : (show == SHOW_ALLOWED ? DICE_LIST_HEADER_ALLOWED : DICE_LIST_HEADER_ALL)),
		me->GetLangString(u, !stricmp(what, "CHANNELS") ? DICE_LIST_HEADER_CHANNELS : DICE_LIST_HEADER_NICKS), pattern,
	...
	if (!stricmp(what, "CHANNELS"))
	{
		...
		if (reg == REG_SHOW_UNREG || reg == REG_SHOW_ALL)
		{
			...
			for (; c; c = nextchan())
			{
				...
				{
					if (reg == REG_SHOW_ALL)
						notice_user(s_DiceServ, u, "   %-20s  %-5s  %s", c->name, me->GetLangString(u, DICE_LIST_UNREG), me->GetLangString(u, diceserv_ignore == -1 ? DICE_IGNORED : DICE_ALLOWED));
					else
						notice_user(s_DiceServ, u, "   %-20s  %s", c->name, me->GetLangString(u, diceserv_ignore == -1 ? DICE_IGNORED : DICE_ALLOWED));
				}
			}
		}
		...
		if (reg == REG_SHOW_REG || reg == REG_SHOW_ALL)
			for (; i < 256; ++i)
			{
				...
				for (; ci; ci = ci->next)
				{
					...
					{
						if (reg == REG_SHOW_ALL)
							notice_user(s_DiceServ, u, "   %-20s  %-5s  %s", ci->name, me->GetLangString(u, DICE_LIST_REG), me->GetLangString(u, diceserv_ignore ? DICE_IGNORED : DICE_ALLOWED));
						else
							notice_user(s_DiceServ, u, "   %-20s  %s", ci->name, me->GetLangString(u, diceserv_ignore ? DICE_IGNORED : DICE_ALLOWED));
					}
				}
			}
	}
	...
	else
	{
		...
		if (reg == REG_SHOW_UNREG || reg == REG_SHOW_ALL)
		{
			...
			for (; nu; nu = nextuser())
			{
				...
				{
					if (reg == REG_SHOW_ALL)
						notice_user(s_DiceServ, u, "   %-20s  %-5s  %s", nu->nick, me->GetLangString(u, DICE_LIST_UNREG), me->GetLangString(u, diceserv_ignore ? DICE_IGNORED : DICE_ALLOWED));
					else
						notice_user(s_DiceServ, u, "   %-20s  %s", nu->nick, me->GetLangString(u, diceserv_ignore ? DICE_IGNORED : DICE_ALLOWED));
				}
			}
		}
		...
		if (reg == REG_SHOW_REG || reg == REG_SHOW_ALL)
			for (; i < 1024; i++)
			{
				...
				for (; na; na = na->next)
				{
					...
					{
						if (reg == REG_SHOW_ALL)
							notice_user(s_DiceServ, u, "   %-20s  %-5s  %s", na->nick, me->GetLangString(u, DICE_LIST_REG), me->GetLangString(u, diceserv_ignore ? DICE_IGNORED : DICE_ALLOWED));
						else
							notice_user(s_DiceServ, u, "   %-20s  %s", na->nick, me->GetLangString(u, diceserv_ignore ? DICE_IGNORED : DICE_ALLOWED));
					}
				}
			}
	}
...
static int do_cs_info(User *u)
{
	...
	if (chan)
	{
		...
		if (ci && (((ci->flags & CI_SECUREFOUNDER) ? is_real_founder(u, ci) : is_founder(u, ci)) || is_services_oper(u)))
...
static void do_load_db()
{
	...
	{
		char *type = myStrGetToken(readbuf, ' ', 0), *name = myStrGetToken(readbuf, ' ', 1);
		if (type)
		{
			if (name)
			{
...
static int do_nick_evt(int argc, char **argv)
{
	...
	if (u->na && u->na->nc->GetExt("diceserv_ignore"))
		u->Extend("diceserv_ignore", sstrdup("1"));

	return MOD_CONT;
}

/** Process a user logging off the network.
 * @param argc The number of arguments (should be 1, technically)
 * @param argv The arguments, the only one of which should be the nick of the user
 *
 * This safely removes the DiceServ ignore flag from the user's Extensible class.
 */
static int do_logoff_evt(int argc, char **argv)
{
	User *u = finduser(argv[0]);
	char *diceserv_ignore;
...
static int do_nick_register_evt(int argc, char **argv)
{
	...
	if (u->GetExt("diceserv_ignore"))
		u->na->nc->Extend("diceserv_ignore", sstrdup("1"));
...
static int do_chan_register_evt(int argc, char **argv)
{
	...
	if (index != -1)
		c->ci->Extend("diceserv_ignore", sstrdup("1"));
...
static int do_fantasy(int argc, char **argv)
{
	...
	ChannelInfo *ci = cs_findchan(argv[2]);
	RollSource = ci && ci->bi ? ci->bi->nick : s_DiceServ;
	SourceIsBot = true;
	diceserv_roller(u, origdice ? origdice : "", argv[2], comment ? comment : "", type, round_result);
...
static int diceserv_create(int argc, char **argv)
{
	DiceServ = findbot(s_DiceServ);
	if (!DiceServ)
	{
		DiceServ = new BotInfo(s_DiceServ, ServiceUser, ServiceHost, "Dice Roll Server");
		ircdproto->SendClientIntroduction(s_DiceServ, ServiceUser, ServiceHost, "Dice Roll Server", ircd->pseudoclient_mode, DiceServ->uid.c_str());
	}
...
class DiceServCore : public Module
{
 public:
	DiceServCore(const std::string &modname, const std::string &creator) : Module(modname, creator)
	{
		...
		this->AddCommand(DiceServ_cmdTable, c, MOD_UNIQUE);
		...
		this->AddCommand(CHANSERV, c, MOD_TAIL);
		...
		hook = createEventHook(EVENT_USER_LOGOFF, do_logoff_evt);
		this->AddEventHook(hook);
		...
		if (servsock == -1)
		{
			hook = createEventHook(EVENT_SERVER_CONNECT, diceserv_create);
			this->AddEventHook(hook);
		}
		else
			diceserv_create(0, NULL);
		...
		for (int i = 0; i < MAX_CMD_HASH; ++i)
			for (CommandHash *current = DiceServ_cmdTable[i]; current; current = current->next)
				for (Command *c = current->c; c; c = c->next)
					this->DelCommand(DiceServ_cmdTable, c->name);
		if (DiceServ)
		{
...
MODULE_INIT("diceserv", DiceServCore)
