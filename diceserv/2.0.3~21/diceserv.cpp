 ...
 * Version : 2.0.3
 * Date    : (Last modified) November 20th, 2011
 * ----------------------------------------------------------------------------
 ...
 * version.
 * ----------------------------------------------------------------------------
 * Requires: Anope-1.9.3-p1 (Will not work with earlier or any other 1.9.x)
 *                          (Does not work with 1.9.3 due to it being broken)
 * ----------------------------------------------------------------------------
 * Description:
 *
...
diceserv
{
	...
	// Please note that this is for backwards compability to versions of
	// Anope 1.9.x prior to 1.9.2, as now the information is stored directly
	// in the main Anope database. If this file exists, it will be migrated,
	// and then deleted.
	//
...
static const char *const VERSION = "v2.0.3";
...
static Anope::string	s_DiceServ, /* DiceServ client name */
						DiceServDBName, /* Database file name */
...
inline static unsigned is_function(const Anope::string &str, unsigned pos = 0)
{
	...
	if (func_4.equals_ci("acos") || func_4.equals_ci("asin") || func_4.equals_ci("atan") || func_4.equals_ci("cbrt") || func_4.equals_ci("ceil") || func_4.equals_ci("cosh") || func_4.equals_ci("rand") ||
		func_4.equals_ci("sinh") || func_4.equals_ci("sqrt") || func_4.equals_ci("tanh"))
		return 4;
	/* abs, cos, deg, exp, fac, log, max, min, rad, sin, tan */
	Anope::string func_3 = func.substr(0, 3);
	if (func_3.equals_ci("abs") || func_3.equals_ci("cos") || func_3.equals_ci("deg") || func_3.equals_ci("exp") || func_3.equals_ci("fac") || func_3.equals_ci("log") || func_3.equals_ci("max") || func_3.equals_ci("min") ||
		func_3.equals_ci("rad") || func_3.equals_ci("sin") || func_3.equals_ci("tan"))
		return 3;
	/* None of the above */
	return 0;
}
...
static unsigned Dice(int num, unsigned sides)
{
	...
	if (DiceEx == 1)
	{
		buf = stringify(num) + "d" + stringify(sides) + "=(";
	...
	for (int i = 0; i < num; ++i)
	{
		...
		if (DiceEx == 1)
		{
			buf = Anope::string(i ? " " : "") + stringify(number);
			...
			if (DoEarthdawn && bonus)
			{
				...
				if (DiceExOutput[DiceExOutput.length() - 1] == '[')
				{
					if (buf[0] == ' ')
	...
	if (DiceEx == 1)
	{
		...
		DiceShortExOutput += stringify(sum);
 ...
 */
static Postfix DoParse(const Anope::string &infix)
{
 ...
 */
static bool ErrorHandler(User *u, const Anope::string &dice, unsigned ErrorPos)
{
	bool WasError = true;
	switch (DiceErrCode)
	{
		case DICE_ERR_PARSE:
		{
			me->SendMessage(DiceServ, u, _("During parsing, an error was found in the following\nexpression:"));
			u->SendMessage(s_DiceServ, " %s", dice.c_str());
			Anope::string spaces(ErrorPos > dice.length() ? dice.length() : ErrorPos, ' ');
			u->SendMessage(s_DiceServ, "(%s^)", spaces.c_str());
			me->SendMessage(DiceServ, u, _("Error description is as follows:"));
			u->SendMessage(s_DiceServ, "%s", DiceErrStr.c_str());
			break;
		}
		case DICE_ERR_DIV0:
			me->SendMessage(DiceServ, u, _("Division by 0 in following expression:"));
			u->SendMessage(s_DiceServ, " %s", dice.c_str());
			break;
		case DICE_ERR_UNDEFINED:
			me->SendMessage(DiceServ, u, _("Undefined result in following expression:"));
			u->SendMessage(s_DiceServ, " %s", dice.c_str());
			break;
		case DICE_ERR_UNACCEPT_DICE:
			if (DiceErrNum <= 0)
				me->SendMessage(DiceServ, u, _("The number of dice that you entered (%d) was under\n1. Please enter a number between 1 and %d."), DiceErrNum, DICE_MAX_DICE);
			else
				me->SendMessage(DiceServ, u, _("The number of dice that you entered (%d) was over the\nlimit of %d. Please enter a lower number of dice."), DiceErrNum, DICE_MAX_DICE);
			break;
		case DICE_ERR_UNACCEPT_SIDES:
			if (DiceErrNum <= 0)
				me->SendMessage(DiceServ, u, _("The number of sides that you entered (%d) was under\n1. Please enter a number between 1 and %d."), DiceErrNum, DICE_MAX_SIDES);
			else
				me->SendMessage(DiceServ, u, _("The number of sides that you entered (%d) was over the\nlimit of %d. Please enter a lower number of sides."), DiceErrNum, DICE_MAX_SIDES);
			break;
		case DICE_ERR_OVERUNDERFLOW:
			me->SendMessage(DiceServ, u, _("Dice results in following expression resulted in either\noverflow or underflow:"));
			u->SendMessage(s_DiceServ, " %s", dice.c_str());
			break;
		case DICE_ERR_STACK:
			me->SendMessage(DiceServ, u, _("The following roll expression could not be properly\nevaluated, please try again or let an administrator know."));
			u->SendMessage(s_DiceServ, " %s", dice.c_str());
			me->SendMessage(DiceServ, u, _("Error description is as follows:"));
			u->SendMessage(s_DiceServ, "%s", DiceErrStr.c_str());
			break;
...
static void DnDRollCorrect(double &v)
{
	...
	bool inex = false;
	Anope::string tmpstr;
	for (unsigned x = 0, y = 0, len = DiceExOutput.length(); x < len; ++x)
	{
 ...
 */
static Anope::string syntax_string(DICE_TYPES type)
{
	switch (type)
	{
		case DICE_TYPE_CALC:
			return _("Syntax: CALC dice [[channel] comment]");
		case DICE_TYPE_EXROLL:
			return _("Syntax: EXROLL dice [[channel] comment]");
		case DICE_TYPE_EXCALC:
			return _("Syntax: EXCALC dice [[channel] comment]");
		case DICE_TYPE_DND3E:
			return _("Syntax: DND3ECHAR [[channel] comment]");
		case DICE_TYPE_EARTHDAWN:
			return _("Syntax: EARTHDAWN step [[channel] comment]");
		default:
			return _("Syntax: ROLL dice [[channel] comment]");
	}
}
 ...
 */
static void diceserv_roller(User *u, const Anope::string &origdice, const Anope::string &chan, const Anope::string &comment, DICE_TYPES type, bool round_result = true)
{
	...
	if (u->GetExt("diceserv_ignore"))
		return;
	if (u->Account() && u->Account()->GetExt("diceserv_ignore"))
		return;
	...
	{
		if (!SourceIsBot)
			me->SendMessage(DiceServ, u, syntax_string(type).c_str());
		return;
	}
	/* Make copies of the passed arguments */
	Anope::string origdicecpy = origdice, chancpy = chan, commentcpy = comment;
	...
	if (inchan)
	{
		...
		if (c)
		{
			if (!c->FindUser(u))
			{
				if (!SourceIsBot)
				{
					u->SendMessage(DiceServ, CHAN_X_INVALID, chancpy.c_str());
					me->SendMessage(DiceServ, u, syntax_string(type).c_str());
				}
				return;
			}
			ChannelInfo *ci = cs_findchan(chancpy);
			bool found_ci_ignore = ci ? ci->GetExt("dicesev_ignore") : false;
			if (c->GetExt("diceserv_ignore") || found_ci_ignore || c->FindUser(u)->Status->HasFlag(CMODE_MODERATED))
			{
				inchan = false;
				chancpy.clear();
			}
	...
	Anope::string commentstr; /* Full comment, constructed from given arguments */
	...
	BotInfo *bi = findbot(inchan && SourceIsBot ? RollSource : s_DiceServ);
	MaxMessageLength -= bi->nick.length();
	MaxMessageLength -= bi->GetIdent().length();
	MaxMessageLength -= bi->host.length();
	...
	MaxMessageLength -= inchan ? chancpy.length() : u->nick.length(); /* inchan uses channel's name, otherwise uses user's nick */
	MaxMessageLength -= 7; /* For the < > [ ] :, and the space before the [ and after the : */
	MaxMessageLength -= origdicecpy.length(); /* The dice expression itself */
	...
	if (inchan)
	{
		...
		MaxMessageLength -= u->nick.length(); /* nick of user who used the command */
	}
	if (!commentstr.empty())
		MaxMessageLength -= commentstr.length() + 1; /* Comment plus space */
	/* Check for overflow prior to adding in dice results */
	if (MaxMessageLength <= 0)
	{
		me->SendMessage(DiceServ, u, _("Dice result buffer has an overflow. This could be due to\n"
	...
	Anope::string	sep = " ", /* Separator between extended output sets, might change in the future if we need a different seperator */
	...
	if (tilde != Anope::string::npos)
	{
		...
		{
			me->SendMessage(DiceServ, u, _("step for an Earthdawn roll must be a number."));
			return;
		}
		...
		Anope::string times = origdicecpy.substr(0, tilde);
		...
		if (DiceErrCode == DICE_ERR_NONE)
		{
			...
			if (n > DICE_MAX_TIMES)
			{
				me->SendMessage(DiceServ, u, _("The number of times that you entered (%d) was over the\nlimit of %d. Please enter a lower number of times."), n, DICE_MAX_TIMES);
				return;
			}
			else if (n <= 0)
			{
				me->SendMessage(DiceServ, u, _("The number of times that you entered (%d) was under\n1. Please enter a number between 1 and %d."), n, DICE_MAX_TIMES);
				return;
			}
		}
		/* Check for errors */
	...
	else
	{
		...
		if (type == DICE_TYPE_EARTHDAWN)
		{
			...
			if (num < 1 || num > 100)
			{
				me->SendMessage(DiceServ, u, _("The step you entered (%d) was out of range, it must be\nbetween 1 and 100."), num);
				return;
			}
			step = num;
			if (!dice.empty())
				origdicecpy = Anope::string("(") + EarthdawnStepTable[step] + ")" + dice;
			else
		...
		do
		{
			...
			for (; n > 0; --n, DnDPos = 0)
			{
				...
				if (!error)
				{
					...
					Anope::string tmp = dtoa(round_result ? static_cast<int>(my_round(v)) : v);
					buf += tmp;
				}
			...
			{
				me->SendMessage(DiceServ, u, DnDmodadd() <= 0 ? _("D&D 3e Character roll resulted in a character that had their\n"
	...
	if (MaxMessageLength - static_cast<int>(buf.length()) >= 0)
	{
		if (DiceEx == 1 && MaxMessageLength - static_cast<int>(buf.length()) - static_cast<int>(extmp.length()) < 0)
			DiceEx = 2;
		if (DiceEx == 2 && MaxMessageLength - static_cast<int>(buf.length()) - static_cast<int>(shortextmp.length()) < 0)
			DiceEx = 3;
		if (DiceEx == 3)
			me->SendMessage(DiceServ, u, _("Too many results for %s to display, omitting results."), type == DICE_TYPE_EXROLL ? "EXROLL" : (type == DICE_TYPE_EXCALC ? "EXCALC" : "DND3ECHAR"));
	}
	else
	{
		me->SendMessage(DiceServ, u, _("Dice result buffer has an overflow. This could be due to\n"
	...
	if (DiceErrCode == DICE_ERR_NONE && !error)
	{
		...
		Anope::string exbuffer = DiceEx == 1 ? extmp : (DiceEx == 2 ? shortextmp : "");
		...
		if (inchan)
		{
			...
			{
				if (SourceIsBot)
					ircdproto->SendPrivmsg(findbot(RollSource), chancpy.c_str(), "<Earthdawn roll for %s [Step %u (%s)]: %s%s>%s%s", u->nick.c_str(), step, origdicecpy.c_str(), exbuffer.c_str(), buf.c_str(),
						!commentstr.empty() ? " " : "", !commentstr.empty() ? commentstr.c_str() : "");
				else
					ircdproto->SendNotice(DiceServ, chancpy.c_str(), "<Earthdawn roll for %s [Step %u (%s)]: %s%s>%s%s", u->nick.c_str(), step, origdicecpy.c_str(), exbuffer.c_str(), buf.c_str(),
						!commentstr.empty() ? " " : "", !commentstr.empty() ? commentstr.c_str() : "");
			}
			else
			{
				if (SourceIsBot)
					ircdproto->SendPrivmsg(findbot(RollSource), chancpy.c_str(), "<%s for %s [%s]: %s%s>%s%s",
						type == DICE_TYPE_ROLL ? "Roll" : (type == DICE_TYPE_CALC ? "Calc" : (type == DICE_TYPE_EXROLL ? "Exroll" : (type == DICE_TYPE_EXCALC ? "Excalc" : "DnD3e Character Roll"))), u->nick.c_str(),
		...
		{
			if (type == DICE_TYPE_DND3E)
				me->SendMessage(DiceServ, u, _("<DnD3e Character roll [%s]: %s%s>%s%s"), origdicecpy.c_str(), exbuffer.c_str(), buf.c_str(), !commentstr.empty() ? " " : "", !commentstr.empty() ? commentstr.c_str() : "");
			else if (type == DICE_TYPE_EARTHDAWN)
				me->SendMessage(DiceServ, u, _("<Earthdawn roll [Step %u (%s)]: %s%s>%s%s"), step, origdicecpy.c_str(), exbuffer.c_str(), buf.c_str(), !commentstr.empty() ? " " : "",
					!commentstr.empty() ? commentstr.c_str() : "");
			else
				me->SendMessage(DiceServ, u,
					DiceEx == 1 || DiceEx == 2 ? (type == DICE_TYPE_EXCALC ? _("<Excalc [%s]: %s%s>%s%s") : _("<Exroll [%s]: %s%s>%s%s")) : (type == DICE_TYPE_CALC ? _("<Calc [%s]: %s%s>%s%s") : _("<Roll [%s]: %s%s>%s%s")),
					origdice.c_str(), exbuffer.c_str(), buf.c_str(), !commentstr.empty() ? " " : "", !commentstr.empty() ? commentstr.c_str() : "");
		}
	}
}
...
static void do_load_config()
{
	...
	s_DiceServ = config.ReadValue("diceserv", "name", "DiceServ", 0);
...
class CommandDiceServHelp : public Command
{
 public:
	...
	CommandReturn Execute(User *u, const std::vector<Anope::string> &params)
	{
		...
		if (cmd.equals_ci("FUNCTIONS"))
			me->SendMessage(DiceServ, u, _("%s recognizes the following functions:\n"
				" \n"
				...
				"tangent) return their values in radians."), s_DiceServ.c_str());
		else
			mod_help_cmd(DiceServ, u, cmd);
	...
	{
		me->SendMessage(DiceServ, u, _("%s allows you to roll any number of dice with any\n"
			...
			" "), s_DiceServ.c_str(), s_DiceServ.c_str(), s_DiceServ.c_str());
		for (CommandMap::const_iterator it = DiceServ->Commands.begin(), it_end = DiceServ->Commands.end(); it != it_end; ++it)
			if (!Config->HidePrivilegedCommands || it->second->permission.empty() || (u->Account() && u->Account()->HasCommand(it->second->permission)))
				it->second->OnServHelp(u);
		me->SendMessage(DiceServ, u, _(" \n"
			...
			"to #DiceServ on jenna.cyberbotx.com."), s_DiceServ.c_str(), !Config->s_BotServ.empty() ? Config->s_BotServ.c_str() : "BotServ", !Config->s_BotServ.empty() ? Config->s_BotServ.c_str() : "BotServ",
			s_DiceServ.c_str(), s_DiceServ.c_str());
	}
};
...
class CommandDiceServRoll : public Command
{
 public:
	...
	CommandReturn Execute(User *u, const std::vector<Anope::string> &params)
	{
		Anope::string origdice = params[0], chan = params.size() > 1 ? params[1] : "", comment = params.size() > 2 ? params[2] : "";
		SourceIsBot = false;
	...
	{
		if (subcommand.empty())
			me->SendMessage(DiceServ, u, _("Syntax: ROLL dice [[channel] comment]\n"
				" \n"
				...
				"    /msg %s ROLL 3~3d6"), s_DiceServ.c_str(), s_DiceServ.c_str(), s_DiceServ.c_str(), s_DiceServ.c_str(), s_DiceServ.c_str());
		else if (subcommand.equals_ci("EXPRESSIONS"))
			me->SendMessage(DiceServ, u, _("Dice expression syntax\n"
				" \n"
				...
				"problems even though it is also used for subtraction."), s_DiceServ.c_str());
		return true;
	}
	...
	{
		me->SendMessage(DiceServ, u, _("    ROLL           Rolls dice (or performs math too)"));
	}
};
...
class CommandDiceServCalc : public Command
{
 public:
	...
	{
		me->SendMessage(DiceServ, u, _("Syntax: CALC dice [[channel] comment]\n"
			" \n"
			...
			"displayed as is."), s_DiceServ.c_str());
		return true;
	}
	...
	void OnServHelp(User *u)
	{
		me->SendMessage(DiceServ, u, _("    CALC           ROLL without rounding, for calculations"));
	}
};
...
class CommandDiceServExRoll : public Command
{
 public:
	...
	CommandReturn Execute(User *u, const std::vector<Anope::string> &params)
	{
		Anope::string origdice = params[0], chan = params.size() > 1 ? params[1] : "", comment = params.size() > 2 ? params[2] : "";
		SourceIsBot = false;
		diceserv_roller(u, origdice, chan, comment, origdice.empty() || origdice == "%" || origdice.find_ci('d') != Anope::string::npos || origdice.find_ci("rand(") != Anope::string::npos ? DICE_TYPE_EXROLL : DICE_TYPE_ROLL);
		return MOD_CONT;
	}

	bool OnHelp(User *u, const Anope::string &)
	{
		me->SendMessage(DiceServ, u, _("Syntax: EXROLL dice [[channel] comment]\n"
			" \n"
	...
	void OnServHelp(User *u)
	{
		me->SendMessage(DiceServ, u, _("    EXROLL         Rolls dice and shows each dice roll"));
	}
};
...
class CommandDiceServExCalc : public Command
{
 public:
	...
	CommandReturn Execute(User *u, const std::vector<Anope::string> &params)
	{
		Anope::string origdice = params[0], chan = params.size() > 1 ? params[1] : "", comment = params.size() > 2 ? params[2] : "";
		SourceIsBot = false;
		diceserv_roller(u, origdice, chan, comment, origdice.empty() || origdice == "%" || origdice.find_ci('d') != Anope::string::npos || origdice.find_ci("rand(") != Anope::string::npos ? DICE_TYPE_EXCALC : DICE_TYPE_CALC,
			false);
		return MOD_CONT;
	}
	...
	bool OnHelp(User *u, const Anope::string &)
	{
		me->SendMessage(DiceServ, u, _("Syntax: EXCALC dice [[channel] comment]\n"
			" \n"
	...
	void OnServHelp(User *u)
	{
		me->SendMessage(DiceServ, u, _("    EXCALC         EXROLL without rounding, for calculations"));
	}
};
...
class CommandDiceServDnD3eChar : public Command
{
 public:
	...
	CommandReturn Execute(User *u, const std::vector<Anope::string> &params)
	{
		Anope::string chan = params.size() ? params[0] : "", comment = params.size() > 1 ? params[1] : "";
		SourceIsBot = false;
	...
	bool OnHelp(User *u, const Anope::string &)
	{
		me->SendMessage(DiceServ, u, _("Syntax: DND3ECHAR [[channel] comment]\n"
			" \n"
			...
			"  /msg %s DND3ECHAR\n"
			"    {4d6=(3 5 5 6)}=16\n"
			"  (The above is basically 19 minus the lowest of 3)"), s_DiceServ.c_str(), s_DiceServ.c_str());
		return true;
	}

	void OnServHelp(User *u)
	{
		me->SendMessage(DiceServ, u, _("    DND3ECHAR      Rolls dice for DnD3e character creation"));
	}
};
...
class CommandDiceServEarthdawn : public Command
{
 public:
	...
	bool OnHelp(User *u, const Anope::string &)
	{
		me->SendMessage(DiceServ, u, _("Syntax: EARTHDAWN step[+karma] [[channel] comment]\n"
			" \n"
			...
			"    Same as /msg %s EXROLL (4d20+6d10+4d8)+1d6"), s_DiceServ.c_str(), s_DiceServ.c_str(), s_DiceServ.c_str(), s_DiceServ.c_str(), s_DiceServ.c_str());
		return true;
	}

	void OnSyntaxError(User *u, const Anope::string &)
	{
		me->SendMessage(DiceServ, u, _("Syntax: EARTHDAWN step [[channel] comment]"));
	}

	void OnServHelp(User *u)
	{
		me->SendMessage(DiceServ, u, _("    EARTHDAWN      Rolls dice for Earthdawn"));
	}
};
...
{
 private:
	CommandReturn DoIgnore(User *u, const std::vector<Anope::string> &params)
	{
		Anope::string where = params[1], param = params[2];
		...
		bool is_servoper = u->Account() && u->Account()->HasCommand("diceserv/set");
		...
		if (where[0] == '#')
		{
			...
			if (!c && (!ci || (ci && (ci->HasFlag(CI_FORBIDDEN) || ci->HasFlag(CI_SUSPENDED)))))
				u->SendMessage(DiceServ, CHAN_X_INVALID, where.c_str());
			...
			else if (ci)
			{
				...
				if (is_servoper || (DiceServChanOpCanIgnore ? isOp : false) || (ci->HasFlag(CI_SECUREFOUNDER) ? IsFounder(u, ci) : check_access(u, ci, CA_FOUNDER)))
				{
					/* Either add or delete the ignore data */
					if (mode == IGNORE_ADD)
					{
						ci->Extend("diceserv_ignore", new ExtensibleItemRegular<bool>(true));
						if (c)
							c->Extend("diceserv_ignore", new ExtensibleItemRegular<bool>(true));
					}
					else
					{
						ci->Shrink("diceserv_ignore");
						if (c)
							c->Shrink("diceserv_ignore");
					}
					me->SendMessage(DiceServ, u, mode == IGNORE_ADD ? _("%s will now ignore all dice rolls sent to %s.") : _("%s will now allow all dice rolls sent to %s."), s_DiceServ.c_str(),
						where.c_str());
				}
				/* Otherwise, deny access */
				else
					u->SendMessage(DiceServ, ACCESS_DENIED);
			}
			...
			else
			{
				...
				if (is_servoper || c->FindUser(u)->Status->HasFlag(CMODE_OP))
				{
					/* Either add or delete the ignore data */
					if (mode == IGNORE_ADD)
						c->Extend("diceserv_ignore", new ExtensibleItemRegular<bool>(true));
		...
		else
		{
			...
			else if (bot || (!nu && (!na || (na && (na->HasFlag(NS_FORBIDDEN) || (na->nc && na->nc->HasFlag(NI_SUSPENDED)))))))
				me->SendMessage(DiceServ, u, _("Nick %s is not a valid nick."), where.c_str());
			...
			else if (na)
			{
				...
				if (!nu)
				{
					user_map::const_iterator it = UserListByNick.begin(), it_end = UserListByNick.end();
					for (; it != it_end; ++it)
					{
						nu = it->second;
						if (nu->Account() == na->nc)
				...
				if (mode == IGNORE_ADD)
				{
					na->nc->Extend("diceserv_ignore", new ExtensibleItemRegular<bool>(true));
					if (nu)
						nu->Extend("diceserv_ignore", new ExtensibleItemRegular<bool>(true));
				}
				else
				{
				...
				me->SendMessage(DiceServ, u, mode == IGNORE_ADD ? _("%s will now ignore all dice rolls by %s.") : _("%s will now allow all dice rolls by %s."), s_DiceServ.c_str(), where.c_str());
			}
 ...
 public:
	...
	CommandReturn Execute(User *u, const std::vector<Anope::string> &params)
	{
		...
		if (readonly)
		{
			me->SendMessage(DiceServ, u, _("Sorry, dice ignore option setting is temporarily disabled."));
			return MOD_CONT;
		}
		Anope::string cmd = params[0];
		/* SET IGNORE sub-command */
		if (cmd.equals_ci("IGNORE"))
			return this->DoIgnore(u, params);
		/* Unknown SET command given */
		else
		{
			me->SendMessage(DiceServ, u, _("Unknown SET option %s."), cmd.c_str());
			u->SendMessage(DiceServ, MORE_INFO, s_DiceServ.c_str(), "SET");
		}
		return MOD_CONT;
	}
	...
	{
		if (subcommand.empty())
			me->SendMessage(DiceServ, u, _("Syntax: SET option parameters\n"
				" \n"
				...
				"option for details."), s_DiceServ.c_str(), s_DiceServ.c_str());
		else if (subcommand.equals_ci("IGNORE"))
		{
			if (u->Account() && u->Account()->HasCommand("diceserv/set"))
				me->SendMessage(DiceServ, u, _("Syntax: SET IGNORE {channel|nick} {ON|OFF}\n"
					" \n"
					...
					"as the user is online."), s_DiceServ.c_str(), s_DiceServ.c_str(), Config->s_ChanServ.c_str(), Config->s_NickServ.c_str());
			else
				me->SendMessage(DiceServ, u, _("Syntax: SET IGNORE channel {ON|OFF}\n"
					" \n"
					...
					"active."), s_DiceServ.c_str(), s_DiceServ.c_str(), Config->s_ChanServ.c_str());
		}
		return true;
	}
	...
	void OnSyntaxError(User *u, const Anope::string &subcommand)
	{
		if (subcommand.empty())
			...
			me->SendMessage(DiceServ, u, u->Account() && u->Account()->HasCommand("diceserv/set") ? _("Syntax: SET IGNORE {channel|nick} {ON|OFF}") : _("Syntax: SET IGNORE channel {ON|OFF}"));
	}

	void OnServHelp(User *u)
	{
		me->SendMessage(DiceServ, u, _("    SET            Set options for %s access"), s_DiceServ.c_str());
	}
};
 ...
 public:
	...
	CommandReturn Execute(User *u, const std::vector<Anope::string> &params)
	{
		...
		if (what[0] == '#')
		{
			...
			/* If the channel wasn't found and a ChanServ entry wasn't found (or the channel is forbidden or suspended), display an error */
			if (!c && (!ci || (ci && (ci->HasFlag(CI_FORBIDDEN) || ci->HasFlag(CI_SUSPENDED)))))
				u->SendMessage(DiceServ, CHAN_X_INVALID, what.c_str());
			/* If we found a registered channel, show the data for that */
			else if (ci)
				me->SendMessage(DiceServ, u, _("Status for registered channel %s: %s"), what.c_str(), ci->GetExt("diceserv_ignore") ? _("Ignore") : _("Allow"));
			/* No registered channel was found, but a channel was, so show the data for that */
			else
				me->SendMessage(DiceServ, u, _("Status for unregistered channel %s: %s"), what.c_str(), c->GetExt("diceserv_ignore") ? _("Ignore") : _("Allow"));
		}
		...
		else
		{
			...
			BotInfo *bot = findbot(what);
			/* If the nick wasn't found and a NickServ entry wasn't found (or the nick is forbidden or suspended), display an error */
			if (bot || (!nu && (!na || (na && (na->HasFlag(NS_FORBIDDEN) || (na->nc && na->nc->HasFlag(NI_SUSPENDED)))))))
				me->SendMessage(DiceServ, u, _("Nick %s is not a valid nick."), what.c_str());
			...
			else if (na)
			{
				...
				if (nu && nu->nick != na->nick)
					me->SendMessage(DiceServ, u, _("Status for registered nick %s: %s\n  (online as %s)"), what.c_str(), na->nc->GetExt("diceserv_ignore") ? _("Ignore") : _("Allow"), nu->nick.c_str());
				/* Otherwise, they are not online, just show that it is registered and it's data */
				else
					me->SendMessage(DiceServ, u, _("Status for registered nick %s: %s"), what.c_str(), na->nc->GetExt("diceserv_ignore") ? _("Ignore") : _("Allow"));
			}
			/* No registered nick was found, but a user was, so show the data for that */
			else
				me->SendMessage(DiceServ, u, _("Status for unregistered nick %s: %s"), what.c_str(), nu->GetExt("diceserv_ignore") ? _("Ignore") : _("Allow"));
		}
		return MOD_CONT;
	}
	...
	bool OnHelp(User *u, const Anope::string &)
	{
		me->SendMessage(DiceServ, u, _("Syntax: STATUS {channel|nick}\n"
			" \n"
	...
	void OnServHelp(User *u)
	{
		me->SendMessage(DiceServ, u, _("    STATUS         Shows allow status of channel or nick"));
	}
};
...
class CommandDiceServList : public Command
{
 public:
	...
	CommandReturn Execute(User *u, const std::vector<Anope::string> &params)
	{
		Anope::string showtype = params[0], what = params[1], pattern = params[2].c_str();
		...
		/* Show the header */
		me->SendMessage(DiceServ, u, _("List of %s %s entries matching %s%s:"), show == SHOW_IGNORED ? _("ignored") : (show == SHOW_ALLOWED ? _("allowed") : _("all")),
			what.equals_ci("CHANNELS") ? _("channels") : _("nicks"), pattern.c_str(), reg == REG_SHOW_ALL ? "" : (reg == REG_SHOW_REG ? _(" (registered only)") : _(" (unregistered only)")));
		bool display;
		int shown = 0;
		...
		if (what.equals_ci("CHANNELS"))
		{
			...
			if (reg == REG_SHOW_UNREG || reg == REG_SHOW_ALL)
			{
				for (channel_map::const_iterator it = ChannelList.begin(), it_end = ChannelList.end(); it != it_end; ++it)
				{
					...
					bool diceserv_ignore = c->GetExt("diceserv_ignore");
					...
					{
						if (reg == REG_SHOW_ALL)
							u->SendMessage(s_DiceServ, "   %-20s  %-5s  %s", c->name.c_str(), _("Unreg"), diceserv_ignore ? _("Ignored") : _("Allowed"));
						else
							u->SendMessage(s_DiceServ, "   %-20s  %s", c->name.c_str(), diceserv_ignore ? _("Ignored") : _("Allowed"));
					}
				}
			}
			...
			if (reg == REG_SHOW_REG || reg == REG_SHOW_ALL)
				for (registered_channel_map::const_iterator it = RegisteredChannelList.begin(), it_end = RegisteredChannelList.end(); it != it_end; ++it)
				{
					...
					if (ci->HasFlag(CI_FORBIDDEN) || ci->HasFlag(CI_SUSPENDED))
						continue;
					display = false;
					/* We will only show the channel if it was part of the ignore type request */
					...
					{
						if (reg == REG_SHOW_ALL)
							u->SendMessage(s_DiceServ, "   %-20s  %-5s  %s", ci->name.c_str(), _("Reg"), diceserv_ignore ? _("Ignored") : _("Allowed"));
						else
							u->SendMessage(s_DiceServ, "   %-20s  %s", ci->name.c_str(), diceserv_ignore ? _("Ignored") : _("Allowed"));
					}
				}
		}
		...
		else
		{
			...
			if (reg == REG_SHOW_UNREG || reg == REG_SHOW_ALL)
			{
				for (user_map::const_iterator it = UserListByNick.begin(), it_end = UserListByNick.end(); it != it_end; ++it)
				{
					...
					BotInfo *bot = findbot(it->second->nick);
					if (bot)
						continue;
					User *nu = it->second;
					...
					{
						if (reg == REG_SHOW_ALL)
							u->SendMessage(s_DiceServ, "   %-20s  %-5s  %s", nu->nick.c_str(), _("Unreg"), diceserv_ignore ? _("Ignored") : _("Allowed"));
						else
							u->SendMessage(s_DiceServ, "   %-20s  %s", nu->nick.c_str(), diceserv_ignore ? _("Ignored") : _("Allowed"));
					}
				}
			}
			...
			if (reg == REG_SHOW_REG || reg == REG_SHOW_ALL)
				for (nickalias_map::const_iterator it = NickAliasList.begin(), it_end = NickAliasList.end(); it != it_end; ++it)
				{
					...
					if (na->HasFlag(NS_FORBIDDEN) || (na->nc && na->nc->HasFlag(NI_SUSPENDED)))
						continue;
					display = false;
					/* We will only show the channel if it was part of the ignore type request */
					...
					{
						if (reg == REG_SHOW_ALL)
							u->SendMessage(s_DiceServ, "   %-20s  %-5s  %s", na->nick.c_str(), _("Reg"), diceserv_ignore ? _("Ignored") : _("Allowed"));
						else
							u->SendMessage(s_DiceServ, "   %-20s  %s", na->nick.c_str(), diceserv_ignore ? _("Ignored") : _("Allowed"));
					}
				}
		}
		/* Show the footer */
		me->SendMessage(DiceServ, u, _("End of list - %d/%d matches shown."), shown > 100 ? 100 : shown, shown);
		return MOD_CONT;
	}

	bool OnHelp(User *u, const Anope::string &)
	{
		me->SendMessage(DiceServ, u, _("Syntax: LIST {IGNORE|ALLOW|ALL} what pattern [{REG|UNREG}]\n"
			" \n"
	...
	void OnServHelp(User *u)
	{
		me->SendMessage(DiceServ, u, _("    LIST           Gives list of %s access"), s_DiceServ.c_str());
	}
};

class DiceServCore : public Module
{
	CommandDiceServHelp cmdDSHelp;
	CommandDiceServRoll cmdDSRoll;
	CommandDiceServCalc cmdDSCalc;
	CommandDiceServExRoll cmdDSExRoll;
	CommandDiceServExCalc cmdDSExCalc;
	CommandDiceServDnD3eChar cmdDSDnD3eChar;
	CommandDiceServEarthdawn cmdDSEarthdawn;
	CommandDiceServSet cmdDSSet;
	CommandDiceServStatus cmdDSStatus;
	CommandDiceServList cmdDSList;
 public:
	DiceServCore(const Anope::string &modname, const Anope::string &creator) : Module(modname, creator)
	{
		me = this;

		Implementation events[] = {I_OnPostCommand, I_OnPostLoadDatabases, I_OnDatabaseReadMetadata, I_OnDatabaseWriteMetadata, I_OnUserConnect, I_OnUserNickChange, I_OnUserLogoff, I_OnNickRegister, I_OnDelCore,
		...
		if (!DiceServ)
			DiceServ = new BotInfo(s_DiceServ, Config->ServiceUser, Config->ServiceHost, "Dice Roll Service");

		this->AddCommand(DiceServ, &this->cmdDSHelp);
		this->AddCommand(DiceServ, &this->cmdDSRoll);
		this->AddCommand(DiceServ, &this->cmdDSCalc);
		this->AddCommand(DiceServ, &this->cmdDSExRoll);
		this->AddCommand(DiceServ, &this->cmdDSExCalc);
		this->AddCommand(DiceServ, &this->cmdDSDnD3eChar);
		this->AddCommand(DiceServ, &this->cmdDSEarthdawn);
		this->AddCommand(DiceServ, &this->cmdDSSet);
		this->AddCommand(DiceServ, &this->cmdDSStatus);
		this->AddCommand(DiceServ, &this->cmdDSList);
	...
	{
		OnSaveDatabase();
		if (DiceServ && UplinkSock)
		{
			for (CommandMap::iterator it = DiceServ->Commands.begin(), it_end = DiceServ->Commands.end(); it != it_end; )
			{
				Command *c = it->second;
				++it;
				this->DelCommand(DiceServ, c);
			}
	...
	{
		if (command.equals_ci("INFO"))
		{
			if (service == NickServ)
			{
				Anope::string nick = params.size() ? params[0] : "";

				if (!nick.empty() && u->Account() && u->Account()->HasCommand("diceserv/info"))
				{
					NickAlias *na = findnick(nick);
					if (na)
						me->SendMessage(NickServ, u, _("  %s Status: %s"), s_DiceServ.c_str(), na->nc->GetExt("diceserv_ignore") ? _("Ignored") : _("Allowed"));
				}
			}
			else if (service == ChanServ)
			{
				...
				if (!chan.empty())
				{
					...
					if (ci && ((ci->HasFlag(CI_SECUREFOUNDER) ? IsFounder(u, ci) : check_access(u, ci, CA_FOUNDER)) || (u->Account() && u->Account()->HasCommand("diceserv/info"))))
						me->SendMessage(ChanServ, u, _("%s Status: %s"), s_DiceServ.c_str(), ci->GetExt("diceserv_ignore") ? _("Ignored") : _("Allowed"));
				}
			}
		}
	}
	...
	void OnPostLoadDatabases()
	{
		...
		/* Return if we were unable to open the database */
		if (!in)
			return;
		...
		{
			Anope::string ignore_type = myStrGetToken(readbuf, ' ', 0), ignore_name = myStrGetToken(readbuf, ' ', 1);
			if (!ignore_type.empty() && !ignore_name.empty())
			{
				/* Make sure to get rid of the trailing \n */
				ignore_name.erase(ignore_name.end() - 1);
		...
		DeleteFile(DiceServDBName.c_str());

		if (debug)
			Log() << "[diceserv] Loaded old database, it has been deleted and ignore data will now be stored as metadata in main database.";
	}

	EventReturn OnDatabaseReadMetadata(NickCore *nc, const Anope::string &key, const std::vector<Anope::string> &params)
	{
		if (key.equals_ci("DICESERV_IGNORE"))
		{
			nc->Shrink("diceserv_ignore");
			nc->Extend("diceserv_ignore", new ExtensibleItemRegular<bool>(true));
		}
	...
	void OnNickRegister(NickAlias *na)
	{
		...
		if (u->GetExt("diceserv_ignore"))
			u->Account()->Extend("diceserv_ignore", new ExtensibleItemRegular<bool>(true));
	}
	...
	{
		if (ci->c && ci->c->GetExt("diceserv_ignore"))
			ci->Extend("diceserv_ignore", new ExtensibleItemRegular<bool>(true));
	}

	void OnJoinChannel(User *u, Channel *c)
	{
		if (c->ci && c->ci->GetExt("diceserv_ignore"))
		{
			c->Shrink("diceserv_ignore");
			c->Extend("diceserv_ignore", new ExtensibleItemRegular<bool>(true));
		}
	...
	{
		if (!command.equals_ci("dnd3echar") && params.empty())
			return;

		Anope::string origdice, comment;
		...
		if (command.equals_ci("roll"))
		{
			origdice = params.find(' ') == Anope::string::npos ? params : myStrGetToken(params, ' ', 0);
			comment = params.find(' ') == Anope::string::npos ? "" : myStrGetTokenRemainder(params, ' ', 1);
			dice_type = DICE_TYPE_ROLL;
		}
		else if (command.equals_ci("calc"))
		{
			origdice = params.find(' ') == Anope::string::npos ? params : myStrGetToken(params, ' ', 0);
			comment = params.find(' ') == Anope::string::npos ? "" : myStrGetTokenRemainder(params, ' ', 1);
		...
		else if (command.equals_ci("exroll"))
		{
			origdice = params.find(' ') == Anope::string::npos ? params : myStrGetToken(params, ' ', 0);
			comment = params.find(' ') == Anope::string::npos ? "" : myStrGetTokenRemainder(params, ' ', 1);
			dice_type = origdice.empty() || origdice == "%" || origdice.find_ci('d') != Anope::string::npos || origdice.find_ci("rand(") != Anope::string::npos ? DICE_TYPE_EXROLL : DICE_TYPE_ROLL;
		}
		else if (command.equals_ci("excalc"))
		{
			origdice = params.find(' ') == Anope::string::npos ? params : myStrGetToken(params, ' ', 0);
			comment = params.find(' ') == Anope::string::npos ? "" : myStrGetTokenRemainder(params, ' ', 1);
			dice_type = origdice.empty() || origdice == "%" || origdice.find_ci('d') != Anope::string::npos || origdice.find_ci("rand(") != Anope::string::npos ? DICE_TYPE_EXCALC : DICE_TYPE_CALC;
			round_result = false;
		}
		else if (command.equals_ci("dnd3echar"))
		{
			if (!params.empty())
				comment = params;
			dice_type = DICE_TYPE_DND3E;
		}
		else if (command.equals_ci("earthdawn"))
		{
			origdice = params.find(' ') == Anope::string::npos ? params : myStrGetToken(params, ' ', 0);
			comment = params.find(' ') == Anope::string::npos ? "" : myStrGetTokenRemainder(params, ' ', 1);
		...
		SourceIsBot = true;
		diceserv_roller(u, origdice, ci->name, comment, dice_type, round_result);
	}
 private:
	void NickEvent(User *u)
	{
		if (u->Account() && u->Account()->GetExt("diceserv_ignore"))
			u->Extend("diceserv_ignore", new ExtensibleItemRegular<bool>(true));
	}
};
