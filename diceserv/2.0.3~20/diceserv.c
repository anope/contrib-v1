 ...
 * version.
 * ----------------------------------------------------------------------------
 * Requires: Anope-1.9.2, Anope-1.9.2-p1, or Anope-1.9.2-p2
 *           (Will not work with earlier or any other 1.9.x)
 * ----------------------------------------------------------------------------
 * Description:
 *
...
static void diceserv_roller(User *u, const ci::string &origdice, const ci::string &chan, const ci::string &comment, DICE_TYPES type, bool round_result = true)
{
	...
	MaxMessageLength -= bi->nick.size();
	MaxMessageLength -= bi->user.size();
	MaxMessageLength -= bi->host.size();
	...
	MaxMessageLength -= inchan ? chancpy.size() : u->nick.size(); /* inchan uses channel's name, otherwise uses user's nick */
	...
	if (inchan)
	{
		...
		MaxMessageLength -= u->nick.size(); /* nick of user who used the command */
	}
	if (!commentstr.empty())
...
class CommandDiceServHelp : public Command
{
 public:
	...
	void OnSyntaxError(User *u, const ci::string &)
	{
		...
		me->NoticeLang(s_DiceServ, u, DICE_HELP_FOOTER, s_DiceServ, Config.s_BotServ ? Config.s_BotServ : "BotServ", Config.s_BotServ ? Config.s_BotServ : "BotServ", s_DiceServ, s_DiceServ);
	}
};
...
{
 private:
	CommandReturn DoIgnore(User *u, const std::vector<ci::string> &params)
	{
		...
		if (*where == '#')
		{
			...
			else if (ci)
			{
				...
				if (is_servoper || (DiceServChanOpCanIgnore ? isOp : false) || (ci->HasFlag(CI_SECUREFOUNDER) ? IsRealFounder(u, ci) : IsFounder(u, ci)))
				{
		...
		else
		{
			...
			else if (!nu && (!na || (na && (na->HasFlag(NS_FORBIDDEN) || (na->nc && na->nc->HasFlag(NI_SUSPENDED))))))
 ...
 public:
	...
	bool OnHelp(User *u, const ci::string &subcommand)
	{
		...
		else if (subcommand == "IGNORE")
		{
			if (u->Account() && u->Account()->HasCommand("diceserv/set"))
				me->NoticeLang(s_DiceServ, u, DICE_SERVADMIN_HELP_SET_IGNORE, s_DiceServ, s_DiceServ, Config.s_ChanServ, Config.s_NickServ);
			else
				me->NoticeLang(s_DiceServ, u, DICE_HELP_SET_IGNORE, s_DiceServ, s_DiceServ, Config.s_ChanServ);
		}
		return true;
	}
	...
	void OnSyntaxError(User *u, const ci::string &subcommand)
	{
		if (subcommand.empty())
			...
			me->NoticeLang(s_DiceServ, u, u->Account() && u->Account()->HasCommand("diceserv/set") ? DICE_SERVADMIN_SET_IGNORE_SYNTAX : DICE_SET_IGNORE_SYNTAX);
	}
};
	...
	CommandReturn Execute(User *u, const std::vector<ci::string> &params)
	{
		...
		if (*what == '#')
		{
			...
			/* No registered channel was found, but a channel was, so show the data for that */
				...
				me->NoticeLang(s_DiceServ, u, DICE_STATUS_CHAN, what, me->GetLangString(u, c->GetExt("diceserv_ignore") ? DICE_STATUS_IGNORE : DICE_STATUS_ALLOW));
		}
		...
		else
		{
			...
			/* If the nick wasn't found and a NickServ entry wasn't found (or the nick is forbidden or suspended), display an error */
			if (!nu && (!na || (na && (na->HasFlag(NS_FORBIDDEN) || (na->nc && na->nc->HasFlag(NI_SUSPENDED))))))
			...
			else if (na)
			{
				...
				if (nu && nu->nick != na->nick)
					me->NoticeLang(s_DiceServ, u, DICE_STATUS_NICK_ONLINE, what, me->GetLangString(u, na->nc->GetExt("diceserv_ignore") ? DICE_STATUS_IGNORE : DICE_STATUS_ALLOW), nu->nick.c_str());
		...
		if (what == "CHANNELS")
		{
			...
			if (reg == REG_SHOW_UNREG || reg == REG_SHOW_ALL)
			{
				...
				for (; c; c = nextchan())
				{
					...
					if (display && (!stricmp(pattern, c->name.c_str()) || Anope::Match(c->name, pattern)) && ++shown <= 100)
					{
						if (reg == REG_SHOW_ALL)
							u->SendMessage(s_DiceServ, "   %-20s  %-5s  %s", c->name.c_str(), me->GetLangString(u, DICE_LIST_UNREG), me->GetLangString(u, diceserv_ignore ? DICE_IGNORED : DICE_ALLOWED));
						else
							u->SendMessage(s_DiceServ, "   %-20s  %s", c->name.c_str(), me->GetLangString(u, diceserv_ignore ? DICE_IGNORED : DICE_ALLOWED));
					}
				}
			}
				...
				for (; i < 256; ++i)
				{
					...
					for (; ci; ci = ci->next)
					{
						...
						if (diceserv_ignore && (show == SHOW_IGNORED || show == SHOW_ALL))
							...
							display = true;
						/* If we are to display the channel, make sure it matches the pattern first, and we haven't shown too many entries already */
						if (display && (!stricmp(pattern, ci->name.c_str()) || Anope::Match(ci->name, pattern)) && ++shown <= 100)
						{
							if (reg == REG_SHOW_ALL)
								u->SendMessage(s_DiceServ, "   %-20s  %-5s  %s", ci->name.c_str(), me->GetLangString(u, DICE_LIST_REG), me->GetLangString(u, diceserv_ignore ? DICE_IGNORED : DICE_ALLOWED));
							else
								u->SendMessage(s_DiceServ, "   %-20s  %s", ci->name.c_str(), me->GetLangString(u, diceserv_ignore ? DICE_IGNORED : DICE_ALLOWED));
						}
					}
				}
		}
		...
		else
		{
			...
			if (reg == REG_SHOW_UNREG || reg == REG_SHOW_ALL)
			{
				...
				for (; nu; nu = nextuser())
				{
					...
					if (display && (!stricmp(pattern, nu->nick.c_str()) || Anope::Match(nu->nick, pattern)) && ++shown <= 100)
					{
						if (reg == REG_SHOW_ALL)
							u->SendMessage(s_DiceServ, "   %-20s  %-5s  %s", nu->nick.c_str(), me->GetLangString(u, DICE_LIST_UNREG), me->GetLangString(u, diceserv_ignore ? DICE_IGNORED : DICE_ALLOWED));
						else
							u->SendMessage(s_DiceServ, "   %-20s  %s", nu->nick.c_str(), me->GetLangString(u, diceserv_ignore ? DICE_IGNORED : DICE_ALLOWED));
					}
				}
			}
				...
				for (; i < 1024; i++)
				{
					...
					for (; na; na = na->next)
					{
						...
						if (diceserv_ignore && (show == SHOW_IGNORED || show == SHOW_ALL))
							...
							display = true;
						/* If we are to display the nick, make sure it matches the pattern first, and we haven't shown too many entries already */
						if (display && (!stricmp(pattern, na->nick) || Anope::Match(na->nick, pattern, false)) && ++shown <= 100)
						{
							if (reg == REG_SHOW_ALL)
...
class DiceServCore : public Module
{
 public:
		...
		if (command == "INFO")
		{
			if (service == Config.s_NickServ)
			{
				...
				if (!nick.empty() && u->Account() && u->Account()->HasCommand("diceserv/info"))
				{
					...
					if (na)
						me->NoticeLang(Config.s_NickServ, u, DICE_INFO_IGNORE, "  ", s_DiceServ, me->GetLangString(u, na->nc->GetExt("diceserv_ignore") ? DICE_IGNORED : DICE_ALLOWED));
				}
			}
			else if (service == Config.s_ChanServ)
			{
				...
				if (!chan.empty())
				{
					...
					if (ci && ((ci->HasFlag(CI_SECUREFOUNDER) ? IsRealFounder(u, ci) : IsFounder(u, ci)) || (u->Account() && u->Account()->HasCommand("diceserv/info"))))
						me->NoticeLang(Config.s_ChanServ, u, DICE_INFO_IGNORE, "", s_DiceServ, me->GetLangString(u, ci->GetExt("diceserv_ignore") ? DICE_IGNORED : DICE_ALLOWED));
				}
			}
		}
	}
	...
	void OnPostLoadDatabases()
	{
		...
		if (debug)
			Alog() << "[diceserv] Loaded old database, it has been deleted and ignore data will now be stored as metadata in main database.";
	}

	EventReturn OnDatabaseReadMetadata(NickCore *nc, const std::string &key, const std::vector<std::string> &params)
	{
		if (key == "DICESERV_IGNORE")
		{
	...
	void OnBotFantasy(char *command, User *u, ChannelInfo *ci, char *params)
	{
		...
		SourceIsBot = true;
		diceserv_roller(u, origdice ? origdice : "", ci->name.c_str(), comment ? comment : "", dice_type, round_result);
