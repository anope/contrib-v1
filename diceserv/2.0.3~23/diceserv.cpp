 ...
 * version.
 * ----------------------------------------------------------------------------
 * Requires: Anope-1.9.5 (Will not work with earlier or any other 1.9.x)
 * ----------------------------------------------------------------------------
 * Description:
 *
 ...
 *
 * ----- Put these lines in your modules.conf ---------------------------------
...
// Give DiceServ a help command
command { service = "DiceServ"; name = "HELP"; command = "generic/help"; }

// Give DiceServ it's main commands
...
commnad { service = "DiceServ"; name = "EARTHDAWN"; command = "diceserv/earthdawn"; }
...
static void diceserv_roller(CommandSource &source, const Anope::string &origdice, const Anope::string &chan, const Anope::string &comment, DICE_TYPES type, bool round_result = true)
{
	...
	{
		if (!SourceIsBot)
			source.Reply(syntax_string(type).c_str());
		return;
	}
	...
	if (tilde != Anope::string::npos)
	{
		...
		{
			source.Reply(_("step for an Earthdawn roll must be a number."));
			return;
		}
		...
		if (DiceErrCode == DICE_ERR_NONE)
		{
			...
			if (n > DICE_MAX_TIMES)
			{
				source.Reply(_("The number of times that you entered (%d) was over the\nlimit of %d. Please enter a lower number of times."), n, DICE_MAX_TIMES);
				return;
			}
			else if (n <= 0)
			{
				source.Reply(_("The number of times that you entered (%d) was under\n1. Please enter a number between 1 and %d."), n, DICE_MAX_TIMES);
				return;
			}
		}
		/* Check for errors */
	...
	else
	{
		...
		if (type == DICE_TYPE_EARTHDAWN)
		{
			...
			if (num < 1 || num > 100)
			{
				source.Reply(_("The step you entered (%d) was out of range, it must be\nbetween 1 and 100."), num);
				return;
			}
			step = num;
			if (!dice.empty())
	...
	if (!error)
	{
		...
		do
		{
			...
			{
				source.Reply(DnDmodadd() <= 0 ? _("D&D 3e Character roll resulted in a character that had their\n"
	...
	if (MaxMessageLength - static_cast<int>(buf.length()) >= 0)
	{
		...
		if (DiceEx == 2 && MaxMessageLength - static_cast<int>(buf.length()) - static_cast<int>(shortextmp.length()) < 0)
			DiceEx = 3;
		if (DiceEx == 3)
			source.Reply(_("Too many results for %s to display, omitting results."), type == DICE_TYPE_EXROLL ? "EXROLL" : (type == DICE_TYPE_EXCALC ? "EXCALC" : "DND3ECHAR"));
	}
	else
	{
		source.Reply(_("Dice result buffer has an overflow. This could be due to\n"
	...
	if (DiceErrCode == DICE_ERR_NONE && !error)
	{
		...
		{
			if (type == DICE_TYPE_DND3E)
				source.Reply(_("<DnD3e Character roll [%s]: %s%s>%s%s"), origdicecpy.c_str(), exbuffer.c_str(), buf.c_str(), !commentstr.empty() ? " " : "", !commentstr.empty() ? commentstr.c_str() : "");
			else if (type == DICE_TYPE_EARTHDAWN)
				source.Reply(_("<Earthdawn roll [Step %u (%s)]: %s%s>%s%s"), step, origdicecpy.c_str(), exbuffer.c_str(), buf.c_str(), !commentstr.empty() ? " " : "", !commentstr.empty() ? commentstr.c_str() : "");
			else
				source.Reply(DiceEx == 1 || DiceEx == 2 ? (type == DICE_TYPE_EXCALC ? _("<Excalc [%s]: %s%s>%s%s") : _("<Exroll [%s]: %s%s>%s%s")) :
					(type == DICE_TYPE_CALC ? _("<Calc [%s]: %s%s>%s%s") : _("<Roll [%s]: %s%s>%s%s")), origdice.c_str(), exbuffer.c_str(), buf.c_str(), !commentstr.empty() ? " " : "",
...
{
 public:
	CommandDiceServRoll(Module *creator) : Command(creator, "diceserv/roll", 1, 3)
	{
	...
	{
		if (subcommand.empty())
			source.Reply(_("Syntax: ROLL dice [[channel] comment]\n"
				" \n"
				...
				"    /msg %s ROLL 3~3d6"), s_DiceServ.c_str(), s_DiceServ.c_str(), s_DiceServ.c_str(), s_DiceServ.c_str(), s_DiceServ.c_str(), s_DiceServ.c_str());
		else if (subcommand.equals_ci("EXPRESSIONS"))
			source.Reply(_("Dice expression syntax\n"
				" \n"
...
{
 public:
	CommandDiceServCalc(Module *creator) : Command(creator, "diceserv/calc", 1, 3)
	{
		this->SetFlag(CFLAG_ALLOW_UNREGISTERED);
	...
	{
		source.Reply(_("Syntax: CALC dice [[channel] comment]\n"
			" \n"
...
{
 public:
	CommandDiceServExRoll(Module *creator) : Command(creator, "diceserv/exroll", 1, 3)
	{
		this->SetFlag(CFLAG_ALLOW_UNREGISTERED);
	...
	bool OnHelp(CommandSource &source, const Anope::string &)
	{
		source.Reply(_("Syntax: EXROLL dice [[channel] comment]\n"
			" \n"
...
{
 public:
	CommandDiceServExCalc(Module *creator) : Command(creator, "diceserv/excalc", 1, 3)
	{
		this->SetFlag(CFLAG_ALLOW_UNREGISTERED);
	...
	bool OnHelp(CommandSource &source, const Anope::string &)
	{
		source.Reply(_("Syntax: EXCALC dice [[channel] comment]\n"
			" \n"
...
{
 public:
	CommandDiceServDnD3eChar(Module *creator) : Command(creator, "diceserv/dnd3echar", 0, 2)
	{
		this->SetFlag(CFLAG_ALLOW_UNREGISTERED);
	...
	bool OnHelp(CommandSource &source, const Anope::string &)
	{
		source.Reply(_("Syntax: DND3ECHAR [[channel] comment]\n"
			" \n"
...
{
 public:
	CommandDiceServEarthdawn(Module *creator) : Command(creator, "diceserv/earthdawn", 1, 3)
	{
		this->SetFlag(CFLAG_ALLOW_UNREGISTERED);
	...
	bool OnHelp(CommandSource &source, const Anope::string &)
	{
		source.Reply(_("Syntax: EARTHDAWN step[+karma] [[channel] comment]\n"
			" \n"
	...
	void OnSyntaxError(CommandSource &source, const Anope::string &)
	{
		source.Reply(_("Syntax: EARTHDAWN step [[channel] comment]"));
	}
};
...
{
 private:
	void DoIgnore(CommandSource &source, const std::vector<Anope::string> &params)
	{
		...
		if (where[0] == '#')
		{
			...
			if (!c && (!ci || (ci && ci->HasFlag(CI_SUSPENDED))))
			...
			else if (ci)
			{
				...
				bool isOp = ci->AccessFor(source.u).HasPriv(CA_AUTOOP);
				if (is_servoper || (DiceServChanOpCanIgnore ? isOp : false) || (ci->HasFlag(CI_SECUREFOUNDER) ? IsFounder(source.u, ci) : ci->AccessFor(source.u).HasPriv(CA_FOUNDER)))
				{
					...
					source.Reply(mode == IGNORE_ADD ? _("%s will now ignore all dice rolls sent to %s.") : _("%s will now allow all dice rolls sent to %s."), s_DiceServ.c_str(), where.c_str());
				}
		...
		else
		{
			...
			else if (bot || (!nu && (!na || (na && na->nc && na->nc->HasFlag(NI_SUSPENDED)))))
			...
			else if (na)
			{
				...
				source.Reply(mode == IGNORE_ADD ? _("%s will now ignore all dice rolls by %s.") : _("%s will now allow all dice rolls by %s."), s_DiceServ.c_str(), where.c_str());
			}
 ...
 public:
	CommandDiceServSet(Module *creator) : Command(creator, "diceserv/set", 3, 3)
	{
	...
	void Execute(CommandSource &source, const std::vector<Anope::string> &params)
	{
		...
		{
			this->DoIgnore(source, params);
			return;
		}
		/* Unknown SET command given */
		else
		{
			source.Reply(_("Unknown SET option %s."), cmd.c_str());
	...
	{
		if (subcommand.empty())
			source.Reply(_("Syntax: SET option parameters\n"
				" \n"
		...
		else if (subcommand.equals_ci("IGNORE"))
		{
			if (source.u->Account() && source.u->HasCommand("diceserv/set"))
				source.Reply(_("Syntax: SET IGNORE {channel|nick} {ON|OFF}\n"
					" \n"
					...
					"as the user is online."), s_DiceServ.c_str(), s_DiceServ.c_str(), !Config->ChanServ.empty() ? Config->ChanServ.c_str() : "ChanServ", !Config->NickServ.empty() ? Config->NickServ.c_str() : "NickServ");
			else
				source.Reply(_("Syntax: SET IGNORE channel {ON|OFF}\n"
					" \n"
					...
					"active."), s_DiceServ.c_str(), s_DiceServ.c_str(), !Config->ChanServ.empty() ? Config->ChanServ.c_str() : "ChanServ");
		}
		return true;
	}
	...
	void OnSyntaxError(CommandSource &source, const Anope::string &subcommand)
	{
		if (subcommand.empty())
			...
			source.Reply(source.u->HasCommand("diceserv/set") ? _("Syntax: SET IGNORE {channel|nick} {ON|OFF}") : _("Syntax: SET IGNORE channel {ON|OFF}"));
	}
};
...
{
 public:
	CommandDiceServStatus(Module *creator) : Command(creator, "diceserv/status", 1, 1)
	{
	...
	void Execute(CommandSource &source, const std::vector<Anope::string> &params)
	{
		...
		if (what[0] == '#')
		{
			...
			/* If we found a registered channel, show the data for that */
				...
				source.Reply(_("Status for registered channel %s: %s"), what.c_str(), ci->GetExt("diceserv_ignore") ? _("Ignore") : _("Allow"));
			/* No registered channel was found, but a channel was, so show the data for that */
			else
				source.Reply(_("Status for unregistered channel %s: %s"), what.c_str(), c->GetExt("diceserv_ignore") ? _("Ignore") : _("Allow"));
		}
		...
		else
		{
			...
			/* If the nick wasn't found and a NickServ entry wasn't found (or the nick is suspended), display an error */
			if (bot || (!nu && (!na || (na && na->nc && na->nc->HasFlag(NI_SUSPENDED)))))
			...
			else if (na)
			{
				...
				if (nu && nu->nick != na->nick)
					source.Reply(_("Status for registered nick %s: %s\n  (online as %s)"), what.c_str(), na->nc->GetExt("diceserv_ignore") ? _("Ignore") : _("Allow"), nu->nick.c_str());
				/* Otherwise, they are not online, just show that it is registered and it's data */
				else
					source.Reply(_("Status for registered nick %s: %s"), what.c_str(), na->nc->GetExt("diceserv_ignore") ? _("Ignore") : _("Allow"));
			}
			/* No registered nick was found, but a user was, so show the data for that */
			else
				source.Reply(_("Status for unregistered nick %s: %s"), what.c_str(), nu->GetExt("diceserv_ignore") ? _("Ignore") : _("Allow"));
		}
	}

	bool OnHelp(CommandSource &source, const Anope::string &)
	{
		source.Reply(_("Syntax: STATUS {channel|nick}\n"
			" \n"
...
{
 public:
	CommandDiceServList(Module *creator) : Command(creator, "diceserv/list", 3, 4)
	{
	...
	void Execute(CommandSource &source, const std::vector<Anope::string> &params)
	{
		...
		/* Show the header */
		source.Reply(_("List of %s %s entries matching %s%s:"), show == SHOW_IGNORED ? _("ignored") : (show == SHOW_ALLOWED ? _("allowed") : _("all")), what.equals_ci("CHANNELS") ? _("channels") : _("nicks"),
		...
		else
		{
			...
			if (reg == REG_SHOW_REG || reg == REG_SHOW_ALL)
				for (nickalias_map::const_iterator it = NickAliasList.begin(), it_end = NickAliasList.end(); it != it_end; ++it)
				{
					...
					if (na->nc && na->nc->HasFlag(NI_SUSPENDED))
						continue;
					display = false;
					/* We will only show the channel if it was part of the ignore type request */
	...
	bool OnHelp(CommandSource &source, const Anope::string &)
	{
		source.Reply(_("Syntax: LIST {IGNORE|ALLOW|ALL} what pattern [{REG|UNREG}]\n"
			" \n"
...
class DiceServCore : public Module
{
 ...
 public:
	DiceServCore(const Anope::string &modname, const Anope::string &creator) : Module(modname, creator), cmdDSRoll(this), cmdDSCalc(this), cmdDSExRoll(this), cmdDSExCalc(this), cmdDSDnD3eChar(this), cmdDSEarthdawn(this),
		cmdDSSet(this), cmdDSStatus(this), cmdDSList(this)
	{
		Implementation events[] = {I_OnPreCommand, I_OnPostCommand, I_OnPreHelp, I_OnPostHelp, I_OnPostLoadDatabases, I_OnDatabaseReadMetadata, I_OnDatabaseWriteMetadata, I_OnUserConnect, I_OnUserNickChange, I_OnUserLogoff,
			I_OnNickRegister, I_OnDelCore, I_OnChanRegistered, I_OnJoinChannel, I_OnPrePartChannel, I_OnDelChan, I_OnPrivmsg, I_OnBotFantasy};
		...
		if (!DiceServ)
		{
			Anope::string hostname = "localhost.net";
			ConfigReader config;
			for (int i = 0, num = config.Enumerate("define"); i < num; ++i)
			{
				Anope::string defineName = config.ReadValue("define", "name", "", i);

				if (defineName.equals_ci("services.host"))
				{
					hostname = config.ReadValue("define", "value", "localhost.net", i);
					break;
				}
			}
			DiceServ = new BotInfo(s_DiceServ, "services", hostname, "Dice Roll Service");
		}

		ModuleManager::RegisterService(&this->cmdDSRoll);
		ModuleManager::RegisterService(&this->cmdDSCalc);
		ModuleManager::RegisterService(&this->cmdDSExRoll);
		ModuleManager::RegisterService(&this->cmdDSExCalc);
		ModuleManager::RegisterService(&this->cmdDSDnD3eChar);
		ModuleManager::RegisterService(&this->cmdDSEarthdawn);
		ModuleManager::RegisterService(&this->cmdDSSet);
		ModuleManager::RegisterService(&this->cmdDSStatus);
		ModuleManager::RegisterService(&this->cmdDSList);

		DiceServ->SetCommand("HELP", "generic/help");
		DiceServ->SetCommand("ROLL", "diceserv/roll");
		DiceServ->SetCommand("CALC", "diceserv/calc");
		DiceServ->SetCommand("EXROLL", "diceserv/exroll");
		DiceServ->SetCommand("EXCALC", "diceserv/excalc");
		DiceServ->SetCommand("DND3ECHAR", "diceserv/dnd3echar");
		DiceServ->SetCommand("EARTHDAWN", "diceserv/earthdawn");
		DiceServ->SetCommand("SET", "diceserv/set");
		DiceServ->SetCommand("STATUS", "diceserv/status", "diceserv/status");
		DiceServ->SetCommand("LIST", "diceserv/list", "diceserv/list");
	...
	void OnPostCommand(CommandSource &source, Command *command, const std::vector<Anope::string> &params)
	{
		if (command->name.equals_ci("nickserv/info"))
		{
			...
			if (!nick.empty() && source.u->HasCommand("diceserv/info"))
			{
				...
				if (na)
					source.Reply(_("  %s Status: %s"), s_DiceServ.c_str(), na->nc->GetExt("diceserv_ignore") ? _("Ignored") : _("Allowed"));
			}
		}
		else if (command->name.equals_ci("chanserv/info"))
		{
			...
			if (!chan.empty())
			{
				...
				if (ci && ((ci->HasFlag(CI_SECUREFOUNDER) ? IsFounder(source.u, ci) : ci->AccessFor(source.u).HasPriv(CA_FOUNDER)) || source.u->HasCommand("diceserv/info")))
					source.Reply(_("%s Status: %s"), s_DiceServ.c_str(), ci->GetExt("diceserv_ignore") ? _("Ignored") : _("Allowed"));
			}
		}
	}
	...
	{
		if (source.owner->nick.equals_ci(s_DiceServ) && params.empty())
			source.Reply(_("%s allows you to roll any number of dice with any\n"
	...
	void OnPostHelp(CommandSource &source, const std::vector<Anope::string> &params)
	{
		if (source.owner->nick.equals_ci(s_DiceServ) && params.empty())
			source.Reply(_(" \n"
				...
				"to #DiceServ on jenna.cyberbotx.com."), s_DiceServ.c_str(), !Config->BotServ.empty() ? Config->BotServ.c_str() : "BotServ", !Config->BotServ.empty() ? Config->BotServ.c_str() : "BotServ",
	...
	/** Partial duplication of bs_main's OnPrivmsg to handle DiceServ's specific fantasy, due to incorrect handling of fantasy in bs_main.
	 */
	void OnPrivmsg(User *u, Channel *c, Anope::string &msg)
	{
		...
		if (c->ci->botflags.HasFlag(BS_FANTASY) && realbuf[0] == Config->BSFantasyCharacter[0] && !was_action)
		{
			...
			if (!DiceServ->commands.count(command))
				return;

			if (c->ci->AccessFor(u).HasPriv(CA_FANTASIA))
				this->OnBotFantasy(command, u, c->ci, rest);
		}
	}
