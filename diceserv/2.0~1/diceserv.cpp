 ...
 * Version : 2.0
 * Date    : (Last modified) August 7th, 2011
 * ----------------------------------------------------------------------------
...
class CommandDiceServSet : public Command
{
 private:
	CommandReturn DoIgnore(User *u, const std::vector<Anope::string> &params)
	{
		...
		bool is_servoper = u->Account() && u->Account()->HasCommand("dicserv/set");
...
class DiceServCore : public Module
{
 ...
 public:
	DiceServCore(const Anope::string &modname, const Anope::string &creator) : Module(modname, creator)
	{
		...
		if (!DiceServ)
			DiceServ = new BotInfo(s_DiceServ, Config->ServiceUser, Config->ServiceHost, "Dice Roll Server");
