 ...
 * version.
 * ----------------------------------------------------------------------------
 * Requires: Anope-1.9.4 or Anope-1.9.4-p1 (Will not work with earlier or any
 *                                          other 1.9.x)
 * ----------------------------------------------------------------------------
 * Description:
 *
 ...
 */
static bool ErrorHandler(CommandSource &source, const Anope::string &dice, unsigned ErrorPos)
{
	bool WasError = true;
	switch (DiceErrCode)
	{
		case DICE_ERR_PARSE:
		{
			me->SendMessage(source, _("During parsing, an error was found in the following\nexpression:"));
			source.Reply(" %s", dice.c_str());
			Anope::string spaces(ErrorPos > dice.length() ? dice.length() : ErrorPos, ' ');
			source.Reply("(%s^)", spaces.c_str());
			me->SendMessage(source, _("Error description is as follows:"));
			source.Reply("%s", DiceErrStr.c_str());
			break;
		}
		case DICE_ERR_DIV0:
			me->SendMessage(source, _("Division by 0 in following expression:"));
			source.Reply(" %s", dice.c_str());
			break;
		case DICE_ERR_UNDEFINED:
			me->SendMessage(source, _("Undefined result in following expression:"));
			source.Reply(" %s", dice.c_str());
			break;
		case DICE_ERR_UNACCEPT_DICE:
			if (DiceErrNum <= 0)
				me->SendMessage(source, _("The number of dice that you entered (%d) was under\n1. Please enter a number between 1 and %d."), DiceErrNum, DICE_MAX_DICE);
			else
				me->SendMessage(source, _("The number of dice that you entered (%d) was over the\nlimit of %d. Please enter a lower number of dice."), DiceErrNum, DICE_MAX_DICE);
			break;
		case DICE_ERR_UNACCEPT_SIDES:
			if (DiceErrNum <= 0)
				me->SendMessage(source, _("The number of sides that you entered (%d) was under\n1. Please enter a number between 1 and %d."), DiceErrNum, DICE_MAX_SIDES);
			else
				me->SendMessage(source, _("The number of sides that you entered (%d) was over the\nlimit of %d. Please enter a lower number of sides."), DiceErrNum, DICE_MAX_SIDES);
			break;
		case DICE_ERR_OVERUNDERFLOW:
			me->SendMessage(source, _("Dice results in following expression resulted in either\noverflow or underflow:"));
			source.Reply(" %s", dice.c_str());
			break;
		case DICE_ERR_STACK:
			me->SendMessage(source, _("The following roll expression could not be properly\nevaluated, please try again or let an administrator know."));
			source.Reply(" %s", dice.c_str());
			me->SendMessage(source, _("Error description is as follows:"));
			source.Reply("%s", DiceErrStr.c_str());
			break;
 ...
 */
static void diceserv_roller(CommandSource &source, const Anope::string &origdice, const Anope::string &chan, const Anope::string &comment, DICE_TYPES type, bool round_result = true)
{
	...
	if (source.u->GetExt("diceserv_ignore"))
		return;
	if (source.u->Account() && source.u->Account()->GetExt("diceserv_ignore"))
		return;
	...
	{
		if (!SourceIsBot)
			me->SendMessage(source, syntax_string(type).c_str());
		return;
	}
	...
	if (inchan)
	{
		...
		if (c)
		{
			if (!c->FindUser(source.u))
			{
				if (!SourceIsBot)
				{
					source.Reply(CHAN_X_INVALID, chancpy.c_str());
					me->SendMessage(source, syntax_string(type).c_str());
				}
				return;
			}
			...
			if (c->GetExt("diceserv_ignore") || found_ci_ignore || c->FindUser(source.u)->Status->HasFlag(CMODE_MODERATED))
			{
				inchan = false;
				chancpy.clear();
			}
	...
	MaxMessageLength -= inchan ? chancpy.length() : source.u->nick.length(); /* inchan uses channel's name, otherwise uses user's nick */
	...
	if (inchan)
	{
		...
		MaxMessageLength -= source.u->nick.length(); /* nick of user who used the command */
	}
	if (!commentstr.empty())
	...
	{
		me->SendMessage(source, _("Dice result buffer has an overflow. This could be due to\n"
	...
	if (tilde != Anope::string::npos)
	{
		...
		{
			me->SendMessage(source, _("step for an Earthdawn roll must be a number."));
			return;
		}
		...
		if (times.empty() || dice.empty())
		{
			DiceErrCode = DICE_ERR_PARSE;
			DiceErrStr = _("An empty number was found.");
			ErrorHandler(source, origdicecpy, tilde);
			return;
		}
		...
		{
			ErrorHandler(source, origdicecpy, DiceErrPos);
			return;
		}
		...
		if (DiceErrCode == DICE_ERR_NONE)
		{
			...
			if (n > DICE_MAX_TIMES)
			{
				me->SendMessage(source, _("The number of times that you entered (%d) was over the\nlimit of %d. Please enter a lower number of times."), n, DICE_MAX_TIMES);
				return;
			}
			else if (n <= 0)
			{
				me->SendMessage(source, _("The number of times that you entered (%d) was under\n1. Please enter a number between 1 and %d."), n, DICE_MAX_TIMES);
				return;
			}
		}
		/* Check for errors */
		error = ErrorHandler(source, origdicecpy, DiceErrPos);
	}
	...
	else
	{
		...
		if (type == DICE_TYPE_EARTHDAWN)
		{
			...
			if (num < 1 || num > 100)
			{
				me->SendMessage(source, _("The step you entered (%d) was out of range, it must be\nbetween 1 and 100."), num);
				return;
			}
			step = num;
			if (!dice.empty())
	...
	if (!error)
	{
		...
		{
			ErrorHandler(source, origdicecpy, (tilde ? tilde + 1 : 0) + DiceErrPos);
			return;
		}
		...
		do
		{
			...
			for (; n > 0; --n, DnDPos = 0)
			{
				...
				error = ErrorHandler(source, origdicecpy, (tilde ? tilde + 1 : 0) + DiceErrPos);
			...
			{
				me->SendMessage(source, DnDmodadd() <= 0 ? _("D&D 3e Character roll resulted in a character that had their\n"
	...
	if (MaxMessageLength - static_cast<int>(buf.length()) >= 0)
	{
		...
		if (DiceEx == 2 && MaxMessageLength - static_cast<int>(buf.length()) - static_cast<int>(shortextmp.length()) < 0)
			DiceEx = 3;
		if (DiceEx == 3)
			me->SendMessage(source, _("Too many results for %s to display, omitting results."), type == DICE_TYPE_EXROLL ? "EXROLL" : (type == DICE_TYPE_EXCALC ? "EXCALC" : "DND3ECHAR"));
	}
	else
	{
		me->SendMessage(source, _("Dice result buffer has an overflow. This could be due to\n"
	...
	if (DiceErrCode == DICE_ERR_NONE && !error)
	{
		...
		if (inchan)
		{
			...
			{
				if (SourceIsBot)
					ircdproto->SendPrivmsg(findbot(RollSource), chancpy.c_str(), "<Earthdawn roll for %s [Step %u (%s)]: %s%s>%s%s", source.u->nick.c_str(), step, origdicecpy.c_str(), exbuffer.c_str(), buf.c_str(),
						!commentstr.empty() ? " " : "", !commentstr.empty() ? commentstr.c_str() : "");
				else
					ircdproto->SendNotice(DiceServ, chancpy.c_str(), "<Earthdawn roll for %s [Step %u (%s)]: %s%s>%s%s", source.u->nick.c_str(), step, origdicecpy.c_str(), exbuffer.c_str(), buf.c_str(),
						!commentstr.empty() ? " " : "", !commentstr.empty() ? commentstr.c_str() : "");
			}
			else
			{
				if (SourceIsBot)
					ircdproto->SendPrivmsg(findbot(RollSource), chancpy.c_str(), "<%s for %s [%s]: %s%s>%s%s",
						type == DICE_TYPE_ROLL ? "Roll" : (type == DICE_TYPE_CALC ? "Calc" : (type == DICE_TYPE_EXROLL ? "Exroll" : (type == DICE_TYPE_EXCALC ? "Excalc" : "DnD3e Character Roll"))), source.u->nick.c_str(),
		...
		{
			if (type == DICE_TYPE_DND3E)
				me->SendMessage(source, _("<DnD3e Character roll [%s]: %s%s>%s%s"), origdicecpy.c_str(), exbuffer.c_str(), buf.c_str(), !commentstr.empty() ? " " : "", !commentstr.empty() ? commentstr.c_str() : "");
			else if (type == DICE_TYPE_EARTHDAWN)
				me->SendMessage(source, _("<Earthdawn roll [Step %u (%s)]: %s%s>%s%s"), step, origdicecpy.c_str(), exbuffer.c_str(), buf.c_str(), !commentstr.empty() ? " " : "", !commentstr.empty() ? commentstr.c_str() : "");
			else
...
class CommandDiceServHelp : public Command
{
 public:
	CommandDiceServHelp() : Command("HELP", 1, 1)
	{
		...
		this->SetDesc(_("Displays this list and gives information about commands"));
	}
	...
	CommandReturn Execute(CommandSource &source, const std::vector<Anope::string> &params)
	{
		...
		if (cmd.equals_ci("FUNCTIONS"))
			me->SendMessage(source, _("%s recognizes the following functions:\n"
				" \n"
			...
			mod_help_cmd(DiceServ, source.u, NULL, cmd);
	...
	{
		me->SendMessage(source, _("%s allows you to roll any number of dice with any\n"
		...
		for (CommandMap::const_iterator it = DiceServ->Commands.begin(), it_end = DiceServ->Commands.end(); it != it_end; ++it)
			if (!Config->HidePrivilegedCommands || it->second->permission.empty() || source.u->HasCommand(it->second->permission))
...
class CommandDiceServRoll : public Command
{
 public:
	...
	CommandReturn Execute(CommandSource &source, const std::vector<Anope::string> &params)
	{
		...
		SourceIsBot = false;
		diceserv_roller(source, origdice, chan, comment, DICE_TYPE_ROLL);
		return MOD_CONT;
	}
	...
	{
		if (subcommand.empty())
			me->SendMessage(source, _("Syntax: ROLL dice [[channel] comment]\n"
				" \n"
			...
			me->SendMessage(source, _("Dice expression syntax\n"
				" \n"
...
class CommandDiceServCalc : public Command
{
 public:
	...
	CommandReturn Execute(CommandSource &source, const std::vector<Anope::string> &params)
	{
		Anope::string origdice = params[0], chan = params.size() > 1 ? params[1] : "", comment = params.size() > 2 ? params[2] : "";
		SourceIsBot = false;
		diceserv_roller(source, origdice, chan, comment, DICE_TYPE_CALC, false);
		return MOD_CONT;
	}

	bool OnHelp(CommandSource &source, const Anope::string &)
	{
		me->SendMessage(source, _("Syntax: CALC dice [[channel] comment]\n"
			" \n"
...
class CommandDiceServExRoll : public Command
{
 public:
	...
	CommandReturn Execute(CommandSource &source, const std::vector<Anope::string> &params)
	{
		Anope::string origdice = params[0], chan = params.size() > 1 ? params[1] : "", comment = params.size() > 2 ? params[2] : "";
		SourceIsBot = false;
		diceserv_roller(source, origdice, chan, comment,
			origdice.empty() || origdice == "%" || origdice.find_ci('d') != Anope::string::npos || origdice.find_ci("rand(") != Anope::string::npos ? DICE_TYPE_EXROLL : DICE_TYPE_ROLL);
		return MOD_CONT;
	}

	bool OnHelp(CommandSource &source, const Anope::string &)
	{
		me->SendMessage(source, _("Syntax: EXROLL dice [[channel] comment]\n"
			" \n"
...
class CommandDiceServExCalc : public Command
{
 public:
	...
	CommandReturn Execute(CommandSource &source, const std::vector<Anope::string> &params)
	{
		...
		SourceIsBot = false;
		diceserv_roller(source, origdice, chan, comment,
			origdice.empty() || origdice == "%" || origdice.find_ci('d') != Anope::string::npos || origdice.find_ci("rand(") != Anope::string::npos ? DICE_TYPE_EXCALC : DICE_TYPE_CALC, false);
		return MOD_CONT;
	}

	bool OnHelp(CommandSource &source, const Anope::string &)
	{
		me->SendMessage(source, _("Syntax: EXCALC dice [[channel] comment]\n"
			" \n"
...
class CommandDiceServDnD3eChar : public Command
{
 public:
	...
	{
		this->SetFlag(CFLAG_ALLOW_UNREGISTERED);
		this->SetDesc(_("Rolls dice for DnD3e character creation"));
	}
	...
	CommandReturn Execute(CommandSource &source, const std::vector<Anope::string> &params)
	{
		...
		SourceIsBot = false;
		diceserv_roller(source, "", chan, comment, DICE_TYPE_DND3E);
		return MOD_CONT;
	}

	bool OnHelp(CommandSource &source, const Anope::string &)
	{
		me->SendMessage(source, _("Syntax: DND3ECHAR [[channel] comment]\n"
			" \n"
...
class CommandDiceServEarthdawn : public Command
{
 public:
	...
	CommandReturn Execute(CommandSource &source, const std::vector<Anope::string> &params)
	{
		Anope::string origdice = params[0], chan = params.size() > 1 ? params[1] : "", comment = params.size() > 2 ? params[2] : "";
		SourceIsBot = false;
		diceserv_roller(source, origdice, chan, comment, DICE_TYPE_EARTHDAWN);
		return MOD_CONT;
	}

	bool OnHelp(CommandSource &source, const Anope::string &)
	{
		me->SendMessage(source, _("Syntax: EARTHDAWN step[+karma] [[channel] comment]\n"
			" \n"
	...
	void OnSyntaxError(CommandSource &source, const Anope::string &)
	{
		me->SendMessage(source, _("Syntax: EARTHDAWN step [[channel] comment]"));
	}
};
...
{
 private:
	CommandReturn DoIgnore(CommandSource &source, const std::vector<Anope::string> &params)
	{
		...
		bool is_servoper = source.u->HasCommand("diceserv/set");
		...
		if (where[0] == '#')
		{
			...
			else if (ci)
			{
				...
				bool isOp = check_access(source.u, ci, CA_AUTOOP);
				if (is_servoper || (DiceServChanOpCanIgnore ? isOp : false) || (ci->HasFlag(CI_SECUREFOUNDER) ? IsFounder(source.u, ci) : check_access(source.u, ci, CA_FOUNDER)))
				{
					...
					me->SendMessage(source, mode == IGNORE_ADD ? _("%s will now ignore all dice rolls sent to %s.") : _("%s will now allow all dice rolls sent to %s."), s_DiceServ.c_str(), where.c_str());
				}
			...
			else
			{
				...
				if (is_servoper || c->FindUser(source.u)->Status->HasFlag(CMODE_OP))
				{
					/* Either add or delete the ignore data */
					if (mode == IGNORE_ADD)
						c->Extend("diceserv_ignore", new ExtensibleItemRegular<bool>(true));
		...
		else
		{
			...
			/* If the nick wasn't found and a NickServ entry wasn't found (or the nick is forbidden or suspended), display an error */
				...
				me->SendMessage(source, _("Nick %s is not a valid nick."), where.c_str());
			...
			else if (na)
			{
				...
				if (!nu)
				{
					Anope::insensitive_map<User *>::const_iterator it = UserListByNick.begin(), it_end = UserListByNick.end();
					for (; it != it_end; ++it)
					{
						nu = it->second;
						if (nu->Account() == na->nc)
				...
				me->SendMessage(source, mode == IGNORE_ADD ? _("%s will now ignore all dice rolls by %s.") : _("%s will now allow all dice rolls by %s."), s_DiceServ.c_str(), where.c_str());
			}
 ...
 public:
	...
	{
		this->SetDesc(Anope::printf(_("Set options for %s access"), s_DiceServ.c_str()));
	}
	...
	CommandReturn Execute(CommandSource &source, const std::vector<Anope::string> &params)
	{
		...
		if (readonly)
		{
			me->SendMessage(source, _("Sorry, dice ignore option setting is temporarily disabled."));
			return MOD_CONT;
		}
		Anope::string cmd = params[0];
		/* SET IGNORE sub-command */
		if (cmd.equals_ci("IGNORE"))
			return this->DoIgnore(source, params);
		/* Unknown SET command given */
		else
		{
			me->SendMessage(source, _("Unknown SET option %s."), cmd.c_str());
			source.Reply(MORE_INFO, s_DiceServ.c_str(), "SET");
		}
		return MOD_CONT;
	}
	...
	{
		if (subcommand.empty())
			me->SendMessage(source, _("Syntax: SET option parameters\n"
				" \n"
		...
		{
			if (source.u->Account() && source.u->HasCommand("diceserv/set"))
				me->SendMessage(source, _("Syntax: SET IGNORE {channel|nick} {ON|OFF}\n"
					" \n"
				...
				me->SendMessage(source, _("Syntax: SET IGNORE channel {ON|OFF}\n"
					" \n"
	...
	void OnSyntaxError(CommandSource &source, const Anope::string &subcommand)
	{
		if (subcommand.empty())
			...
			me->SendMessage(source, source.u->HasCommand("diceserv/set") ? _("Syntax: SET IGNORE {channel|nick} {ON|OFF}") : _("Syntax: SET IGNORE channel {ON|OFF}"));
	}
};
...
class CommandDiceServStatus : public Command
{
 public:
	...
	CommandReturn Execute(CommandSource &source, const std::vector<Anope::string> &params)
	{
		...
		if (what[0] == '#')
		{
			...
			/* If we found a registered channel, show the data for that */
				...
				me->SendMessage(source, _("Status for registered channel %s: %s"), what.c_str(), ci->GetExt("diceserv_ignore") ? _("Ignore") : _("Allow"));
			/* No registered channel was found, but a channel was, so show the data for that */
			else
				me->SendMessage(source, _("Status for unregistered channel %s: %s"), what.c_str(), c->GetExt("diceserv_ignore") ? _("Ignore") : _("Allow"));
		}
		...
		else
		{
			...
			if (bot || (!nu && (!na || (na && (na->HasFlag(NS_FORBIDDEN) || (na->nc && na->nc->HasFlag(NI_SUSPENDED)))))))
				me->SendMessage(source, _("Nick %s is not a valid nick."), what.c_str());
			...
			else if (na)
			{
				...
				if (nu && nu->nick != na->nick)
					me->SendMessage(source, _("Status for registered nick %s: %s\n  (online as %s)"), what.c_str(), na->nc->GetExt("diceserv_ignore") ? _("Ignore") : _("Allow"), nu->nick.c_str());
				/* Otherwise, they are not online, just show that it is registered and it's data */
				else
					me->SendMessage(source, _("Status for registered nick %s: %s"), what.c_str(), na->nc->GetExt("diceserv_ignore") ? _("Ignore") : _("Allow"));
			}
			/* No registered nick was found, but a user was, so show the data for that */
			else
				me->SendMessage(source, _("Status for unregistered nick %s: %s"), what.c_str(), nu->GetExt("diceserv_ignore") ? _("Ignore") : _("Allow"));
		}
		return MOD_CONT;
	}
	...
	bool OnHelp(CommandSource &source, const Anope::string &)
	{
		me->SendMessage(source, _("Syntax: STATUS {channel|nick}\n"
			" \n"
...
class CommandDiceServList : public Command
{
 public:
	...
	{
		this->SetDesc(Anope::printf(_("Gives list of %s access"), s_DiceServ.c_str()));
	}
	...
	CommandReturn Execute(CommandSource &source, const std::vector<Anope::string> &params)
	{
		...
		/* Show the header */
		me->SendMessage(source, _("List of %s %s entries matching %s%s:"), show == SHOW_IGNORED ? _("ignored") : (show == SHOW_ALLOWED ? _("allowed") : _("all")),
		...
		else
		{
			...
			if (reg == REG_SHOW_UNREG || reg == REG_SHOW_ALL)
			{
				for (Anope::insensitive_map<User *>::const_iterator it = UserListByNick.begin(), it_end = UserListByNick.end(); it != it_end; ++it)
				{
		...
		/* Show the footer */
		me->SendMessage(source, _("End of list - %d/%d matches shown."), shown > 100 ? 100 : shown, shown);
		return MOD_CONT;
	}

	bool OnHelp(CommandSource &source, const Anope::string &)
	{
		me->SendMessage(source, _("Syntax: LIST {IGNORE|ALLOW|ALL} what pattern [{REG|UNREG}]\n"
			" \n"
...
class DiceServCore : public Module
{
 ...
 public:
	...
	{
		if (command->name.equals_ci("INFO"))
		{
			if (command->service == NickServ)
			{
				Anope::string nick = params.size() ? params[0] : "";

				if (!nick.empty() && source.u->HasCommand("diceserv/info"))
				{
					NickAlias *na = findnick(nick);
					if (na)
						me->SendMessage(source, _("  %s Status: %s"), s_DiceServ.c_str(), na->nc->GetExt("diceserv_ignore") ? _("Ignored") : _("Allowed"));
				}
			}
			else if (command->service == ChanServ)
			{
				...
				if (!chan.empty())
				{
					...
					if (ci && ((ci->HasFlag(CI_SECUREFOUNDER) ? IsFounder(source.u, ci) : check_access(source.u, ci, CA_FOUNDER)) || source.u->HasCommand("diceserv/info")))
						me->SendMessage(source, _("%s Status: %s"), s_DiceServ.c_str(), ci->GetExt("diceserv_ignore") ? _("Ignored") : _("Allowed"));
				}
			}
		}
	}
	...
	void OnBotFantasy(const Anope::string &command, User *u, ChannelInfo *ci, const Anope::string &params)
	{
		...
		source.u = u;
		source.owner = source.service = DiceServ;
		source.fantasy = true;
		diceserv_roller(source, origdice, ci->name, comment, dice_type, round_result);
		source.DoReply();
	}
 private:
