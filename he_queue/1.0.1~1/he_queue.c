...
int new_ticket(User *u, char *ticketmsg) {
   ...
   p->next = NULL;
   strncpy(p->nick, u->nick, sizeof(p->nick));
   p->messages = 0;
   strcpy(p->msg[0],"");
   strcpy(p->c_id, he_id);
...
int take_ticket(User * u, char *ticketbuffer) {
   ...
   if(ticket_u != NULL) {
      if(strcmp(p->helper,"") == 0) {
         strncpy(p->helper, u->nick, sizeof(p->helper));
...
int reassign_ticket(User *u, char *msg) {
   ...
   if(!helper) {
      moduleNoticeLang(s_HelpServ, u, LANG_REASSIGN_NOHELPER, helpername);
      free(ticketname);
      free(helpername);
      return MOD_CONT;
   }
   ...
   if(stricmp(p->nick,ticketname) != 0 && strlen(ticketname) > 0) {
      ...
      else
      {
         ...
         strncpy(p->helper, helper->nick, sizeof(p->helper));
         strcpy(p->helper_c_id, he_id);
      ...
      if(data != NULL) {
         while(p != NULL) {
            if(strcmp(data,p->c_id) == 0) {
               ...
               strncpy(p->nick, av[0], sizeof(p->nick));
               break;
            }
            else {
               if(strlen(p->helper) > 0) {
