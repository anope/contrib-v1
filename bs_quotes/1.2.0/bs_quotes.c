 ...
 * Contributor: CrazyCat - crazycat@c-p-f.org
 ...
 * QuoteMySQLTable "bs_quotes" - The table to use (optionnal)
 ...
 *
 * v1.0.0 - 14/06/2007 - Initial release
 *
 * v1.1.0 - 18/06/2007 - Added multiline support, when adding quotes use !n! to insert a new line
 ...
 * 
 * v1.1.1 - 20/06/2007 - Fix compile problem for MySQL <5.0.13 (MYSQL_OPT_RECONNECT)
 *
 * v1.2.0 - 10/11/2011 - Added parameter for table name (default is bs_quotes)
 *                     - Added field "channel" in table
 *                     - The info about the quotes shows the channel
 *
 **/
...
#define VERSION              "$Id: bs_quotes.c v1.2.0 10-11-2011 CrazyCat $"
...
#define QUOTE_TEMPLATE_BY    "[QUOTE INFO] Added by %s (%s) on the %s."
...
int AnopeInit(void)
{
  ...
  /* Create our tables */
  unsigned int ret = do_mysql_query("CREATE TABLE IF NOT EXISTS `%s` ("
                                    ...
                                    " PRIMARY KEY  (`id`)"
                                    " ) ENGINE=MyISAM  DEFAULT CHARSET=utf8;", Q_MYSQL_TABLE);
...
void AnopeFini(void)
{
  ...
  if (Q_MYSQL_TABLE){ free(Q_MYSQL_TABLE); Q_MYSQL_TABLE = NULL; }
...
int load_config(void)
{
  Directive confvalues[][1] = {
    ...
    {{ "QuoteMySQLTable",		 { { PARAM_SET,    PARAM_RELOAD, &Q_MYSQL_TABLE		  } } }}
  };
  ...
  if (Q_MYSQL_PASS) { free(Q_MYSQL_PASS); Q_MYSQL_PASS = NULL; }
  if (Q_MYSQL_DB)   { free(Q_MYSQL_DB);   Q_MYSQL_DB = NULL;   }
  if (Q_MYSQL_TABLE){ free(Q_MYSQL_TABLE); Q_MYSQL_TABLE = "bs_quotes"; }
...
void do_add_quote(ChannelInfo *ci, User *u, char *arg)
{
  ...
  /* Do query */
  unsigned int ret = do_mysql_query("INSERT INTO `%s`.`%s` ( `id` , `channel`, `user` , `quote`, `time` , `views` ) VALUES ( NULL , '%s', '%s', '%s', CURRENT_TIMESTAMP, 0 );", Q_MYSQL_DB, Q_MYSQL_TABLE, ci->name, user, quote);
...
void do_del_quote(ChannelInfo *ci, User *u, char *arg)
{
  ...
  unsigned int ret = do_mysql_query("DELETE FROM `%s`.`%s` WHERE id = %llu;", Q_MYSQL_DB, Q_MYSQL_TABLE, id_num);
  if (ret != 0)
  {
    anope_cmd_privmsg(ci->bi->nick, ci->name, QUOTE_MYSQL_ERR);
    return;
...
void do_view_quote(ChannelInfo *ci, User *u, char *arg)
{
  ...
  if (arg)
    ret = do_mysql_query("SELECT quote, channel, user, time FROM `%s`.`%s` WHERE id = %llu;", Q_MYSQL_DB, Q_MYSQL_TABLE, id_num);
  else
    ret = do_mysql_query("SELECT quote, channel, user, time, id FROM `%s`.`%s` ORDER BY RAND() LIMIT 1;", Q_MYSQL_DB, Q_MYSQL_TABLE);
  ...
  if (row)
  {
    char *quote = NULL, *channel = NULL, *user = NULL, *date = NULL, *id_str = NULL;

    /* Get results */
    unsigned long *lengths = mysql_fetch_lengths(mysql_result_set);
    quote = str("%.*s", (int) lengths[0], row[0] ? row[0] : "");
    channel = str("%.*s", (int) lengths[1], row[1] ? row[1] : "");
    user  = str("%.*s", (int) lengths[2], row[2] ? row[2] : "");
    id_str = arg ? str("%llu", id_num) : str("%.*s", (int) lengths[4], row[4] ? row[4] : "");

    /* Format the date */          
    date  = str("%.*s", (int) lengths[3], row[3] ? row[3] : "");
    int date_parts[6];
    ...
    /* Send quote by */
    anope_cmd_privmsg(ci->bi->nick, ci->name, QUOTE_TEMPLATE_BY, user, channel, date);
    ...
    do_mysql_query("UPDATE `%s`.`%s` SET views = views + 1 WHERE id = %llu;", Q_MYSQL_DB, Q_MYSQL_TABLE, id_num);
  }
  else
  {
    anope_cmd_privmsg(ci->bi->nick, ci->name, QUOTE_NO_QUOTE);
...
void do_search_quote(ChannelInfo *ci, User *u, char *arg)
{
  ...
  unsigned int ret = do_mysql_query("SELECT id FROM `%s`.`%s` WHERE quote LIKE '%%%s%%'", Q_MYSQL_DB, Q_MYSQL_TABLE, search_string);
  free(search_string);
...
{
  /* Run query */
  unsigned int ret = do_mysql_query("SELECT COUNT(id) AS ids FROM `%s`.`%s`;", Q_MYSQL_DB, Q_MYSQL_TABLE);
...
int do_mysql_connect()
{
  ...
  {
    /* Bug causes this to be reset, set it here */
    /* Use mysql_options() if possible for >5.0.13 */
