 ...
 *
 * v1.2.0 - 10/11/2011 - Added parameter for table name (default is anope_bs_quotes)
 *                     - Added field "channel" in table
 *                     - The info about the quotes shows the channel
 *
 * v1.2.1 - 15/12/2011 - Fix bug making crash services on reload
 **/
...
#define VERSION              "$Id: bs_quotes.c v1.1.1 20-06-2007 mikea $"
...
char *Q_MYSQL_TABLE = "anope_bs_quote";
...
void do_add_quote(ChannelInfo *ci, User *u, char *arg)
{
  ...
  /* Do query */
  unsigned int ret = do_mysql_query("INSERT INTO `%s`.`%s` ( `id` , `user` , `channel`, `quote`, `time` , `views` ) VALUES ( NULL , '%s', '%s', '%s', CURRENT_TIMESTAMP, 0 );", Q_MYSQL_DB, Q_MYSQL_TABLE, user, ci->name, quote);
...
void do_view_quote(ChannelInfo *ci, User *u, char *arg)
{
  ...
  if (arg)
    ret = do_mysql_query("SELECT quote, user, channel, time FROM `%s`.`%s` WHERE id = %llu;", Q_MYSQL_DB, Q_MYSQL_TABLE, id_num);
  else
    ret = do_mysql_query("SELECT quote, user, time, channel, id FROM `%s`.`%s` ORDER BY RAND() LIMIT 1;", Q_MYSQL_DB, Q_MYSQL_TABLE);
  ...
  if (row)
  {
    char *quote = NULL, *user = NULL, *chan = NULL, *date = NULL, *id_str = NULL;
    ...
    chan  = str("%.*s", (int) lengths[2], row[2] ? row[2] : "");
